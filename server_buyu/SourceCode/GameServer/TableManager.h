...
public:
	...
	//修改房间库存
	void OnAddTableGlobel(BYTE TableID, double nValue);
	double QueryTableTypePool(BYTE TableTypeID);//查询库存
	std::list<DWORD> GetBlackList();
	bool SetBlackList(DWORD *pid,BYTE byCount);
	bool Isabhor(DWORD dwUserid);
	//添加的黑白名单的检验
	bool IsBlackUser(DWORD deUserID);
	bool IsWhightUser(DWORD dwWUserId);
	bool  ReadBWIni();
	...
	double KucunNum[5];//库存
	DWORD  BlackUser[10];//黑名单
	DWORD	WightUser[10];//白名单
