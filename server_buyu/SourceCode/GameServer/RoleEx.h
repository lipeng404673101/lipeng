...
public:
	...
	void	OnUserLoadFinish(bool IsLogonGameServer);// 玩家登陆更新客户端信息金币变化tagRoleInfo* pUserInfo, tagRoleServerInfo* pUserServerInfo, 
	void	OnUserLoadFinish(tagRoleInfo* pUserInfo, tagRoleServerInfo* pUserServerInfo, bool IsLogonGameServer);
	...
	//重载新的增加金币函数    添加参数
	bool	ChangeRoleGlobe(DWORD AddGlobe, bool IsSendToClient = true, bool IsSaveToDB = false, bool IsSendToMiniGame = false, DWORD dwDaiLiUserID = 0ul, bool bIsCanContrl=false,bool bIsZongDai=true);
	//新加减少金币
	bool	ChangeRoleGlobeDown(DWORD DownGlobe, bool IsSendToClient = true, bool IsSaveToDB = false, bool IsSendToMiniGame = false, bool bIsCanContrl = false);
