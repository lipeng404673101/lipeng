	...
	case CMD_BULLET:
		{
			...
			if (player->GetRoleExInfo()->IsRobot())
			{
				...
				//}
				//LogInfoToFile("Control.txt", "机器人没钱开炮+++++++::::::%ld:::::::::%d:::::::::", player->GetScore(), player->GetRoleExInfo()->GetRoleVip().GetLauncherReBoundNum());
	...
	case CMD_USE_SKILL:
		{
			...
			SkillFailedType sft11 = pSetting->UseSkill(player->GetID(), (SkillType)skill->SkillID);
			// xiugai修改
			SkillFailedType sft = SFT_OK;
			if (sft == SFT_OK)
			{
...
void FishDesk::AsyncPlayerJoin(CRole* player, bool bAsyncScene)
{
	...
	if (bAsyncScene == false)
	{
		...
		TCHARCopy(ncp.PlayerInfo.szHeadHttp, CountArray(ncp.PlayerInfo.szHeadHttp), player->GetRoleExInfo()->GetRoleInfo().szHeadHttp, _tcslen(player->GetRoleExInfo()->GetRoleInfo().szHeadHttp));
	...
	for (BYTE i = 0; i < m_PlayerManager->GetMaxPlayerSum()/*m_nMaxSeatNum*/; ++i)
	{
		...
		if (pRole && pRole->IsActionUser() && pRole->GetID() != player->GetID())
		{
			...
			TCHARCopy(ncp.PlayerInfo.szHeadHttp, CountArray(ncp.PlayerInfo.szHeadHttp), pRole->GetRoleExInfo()->GetRoleInfo().szHeadHttp, _tcslen(pRole->GetRoleExInfo()->GetRoleInfo().szHeadHttp));
...
void FishDesk::ProcessSkillFreeze(CRole *player)
{
	...
	LogInfoToFile("CollideBuYu.txt", "技能变化金币 ---::::::%d *************%d********* %d----------", goldNum, player->GetRoleExInfo()->GetRoleInfo().dwGlobeNum, player->GetID());
}
