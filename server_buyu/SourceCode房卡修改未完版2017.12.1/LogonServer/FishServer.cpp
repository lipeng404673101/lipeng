	...
	case CL_AccountLogon:
		{
			...
			//先进行 版本 路径 的验证  验证成功 后 我们让玩家进行登陆 登陆必须携带 平台的ID  配置文件的版本号大于客户端版本号才生效
			...
			SendNetCmdToDB(&msg);
			LogInfoToFile("LogonError.txt", TEXT("DBR_QueryLogon 先向缓存请求"));
	...
	case CL_WeiXinLogon:
		{
			...
			LogInfoToFile("LogonError.txt", TEXT("玩家试图通过微信登陆"));
			if (!m_FishConfig.CheckVersionAndPathCrc(pMsg->VersionID, pMsg->PathCrc))
			{
				ASSERT(false);
				LC_Cmd_CheckVersionError msg;
			...
			LogInfoToFile("LogonError.txt", TEXT("玩家试图通过微信登陆1"));
			...
			LogInfoToFile("LogonError.txt", TEXT("玩家试图通过微信登陆2"));
			std::vector<char*> pVec;
			GetStringArrayVecByData(pVec, &pMsg->CodeInfo);

			LogInfoToFile("LogonError.txt", TEXT("接收微信登陆	玩家试图通过微信登陆 接收微信登陆接收微信登陆:::%s*****%d****%d*****%s*"), pMsg->NickName, pMsg->CodeInfo.HandleSum, pMsg->PlateFormID, pMsg->szHeadHttp);

			if (pVec.size() != pMsg->CodeInfo.HandleSum)
			{
				LogInfoToFile("LogonError.txt", TEXT("玩家试图通过微信登陆6"));
				//ASSERT(false);
				return;
			}
			LogInfoToFile("LogonError.txt", TEXT("玩家试图通过微信登陆7"));
			string OpenID = pVec[0];
			FreeVec(pVec);
			LogInfoToFile("LogonError.txt", TEXT("玩家试图通过微信登陆3"));
			//生成渠道数据添加进去
			...
			LogInfoToFile("LogonError.txt", TEXT("玩家试图通过微信登陆4"));
			...
			for (int i = length + 1; i < 32; ++i)
				Password[i] = 0xfefe;
			free(pOpenID);
			LogInfoToFile("LogonError.txt", TEXT("玩家试图通过微信登陆5"));
			AE_CRC_THREE pTree;
			...
			TCHARCopy(msg.szHeadHttp, CountArray(msg.szHeadHttp), pMsg->szHeadHttp, _tcslen(pMsg->szHeadHttp));
			...
			LogInfoToFile("LogonError.txt", TEXT("接收微信登陆 接收微信登陆接收微信登陆:::%s*****%s"), msg.AccountName, Password);
	...
	case CL_QueryLogon:
		{
			...
			// LogInfoToFile("LogonError.txt", TEXT("CL_QueryLogon 客户端进行快速登陆"));
			...
			if (!m_FishConfig.CheckStringIsError(pMsg->AccountName, ACCOUNT_MIN_LENGTH, ACCOUNT_LENGTH, SCT_AccountName))//敏感词
			{
				LC_Cmd_AccountOnlyID msgClient;
				SetMsgInfo(msgClient, GetMsgType(Main_Logon, LC_AccountOnlyID), sizeof(LC_Cmd_AccountOnlyID));
				msgClient.dwOnlyID = 0;
			...
			else
			{
				...
				else
				{
					...
					LogInfoToFile("LogonError.txt", TEXT("客户端进行快速登陆  玩家账号:%s 密码:%u,%u,%u"), pMsg->AccountName, pMsg->PasswordCrc1, pMsg->PasswordCrc2, pMsg->PasswordCrc3);
	...
	case DBO_AccountRsg:
		{
			...
			if (pMsg->dwUserID == 0)
			{
				...
				if (pMsg->dwBangDing==0)
				{
					msgClient.LogonType = 0;
				}
				else{
					msgClient.LogonType = 2;
			...
			LogInfoToFile("LogonError.txt", TEXT("注册消息+++++:******%d**********"), pMsg->dwUserID);
