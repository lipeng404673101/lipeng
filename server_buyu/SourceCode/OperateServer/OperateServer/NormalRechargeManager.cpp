...
bool NormalRechargeManager::OnHandleAddOrder(DWORD dwUserID, DWORD OrderID, DWORD ShopID)
{
	...
	LogInfoToFile("Chongzhi.txt", TEXT("OnHandleAddOrder向外部服务器直接下单***6666*****%u**"), cporderid);
	...
	//进行加密处理
	string prvKey = pConfig->ApppayPrvKey;
	//EVP_PKEY* vkey = CryptHelper::getKeyByPKCS1(prvKey, 1);//获取错误  目前是
	//if (!vkey)
	//{
	//	ASSERT(false);
	//	return false;
...
//	LogInfoToFile("Chongzhi.txt", TEXT("OnHandleAddOrder进行加密处理到哪里了********%u**"), vkey);
	//std::string reqSign;
	...
	////字符串处理完毕 我们进行加密
	////string Url = UrlEncode(DestUrl);//没有用
...
//	LogInfoToFile("Chongzhi.txt", TEXT("OnHandleAddOrder订单成功了********%s**"), DestUrl);
	...
	m_OrderMap.insert(HashMap<UINT64, OrderInfo>::value_type(orderInfo.OnlyID, orderInfo));//加入订单集合中
	OnHandleAddOrderResult(orderInfo.OnlyID, NULL, 0);
	...
	msg.OrderID = OrderID;
	msg.Price = Iter->second.dDisCountPrice;
	msg.CheckShopID = ShopID;
	msg.CheckUserID = dwUserID;
...
{
	LogInfoToFile("Chongzhi.txt", TEXT("OnHandleAddOrderResult充值订单入口*********"));
	...
	LogInfoToFile("Chongzhi.txt", TEXT("OnHandleAddOrderResult充值订单入口222*********"));
	...
	//	LogInfoToFile("Chongzhi.txt", TEXT("OnHandleAddOrderResult商户订单数据异常********%s**"), Url.c_str());
		return;
	}
	else if (pMap.count(m_NormalRechargeAddOrderCrcInfo.transidCrc) == 1 && pMap.count(m_NormalRechargeAddOrderCrcInfo.signCrc) == 1)
	{
		...
		Log(ClientSign);
		//////////////////////////////////////////////////////////////////////////
	//	LogInfoToFile("Chongzhi.txt", TEXT("OnHandleAddOrderResult给客户端发送的签名参数********%s*****"), ClientSign);
...
//	LogInfoToFile("LogRechargeInfo.txt", TEXT( "普通充值异步回调:::::%s::::::"), UrlStr.c_str());
...
void NormalRechargeManager::OnHandleNormalRechargeInfoResult(DBO_Cmd_QueryRechargeOrderInfo* pMsg)
{
	...
	LogInfoToFile("Chongzhi.txt", TEXT("购买成功********%d**********%d************%d******************"), pMsg->Price, pMsg->dwUserID, pMsg->ShopID);
	DWORD BeginIndex = 0;
	{
