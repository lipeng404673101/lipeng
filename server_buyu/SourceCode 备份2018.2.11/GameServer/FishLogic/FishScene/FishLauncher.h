	...
	UINT LaunchFish(int fishGroup, bool bforceLaunch, UINT pcakgeType = -1)
	{
		...
		for (int n = 0; n < pGroup->GroupDataCount; ++n)
		{
			...
			for (int i = 0; i < gd.FishNum; ++i)
			{
				...
				Fish *pFish = new Fish;
				pFish->InitFishData(ConvertIntToWORD(fishGroup), m_FishID, gd.FishIndex, ConvertIntToWORD(pathIndex), ConvertIntToWORD((n << 8) | i),
										gd.FishScaling, gd.SpeedScaling, gd.pFishPosList[i], time, pPathData);

				if (fishIdx == i)
					fishPackageID = m_FishID;

				if (++m_FishID == MAX_FISH_NUM) m_FishID = START_FISH_ID;
