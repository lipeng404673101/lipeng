...
#include <set>
//一些常量的定义
#define MAX_NICKNAME 8   //玩家昵称的长度    8
...
enum FishDBCmd
{
	...
	//////////////////////////////////////////////////////////////////////////
	DBR_SaveRoleGlobelDown = 180,
	DBR_CheckUserIsDaili = 181,
	DBO_CheckUserIsDailiO = 182,

	DBR_CheckUserIsContrlDaili=183,//检测赠送
	DBR_CheckUserIsContrlDailiBack = 184,//检测赠送


	DBR_CheckUserDailiBond = 185,//检测玩家绑定代理
	DBR_UserDailiBond = 186,//玩家绑定代理	
	DBR_AccountHeadHttp = 187,//存储玩家头像

	DBO_CheckDailiBond=188,//检测代理绑定

	DBO_DailiBondResult = 189,//代理绑定  结果


	DBR_QueryPrivateRoom=190,//请求房间号   房卡模式
	DBO_QueryPrivateRoomResult=191,//请求房间号结果  房卡专用

	DBR_DeletePrivateRoom = 192,//删除房间   房卡模式
...
//添加微信头像的数据库写入
struct DBR_Cmd_AccountHeadHttp : public NetCmd
{
	DWORD	ClientID;
	DWORD	dwUserID;
	TCHAR							szHeadHttp[256];			//头像
...
//添加一个注册返回值  提示绑定代理失败或成功
struct DBO_Cmd_AccountRsg : public NetCmd
{
	DWORD ClientID;
	DWORD	dwUserID;

	DWORD	dwBangDing;//绑定参数
};
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//检查玩家的代理资格
struct DBR_Cmd_CheckUserDaiLi :	public NetCmd
{
	DWORD			dwUserID;//玩家的id

	DWORD			dwGlobel;//代理金币
};
//检查玩家代理资格回复客户端
struct DBO_Cmd_CheckUserDaiLiBake : public NetCmd
{
	DWORD			dwUserID;//玩家的id
	//DWORD			dwGlobel;//代理金币

	BYTE			BackeNum;//数据库返回的资格普通0  1超级  2代理
...
//////////////////////////////////////////////////////////////////////////
//添加赠送验证
struct CL_Cmd_CheckClientInfoContrlDaili : public NetCmd
{

	DWORD			dwUserID;//请求本身的id
	DWORD			dwOtherUserID;//赠送给另外一个玩家的id  必须是gameid

};
//////////////////////////////////////////////////////////////////////////
//玩家代理绑定    检测
struct CL_Cmd_CheckClientDailiBond : public NetCmd
{
...
//////////////////////////////////////////////////////////////////////////
//数据库请求  代理绑定
struct DBR_Cmd_CheckUserDailiBond : public NetCmd
{
	DWORD			dwUserID;//玩家的id

	DWORD			dwOtherUserID;//赠送给另外一个玩家的
};

//玩家代理绑定
struct CL_Cmd_ClientDailiBond : public NetCmd
{
...
//数据库请求  代理绑定
struct DBR_Cmd_UserDailiBond : public NetCmd
{
	DWORD			dwUserID;//玩家的id
...
//请求返回   检测代理绑定  
struct DBO_Cmd_CheckUserDailiBond : public NetCmd
{
	DWORD			dwUserID;//玩家的id
	//DWORD			dwGlobel;//代理金币

	BYTE			BackeNum;//数据库返回的资格普通0   1返回成功  也就是可以

};

//检测代理绑定      fanhui返回结果
struct LC_Cmd_CheckUserDailiBond : public NetCmd
{
	bool			Result;
	DWORD			dwUserID;//玩家的id
...
//请求返回   绑定结果  
struct DBO_Cmd_UserDailiBondResult : public NetCmd
{
	DWORD			dwUserID;//玩家的id
	//DWORD			dwGlobel;//代理金币
...
//代理绑定      结果
struct LC_Cmd_UserDailiBondResult : public NetCmd
{
	bool			Result;
	DWORD			dwUserID;//玩家的id
...
//数据库请求
struct DBR_Cmd_CheckUserContrlDaiLi : public NetCmd
{
	DWORD			dwUserID;//玩家的id
...
//请求返回
struct DBO_Cmd_CheckUserContrlDaiLiBacke : public NetCmd
{
	DWORD			dwUserID;//玩家的id
	//DWORD			dwGlobel;//代理金币
...
struct LC_Cmd_CheckClientInfoContrlDaili : public NetCmd
{
	bool			Result;
	DWORD			dwUserID;//玩家的id
...
struct DBR_Cmd_SaveRoleGlobel : public NetCmd
{
	...
	DWORD			dwGlobel;
	 /////////////////////添加参数
	DWORD			dwDaiLiUserID;//代理玩家

	bool			bIsCanContrl;//是否是控制消息	

	DWORD			dwAddGlobel;//金币的变化
};
//减少金币的
struct DBR_Cmd_SaveRoleGlobelDown : public NetCmd
{
	DWORD			dwUserID;
	DWORD			dwGlobel;
	/////////////////////添加参数
...
//删除房间
struct DBR_GR_DeletePrivateRoom :public NetCmd
{
	DWORD							dwUserID;							//用户 I D
	int							szPrivateTableID;	//私有房间ID
};
//请求房卡  
struct DBR_Cmd_QueryPrivateRoom : public NetCmd
{
	BYTE							cbCreateRoom;							//是否加入房间
	DWORD							dwUserID;							//用户 I D
	WORD							wFeeType;							//货币类型
	DWORD							dwCostFeeCount;							//费用
	int								wDataSize;
	//添加参数
	WORD							wDifen;				//底分
...
//房卡  房间号请求返回   结果   创建房间结果   房卡专用
struct DBO_GR_CreatePrivateRoomResult : public NetCmd
{
	DWORD							result;								//操作结果
	TCHAR							szDescribeString[128];				//描述消息
	//TCHAR							szPrivateRoomID[LEN_PRIVATE_TABLE_ID];	//私有房间号
	DWORD							dwUserID;							//用户 I D

	BYTE							bTableTypeID;						//选择进入桌子的类型
	int								szPrivateRoomID;//房间号   房卡  
	DWORD							dwSize;
...
struct CL_Cmd_QueryLogon : public NetCmd
{
	...
	//添加微信头像
	TCHAR							szHeadHttp[256];			//头像微信
	TCHAR							NickName[MAX_NICKNAME + 1];					//微信昵称
...
enum TableCmd
{
	...
	LC_TableChangeRoleGlobeDown =32,//减少金币
...
struct tagGameConfig
{
	...
	DWORD				dwPlayRule;						//玩法选项(1带幺九、将对,2换三张，3门清中张)
	TCHAR				sPrivateTableID[11];

	WORD				wMaxScore;						 //最大分数
	WORD				wPlayCountRule;					//一次玩的局数（4局？8局？）
	WORD				wHadPlayCount;					//已玩局数

	DWORD				dwReservedRule1;				//预留规则1//dwReservedRule1 
	DWORD				dwReservedRule2;				//预留规则2//dwReservedRule2 
	DWORD				dwReservedRule3;				//预留规则3//dwReservedRule3 
	////////////////////////////////////////////////////////////////////////////
	////1常规  2风耗子   3不带风
	//WORD				wGameCardRule;		//1 2 3 
	//WORD				wPayCosType;		//支付方式 1房主  2AA支付
	//WORD				wDifen;				//底分
	//WORD				wIPfangZuobi;		//防作弊
};
struct CL_JoinTable : public NetCmd
{
	BYTE							bTableTypeID;						//选择进入桌子的类型

	BYTE							bCreateTable;						//1创建  2加入  0正常金币场
	//TCHAR							szPrivateRoomID[11];				//私有房间号
	int								szPrivateRoomID;					//私有房间号
	WORD							wTableID;							//桌子位置
	WORD							wChairID;							//椅子位置
...
//加入桌子
struct CL_JoinTableOne : public NetCmd
{
	BYTE							bTableTypeID;						//选择进入桌子的类型
...
//新添加的减少桌子玩家金币
struct LC_Cmd_TableChangeRoleGlobeDown : public NetCmd
{
	DWORD		dwDestUserID;
	int			dwGlobelSum;
};
...
enum RoleSub
{
	...
	//////////////////////////////////////////////////////////////////////////
	LC_ChangeRoleGlobeDown = 89,//减少金币
	LC_CheckRoleUserDailito=90,// daili代理信息
	LC_ChangeRoleGlobeSuccess=91,//shou手动充值成功回调

	LC_QueryFishPoolResultRol = 92,//查询库存结果

	LC_QueryOnlineUserInfoRol = 93,//返回查询在线玩家

	LC_QueryBlackListResultRol = 94,//查询黑名单结果
	LC_SetBlackListResultRol = 95,//设置黑名单结果

	LC_OxAdminRol = 96,//牛牛控制

	LC_CheckRoleUserContrlDailito = 97,// 检测信息ze赠送

	//////////////////////////////////////////////////////////////////////////

	LC_CheckRoleUserDailiBond = 98,// daili代理绑定  邀请码  检测结果  客户端 
	LC_RoleUserDailiBondResult = 99,// daili代理绑定  邀请码  绑定结果  客户端 
...
// 新添修改金币减少
struct LC_Cmd_ChangeRoleGlobeDown : public NetCmd
{
	//DWORD		dwUserID;
	int		dwGlobeNum;
};
//充值消息通知
struct LC_Cmd_ChangeRoleGlobeSuccess : public NetCmd
{
	DWORD		dwUserID;
	int			dwGlobeNum;//充值数量

	bool		bIsSuccess;//是否成功
	bool		bIsUserOnline;//充值玩家是否在线

	//bool		bIslowuserGold;	//减少代理金币是否成功
};
// 新添修改金币减少
//struct LC_Cmd_ChangeRoleGlobeDown : public NetCmd
//{
...
//添加金币兑换
struct CL_Cmd_ShopGoldItem : public NetCmd
{
	DWORD		dwUserID;//兑换玩家id
	//tagItemOnce pItem;
	//BYTE		ShopItemIndex;
	//DWORD		ItemSum;
	DWORD		ChangeGlobel;//兑换的金币

	TCHAR		UseName[MAX_NAME_LENTH + 1];
...
enum ControlSub
{
	...
	//新添
	CL_ChangeUserGold=55,//修改玩家金币

	//////////////////////////////////////////////////////////////////////////

	CL_CheckClientInfoContrlDaili=56,//客户端发送的验证是否是代理请求  区别于控制请求  赠送请求

	CL_ChangeTablePool=57,//修改桌子的库存  提高或者减少幸运值

	CL_HandleGailv=58,//修改捕鱼概率

	//////////////////////////////////////////////////////////////////////////

	CL_CheckClientDailiBond=59,//检测玩家绑定代理

	CL_ClientDailiBond = 60,//玩家绑定代理
...
//修改为控制概率
struct CL_Cmd_HandleGailv : public NetCmd
{
...
//修改桌子的库存值  库存影响幸运值
struct CL_Cmd_HandleTablePoolGailv : public NetCmd
{
	WORD TableID;//桌子的id 0 1 2 3 
	int64 nValue;//修改的库存值
...
struct CL_Cmd_ChangeUserGold:public NetCmd
{
	DWORD			dwDaiLiUserID;//给别人充值的玩家
	DWORD			dwUserID;//玩家id    要充值的玩家
	DWORD			dwGlobeNum;//增加的金币  zhi只能加不能减
	bool			bIsZongDai;//是否是总代理
};
...
struct CL_CMD_OxAdminReq :public NetCmd
{
	...
	DWORD dwUserID;				  //新添参数  玩家id
	DWORD dwUserIDContrl;//玩家id  xin新添参数  控制玩家的

	tagOxAdminReq cbExtendData;//修改
};
...
struct LC_CMD_OxAdminReq :public NetCmd
{
	...
	DWORD dwUserID;//玩家id  xin新添参数  控制玩家的
	DWORD dwUserIDContrl;//玩家id  xin新添参数  控制玩家的
...
struct CL_CMD_AnimalAdminReq :public NetCmd
{
	...
	tagAnimalAdminReq cbExtendData;//修改
...
enum DialSub
{
	...
	//LC_OxAdminQueryPlayerJettonContrl = 66,//获取房间玩家的下注信息

	//LC_OxAdminQueryAllJettonContrl = 67,//获取所有玩家下注总

	//LC_OxAdminQueryNameContrl = 68,//查询玩家昵称


	LC_AnimalAdminContrlBack = 69,//
	LC_AnimalAdminQueryNameContrl = 70,//查询名字
	LC_AnimalAdminQueryAllJettonContrl = 71,//查询所有
	LC_AnimalAdminQueryPlayerJettonContrl = 72,//
};
struct DialRoleInfo
{
	DWORD	dwUserID;
...
LC_OxAdminContrlBack = 65,//游戏控制消息返回
