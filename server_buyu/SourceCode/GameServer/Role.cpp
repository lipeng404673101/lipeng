...
void CRole::OnCatchFish(CatchType catchType, byte subType,WORD FishType, BYTE byPackageType, int nScore, int nExp)
{
	...
	//{
	//	if (m_pRoleEx->GetRoleInfo().dwGlobeNum < nScore * 2)
	//	{
	//		LogInfoToFile("Control.txt", "机器人没钱22222:::::::::::::::%d:::::%ld::::", nScore, m_pRoleEx->GetRoleInfo().dwGlobeNum);
	//		return;
	//	}
	...
	else
	{
		...
		//if (m_pRoleEx->IsRobot())
		//{
		//	m_pRoleEx->ChangeRoleGlobe(nScore);//金币加减   这里面不是时时更新数据库的  需要在离开桌子那里看
		//}
		//else
		//{
		//	m_pRoleEx->ChangeRoleGlobe(nScore, false);//金币加减   这里面不是时时更新数据库的  需要在离开桌子那里看
		//}
	...
	if (!m_pRoleEx->GetRoleMonth().IsInMonthTable())
	{
		...
		if (!m_pRoleEx->IsRobot())
		{
			BYTE TyID = g_FishServer.GetTableManager()->GetTable(GetTableID())->GetTableTypeID();
		//	LogInfoToFile("Control.txt", "da打鱼:::::::::::::::%d:::::%ld::::", nScore, g_FishServer.GetTableManager()->KucunNum[TyID]);
			g_FishServer.GetTableManager()->KucunNum[TyID] -= nScore;    //库存  库存的积累
...
bool CRole::CheckFire(BYTE byLauncher)
{
	...
	if (launcher.nEnergy >= m_pConfig->LaserThreshold(m_LauncherType)*m_pConfig->BulletMultiple(m_nMultipleIndex))// *rate 
	{
		LogInfoToFile("Control.txt", "CheckFire大炮能量值已满:::::::%d:::::::::::%d::::::::::::::::::::", launcher.nEnergy, m_pConfig->LaserThreshold(m_LauncherType)*m_pConfig->BulletMultiple(m_nMultipleIndex));
	...
	else
	{
		...
		//if (m_pRoleEx->IsRobot())
		//{
		//	if (m_pRoleEx->GetRoleInfo().dwGlobeNum < nConsume * 2)
		//	{
		//		LogInfoToFile("Control.txt", "机器人没钱:::::::::::::::%d:::::%ld::::", nConsume, m_pRoleEx->GetRoleInfo().dwGlobeNum);
		//		return false;
		//	}
		...
		//if (m_pRoleEx->IsRobot())
		//{
		//	if (!m_pRoleEx->ChangeRoleGlobe(nConsume*-1))
		//	{
		...
		{
			BYTE TyID = g_FishServer.GetTableManager()->GetTable(GetTableID())->GetTableTypeID();
			//LogInfoToFile("Control.txt", "没打到鱼:::::::::::::::%d:::::%ld::::", nConsume, g_FishServer.GetTableManager()->KucunNum[TyID]);
			//g_FishServer.GetTableManager()->KucunNum[TyID] += nConsume*0.98;
			//if (g_FishServer.GetTableManager()->KucunNum[TyID] > 0)
			//{
			//	g_FishServer.GetTableManager()->KucunNum[TyID] += nConsume*0.98;
			//}
			//else{
			//	g_FishServer.GetTableManager()->KucunNum[TyID] += nConsume;
			//}

			g_FishServer.GetTableManager()->KucunNum[TyID] += nConsume;//库存   库存衰减  
			//g_FishServer.GetTableManager()->KucunNum[TyID] *=  0.95;
