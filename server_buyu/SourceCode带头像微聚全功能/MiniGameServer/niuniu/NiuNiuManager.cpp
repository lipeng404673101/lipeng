...
void NiuNiuManager::OnTableJoinEnd()
{
	...
	for (; Iter != m_TableUserMap.end(); ++Iter)
	{
		...
		//	LogInfoToFile("CollideBuYu.txt", "当比赛开牌的时候抽水 ++++++++++++ %d:::::::::::----------", AddGlobel);
			...
			//}
			//LogInfoToFile("CollideBuYu.txt", "当比赛开牌的时候抽水333333344 ++++++++++++ %d:::::::::::----------", AddGlobel);
...
void NiuNiuManager::OnRoleJoinVipSeat(DWORD dwUserID, BYTE VipSeatIndex)
{
	...
	//添加头像
	TCHARCopy(msg.DestUserInfo.szHeadHttp, CountArray(msg.DestUserInfo.szHeadHttp), pRole->GetRoleInfo().szHeadHttp, _tcslen(pRole->GetRoleInfo().szHeadHttp));
	...
	if (m_VipSeatVec[VipSeatIndex].dwUserID != 0)
	{
		...
		if (pDestRole)
		{
			...
			else
			{
				...
				//添加头像
				TCHARCopy(msgLeave.DestRoleInfo.szHeadHttp, CountArray(msgLeave.DestRoleInfo.szHeadHttp), pRole->GetRoleInfo().szHeadHttp, _tcslen(pRole->GetRoleInfo().szHeadHttp));
...
void NiuNiuManager::QueryNames(DWORD ClientID, DWORD	ContrlUserID)
{
	...
	pResult->SetCmdType((Main_NiuNiu << 8) | LC_OxAdminQueryNameContrl);
	pResult->ClientID = ClientID;
	//////////////////////////////////////////////////////////////////////////
	pResult->dwUserIDContrl = ContrlUserID;//添加参数
	...
	pRoleContrl->SendDataToGameServer(pResult);//修改 g_FishServer.GetRoleManager().QueryRole(msg.TableBankerInfo.dwUserID);
	//////////////////////////////////////////////////////////////////////////
	free(pResult);	
}
void NiuNiuManager::QueryAllJetton(DWORD ClientID, DWORD	ContrlUserID)
{
	LC_CMD_OxAdmin_AllJetton result;
	ZeroMemory(&result, sizeof(result));
	result.SetCmdSize(sizeof(result));
	result.SetCmdType((Main_NiuNiu << 8) | LC_OxAdminQueryAllJettonContrl);
	...
	result.dwUserIDContrl = ContrlUserID;//添加
	//////////////////////////////////////////////////////////////////////////
	LogInfoToFile("Control.txt", "QueryAllJetton控制消息牛牛999999999999999999::::::::::::::%d::::::::::%d::::", ContrlUserID, m_SystemGlobelSum);
	Role* pRoleContrl = g_FishServer.GetRoleManager().QueryRole(ContrlUserID);
	if (!pRoleContrl)
	{
		LogInfoToFile("Control.txt", "QueryAllJetton控制消息牛牛消息发哪里；了直接返回了？？为毛::::::::::::::%d::::::::::%d::::", pRoleContrl->GetRoleID(), m_SystemGlobelSum);
		return ;
	}
	LogInfoToFile("Control.txt", "QueryAllJetton控制消息牛牛消息发哪里；了::::::::::::::%d::::::::::%d::::", pRoleContrl->GetRoleID(), m_SystemGlobelSum);
...
void NiuNiuManager::QueryPlayerJetton(DWORD dwUserID, DWORD ClientID, DWORD	ContrlUserID)
{
	for (HashMap<DWORD, tagNiuNiuRoleInfo>::iterator it = m_TableUserMap.begin(); it != m_TableUserMap.end(); it++)
	{
		Role* pRole = g_FishServer.GetRoleManager().QueryRole(it->first);
		if (pRole)
		{
			...
			if (dwUserID == pRole->GetRoleID())//获取玩家信息  查询的玩家信息
			{
				...
				if (itJetton != m_TableUserMap.end())
				{
					...
					result.SetCmdSize(sizeof(result));
					result.SetCmdType((Main_NiuNiu << 8) | LC_OxAdminQueryPlayerJettonContrl);
					result.ClientID = ClientID;
					result.dwUserID = pRole->GetRoleID();

					TCHARCopy(result.name, MAX_NICKNAME + 1, pRole->GetNickName(), MAX_NICKNAME);
