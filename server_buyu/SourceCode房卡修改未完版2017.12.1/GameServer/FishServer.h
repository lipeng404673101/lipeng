...
private:
	...
	//////////////////////////////////////////////////////////////////////////
	//玩家绑定代理  检测结果  邀请码
	bool OnHandleCheckUserDailiBond(NetCmd* pCmd);

	//玩家绑定代理  绑定结果  邀请码   返回客户端消息
	bool OnHandleUserDailiBondResult(NetCmd* pCmd);

	//玩家申请房间号  创建房间  房卡   房卡模式
	bool OnHandleCreatePrivateRoomResult(NetCmd* pCmd);

	//////////////////////////////////////////////////////////////////////////
	//代理信息
	bool OnHandleCheckUserDailiTo(NetCmd* pCmd);

	//赠送检测
	bool OnHandleCheckUserContrlDailiTo(NetCmd* pCmd);

	//减少金币
	bool	ChangeGlobeContrlDown(int ChangeGlobe, bool IsSendToClient = true, bool IsSaveToDB = false, bool IsSendToMiniGame = false, DWORD dwDaiLiUserID = 0, bool bIsCanContrl = false, bool bIsZongDai = true);
...
private:
	...
	//////////////////////////////////////////////////////////////////////////
	//std::unordered_map<DWORD, std::wstring> m_UserID_2_PriTableID;	//创建者ID 转 私有桌子ID
