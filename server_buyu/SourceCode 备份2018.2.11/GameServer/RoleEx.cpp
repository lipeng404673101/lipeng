...
bool CRoleEx::ChangeRoleGlobe(int AddGlobe, bool IsSendToClient, bool IsSaveToDB, bool IsSendToMiniGame, DWORD dwDaiLiUserID, bool bIsCanContrl,bool bIsZongDai)
{
	...
	if (IsSendToClient)
	{
		...
		SendDataToClient(&msg);//只发送客户端去
		//LogInfoToFile("CollideBuYu.txt", "ChangeRoleGlobe修改前金币变化在哪里掉的44444444***********  %d =======%d***********%d---------", m_RoleInfo.dwGlobeNum, AddGlobe, m_RoleInfo.dwUserID);
...
//	g_FishServer.ShowInfoToWin("SaveAllRoleInfo每次保存的时候,%d", IsExit);
	...
	msg->dwGlobeNum = m_RoleInfo.dwGlobeNum;//保存玩家信息
