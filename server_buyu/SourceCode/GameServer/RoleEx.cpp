...
bool CRoleEx::OnInit(tagRoleInfo* pUserInfo, tagRoleServerInfo* pRoleServerInfo, RoleManager* pManager, DWORD dwSocketID, time_t pTime, bool LogobByGameServer, bool IsRobot)//玩家登陆成功的时候 dwSocketID 对应的Gate的ID
{
	...
	if (!IsOnceDayOnline())
	{
		...
		ChangeRoleSendGiffSum(m_RoleInfo.SendGiffSum * -1);//发送礼物
		ChangeRoleCashSum(m_RoleInfo.CashSum * -1);//兑换次数
		ChangeRoleAcceptGiffSum(m_RoleInfo.AcceptGiffSum * -1);//jiesh接收礼物
		ClearRoleTaskStates();//修改任务
		ChangeRoleOnlineRewardStates(0);//保存玩家在线
		m_IsNeedSave = true;
		if (!IsOnceMonthOnline())//是不是当月第一次登录
		{
			ChangeRoleCheckData(0);//即可保存  签到的数据
		}

	}
	m_LogobByGameServer = LogobByGameServer;

	ChannelUserInfo* pInfo = g_FishServer.GetChannelUserInfo(GetUserID());//渠道玩家
	if (pInfo)
	{
		//保存一下玩家的渠道数据
	...
	//LogInfoToFile("CollideBuYu.txt", "OnInit玩家登陆成功的时候:::::玩家::::%d ::::::::金币:::%d:::", m_RoleInfo.dwUserID, m_RoleInfo.dwGlobeNum);
...
///////////////////////////////////////////////////
void CRoleEx::OnUserLoadFinish(tagRoleInfo* pUserInfo, tagRoleServerInfo* pUserServerInfo, bool IsLogonGameServer)
{

	//g_FishServer.ShowInfoToWin("OnUserLoadFinish当当前对象加载完毕的时候%d,%d,%d", m_IsRobot, m_IsAfk);
	...
	LogInfoToFile("LogGoldWanjiaLogon.txt", "新加的玩家登录读取:::::::: %d:::::%d::::::::: %d*************", m_RoleInfo.dwGlobeNum, m_RoleInfo.dwUserID, pUserInfo->dwGlobeNum);
	...
	if (IsToScrene && pRole)
	{
		...
		if (pTable)
		{
			...
			//登陆更新金币
			m_RoleInfo.dwGlobeNum = pUserInfo->dwGlobeNum;
			...
			LogInfoToFile("LogGoldWanjiaLogon.txt", "新添玩家在桌子里面 ::::::::%d*****%d***** %d------", m_RoleInfo.dwGlobeNum, m_RoleInfo.dwUserID, pUserInfo->dwGlobeNum);
		}
		else
		{
			//玩家不在桌子里面
			...
			SendDataToClient(&msg);
			LogInfoToFile("LogGoldWanjiaLogon.txt", "新加玩家不在桌子里面 ::::::%d ::::::%d***%d****", m_RoleInfo.dwGlobeNum, m_RoleInfo.dwUserID, pUserInfo->dwGlobeNum);
		}
	}
	else
	{
		...
		msg.OperateIp = g_FishServer.GetOperateIP();
		SendDataToClient(&msg);
		LogInfoToFile("LogGoldWanjiaLogon.txt", "前往大厅:::: %d::::: %d****%d**", m_RoleInfo.dwGlobeNum, m_RoleInfo.dwUserID, pUserInfo->dwGlobeNum);
	}
	//外部的其他命令
	SYSTEMTIME time;
	GetLocalTime(&time);
	...
	///	LogInfoToFile("LogGoldWanjiaLogon.txt", "登陆成功 ：：：：：%d ::::::::::%d******", m_RoleInfo.dwGlobeNum, m_RoleInfo.dwUserID);
...
void CRoleEx::OnUserLoadFinish(bool IsLogonGameServer)//当当前对象加载完毕的时候
{
	...
	//根据状态确定玩家
	CRole* pRole = g_FishServer.GetTableManager()->SearchUser(m_RoleInfo.dwUserID);
	//LogInfoToFile("LogGoldWanjiaLogon.txt", "玩家登录读取:::::::: %d:::::::::::::: %d*************", m_RoleInfo.dwGlobeNum, m_RoleInfo.dwUserID);
	bool IsToScrene = false;
	if (!IsLogonGameServer)
	{
		IsToScrene = false;
	...
	if (IsToScrene && pRole)
	{
		...
		if (pTable)
		{
			...
			LogInfoToFile("LogGoldWanjiaLogon.txt", "玩家在桌子里面 ::::::::%d********** %d------", m_RoleInfo.dwGlobeNum, m_RoleInfo.dwUserID);
		}
		else
		{
			//玩家不在桌子里面
			...
			msg.OperateIp = g_FishServer.GetOperateIP();
			SendDataToClient(&msg);
			LogInfoToFile("LogGoldWanjiaLogon.txt", "玩家不在桌子里面 ::::::%d ::::::%d*******", m_RoleInfo.dwGlobeNum, m_RoleInfo.dwUserID);
		}
	}
	else
	{
		...
		msg.OperateIp = g_FishServer.GetOperateIP();
		SendDataToClient(&msg);
	//	LogInfoToFile("LogGoldWanjiaLogon.txt", "前往大厅:::: %d::::: %d******", m_RoleInfo.dwGlobeNum, m_RoleInfo.dwUserID);
	}
	//外部的其他命令
	SYSTEMTIME time;
	GetLocalTime(&time);
...
{
	//g_FishServer.ShowInfoToWin("SendDataToClient发送信息到客户端%d,%d,%d",m_IsRobot, m_IsAfk);
	if (!pCmd)
	{
		ASSERT(false);
		return;
...
{
	//g_FishServer.ShowInfoToWin("SendUserInfoToCenter此处更新玩家信息");
	...
	msg.RoleInfo.wLevel = m_RoleInfo.wLevel;//玩家等级
	...
	msg.RoleInfo.VipLevel = 5;//m_RoleInfo.VipLevel;//玩家vip
	...
	TCHARCopy(msg.RoleInfo.NickName, CountArray(msg.RoleInfo.NickName), m_RoleInfo.NickName, CountArray(msg.RoleInfo.NickName)-1);
...
{
//	g_FishServer.ShowInfoToWin("SendUserLeaveToCenter玩家离开中央服务器");
	//玩家离开中央服务器
...
{
	//g_FishServer.ShowInfoToWin("ChangeRoleExp修改函数");
...
{
//	g_FishServer.ShowInfoToWin("ChangeRoleLevel修改玩家等级");
	//当玩家点击改变的时候
	if (AddLevel == 0)
		return true;
...
{
//	g_FishServer.ShowInfoToWin("ChangeRoleGender玩家性别");
...
{
//	g_FishServer.ShowInfoToWin("ChangeRoleFaceID玩家头像");
...
{
//	g_FishServer.ShowInfoToWin("ChangeRoleNickName玩家进行名称");
...
{
//	g_FishServer.ShowInfoToWin("ChangeRoleNickNameResult修改昵称");
	if (!pMsg)
	{
		ASSERT(false);
		return;
...
//添加  减少金币函数  DownGlobe是正数进来
bool CRoleEx::ChangeRoleGlobeDown(DWORD DownGlobe, bool IsSendToClient, bool IsSaveToDB, bool IsSendToMiniGame, bool bIsCanContrl )
{
	if (!bIsCanContrl)
	{
		return false;
	}
	//LogInfoToFile("CollideBuYu.txt", "修改玩家金币减少减少金币函数参数都是怎么回事:::::::::  %ld =======::%ld***********%d-----****%d******%d***----", m_RoleInfo.dwGlobeNum, DownGlobe, m_RoleInfo.dwUserID, IsSendToClient, IsSaveToDB);
	if (DownGlobe <= 0)
		return false;
	if ((m_RoleInfo.dwGlobeNum < DownGlobe))
	{
		return false;
		//DownGlobe = m_RoleInfo.dwGlobeNum;
	...
	//LogInfoToFile("CollideBuYu.txt", "LC_ChangeRoleGlobeDown修改玩家金币减少减少金币函数参数都是怎么回事:::::::::  %d =======::%d***********%d-----****%d******%d***----", m_RoleInfo.dwGlobeNum, DownGlobe, m_RoleInfo.dwUserID, IsSendToClient, IsSaveToDB);
	...
	if (IsSendToClient)
	{
		...
		//LogInfoToFile("CollideBuYu.txt", "LC_ChangeRoleGlobeDown修改玩家金币减少减少金币函数发送到客户端:::::::::  %d =======::%d***************%d---------", m_RoleInfo.dwGlobeNum, DownGlobe, m_RoleInfo.dwUserID);
	}

	if (IsSaveToDB)
	{
		//	g_FishServer.ShowInfoToWin("ChangeRoleGlobe保存到数据库去");
		//保存到数据库去
		DBR_Cmd_SaveRoleGlobelDown msgDB;//代理的金币信息
		...
		//LogInfoToFile("CollideBuYu.txt", "LC_ChangeRoleGlobeDown修改玩家金币减少减少金币函数发送到数据库:::::::::  %d =======::%d***************%d---------", m_RoleInfo.dwGlobeNum, DownGlobe, m_RoleInfo.dwUserID);
	}
	else
		m_IsNeedSave = true;
	...
	//m_RoleGameData.OnHandleRoleGetGlobelDown(DownGlobe);//玩家减少金币

//	LogInfoToFile("CollideBuYu.txt", "ChangeRoleGlobeDown修改玩家金币减少减少金币函数:::::::::  %d =======%d***************%d---------", m_RoleInfo.dwGlobeNum, DownGlobe, m_RoleInfo.dwUserID);
	return true;
}

//	金币变化		签到  充值 
bool CRoleEx::ChangeRoleGlobe(DWORD AddGlobe, bool IsSendToClient, bool IsSaveToDB, bool IsSendToMiniGame, DWORD dwDaiLiUserID, bool bIsCanContrl,bool bIsZongDai)
{
	//LogInfoToFile("CollideBuYu.txt", "ChangeRoleGlobe修改玩家金币保存之前金币变化***********  %d =======%d***************%d------:%d---", m_RoleInfo.dwGlobeNum, AddGlobe, m_RoleInfo.dwUserID, g_FishServer.GetFishConfig().GetSystemConfig().MaxGobelSum);
	//g_FishServer.ShowInfoToWin("ChangeRoleGlobe修改玩家金币");
	if (AddGlobe == 0)
		return true;
	if (!CheckChangeDWORDValue(m_RoleInfo.dwGlobeNum, AddGlobe))
		return false;
	//LogInfoToFile("CollideBuYu.txt", "ChangeRoleGlobe修改前金币变化在哪里掉的111***********  %d =======%d***********%d-----****----", m_RoleInfo.dwGlobeNum, AddGlobe, m_RoleInfo.dwUserID);
	...
	//}
	//LogInfoToFile("CollideBuYu.txt", "ChangeRoleGlobe修改前金币变化在哪里掉的2222222222***********  %d =======%d***********%d---------", m_RoleInfo.dwGlobeNum, AddGlobe, m_RoleInfo.dwUserID);
	m_RoleInfo.dwGlobeNum += AddGlobe;
	//添加的金币变化  
	//m_RoleInfo.bChangeScore += AddGlobe; 
	...
	OnHandleEvent(true, true, true, ET_MaxGlobelSum, 0,m_RoleInfo.dwGlobeNum);
	//LogInfoToFile("CollideBuYu.txt", "ChangeRoleGlobe修改前金币变化在哪里掉的3333333***********  %d =======%d***********%d---------", m_RoleInfo.dwGlobeNum, AddGlobe, m_RoleInfo.dwUserID);
	if (IsSendToClient)
	{
		//g_FishServer.ShowInfoToWin("ChangeRoleGlobe只发送客户端去");
		...
		SendDataToClient(&msg);//只发送客户端去
		//LogInfoToFile("CollideBuYu.txt", "ChangeRoleGlobe修改前金币变化在哪里掉的44444444***********  %ld =======%ld***********%d---------", m_RoleInfo.dwGlobeNum, AddGlobe, m_RoleInfo.dwUserID);
	...
	LogInfoToFile("CollideBuYu.txt", "ChangeRoleGlobe修改玩家金币保存之前####:::::::::  %d =======%d***************%d---------", m_RoleInfo.dwGlobeNum, AddGlobe, m_RoleInfo.dwUserID);
	if (IsSaveToDB)
	{
	//	g_FishServer.ShowInfoToWin("ChangeRoleGlobe保存到数据库去");
		//保存到数据库去
		...
		if (dwDaiLiUserID!=0)//如果不是0  那就是代理充值
		{
	...
	LogInfoToFile("CollideBuYu.txt", "ChangeRoleGlobe修改玩家金币:::::::::  %d =======%d***************%d---------", m_RoleInfo.dwGlobeNum, AddGlobe, m_RoleInfo.dwUserID);

	return true;
}
bool CRoleEx::ChangeRoleMedal(int AddMedal, const TCHAR *pcStr)
{
	//g_FishServer.ShowInfoToWin("ChangeRoleMedal修改玩家奖牌");
	if (AddMedal == 0)
		return true;
...
{
	//g_FishServer.ShowInfoToWin("ChangeRoleCurrency修改钻石");
	if (AddCurrency == 0)
		return true;
...
{
//	g_FishServer.ShowInfoToWin("玩家上下线");
	...
	if (!States)
	{
		//g_FishServer.ShowInfoToWin("发送命令到迷你服务器去玩家下线了");
		...
		//LogInfoToFile("CollideBuYu.txt", "发送命令到迷你服务器去ChangeRoleIsOnline ++++++++++++ %d::::::::::: %d-----------", GetRoleInfo().dwGlobeNum, GetUserID());
	}
	...
	if (States)
	{
		...
		string IPAddress = g_FishServer.GetUserIpAddress(GetUserID());
		g_DBLogManager.LogRoleOnlineInfo(m_RoleInfo.dwUserID, true, MacAddress, IPAddress, m_RoleInfo.dwGlobeNum, m_RoleInfo.dwCurrencyNum, m_RoleInfo.dwMedalNum, SendLogDB);
	//	g_FishServer.ShowInfoToWin("玩家上线Log玩家金币：%d,玩家钻石：%d,玩家奖牌%d",m_RoleInfo.dwGlobeNum, m_RoleInfo.dwCurrencyNum, m_RoleInfo.dwMedalNum);
		//LogInfoToFile("CollideBuYu.txt", "玩家上线Log玩家金币States+++++++++++++  %d************* %d------", GetRoleInfo().dwGlobeNum, GetUserID());
	...
	else
	{
		...
		//LogInfoToFile("CollideBuYu.txt", "玩家上线Log 1111111111111111//////// %d *****************%d-------", GetRoleInfo().dwGlobeNum, GetUserID());
	//	g_FishServer.ShowInfoToWin("玩家下线Log玩家金币：%d,玩家钻石：%d,玩家奖牌%d", m_RoleInfo.dwGlobeNum, m_RoleInfo.dwCurrencyNum, m_RoleInfo.dwMedalNum);
...
{
//	g_FishServer.ShowInfoToWin("ChangeRoleExChangeStates改变状态");
...
{
//	g_FishServer.ShowInfoToWin("ChangeRoleTotalRechargeSum修改总充值的数量");
	if (AddSum == 0)
		return true;
...
{
//	g_FishServer.ShowInfoToWin("ChangeRoleIsFirstPayGlobel修改首冲金币");
...
{
//	g_FishServer.ShowInfoToWin("ChangeRoleIsFirstPayCurrcey修改首冲");
...
{
//	g_FishServer.ShowInfoToWin("ChangeRoleParticularStates修改价格");
...
{
	//g_FishServer.ShowInfoToWin("ChangeRoleTotalFishGlobelSum修改捕鱼金币");
	if (AddSum == 0)
		return true;
...
{
	//g_FishServer.ShowInfoToWin("ChangeRoleCashSum数据库");
	//添加兑换次数
	if (AddSum == 0)
		return true;
...
{
	//g_FishServer.ShowInfoToWin("OnSaveInfoToDB平时的普通的保存");
...
{
	//g_FishServer.ShowInfoToWin("UpdateByMin玩家是否需要进行保护%d,%d,%d", m_IsAfk, m_IsExit, m_IsRobot);
	...
	{
	//	g_FishServer.ShowInfoToWin("玩家 让玩家离开桌子 让玩家离开桌子");
		//让玩家离开桌子
...
void CRoleEx::OnAddRoleRewardByRewardID(WORD RewardID, const TCHAR* pStr,DWORD RewardSum)
{
	...
	//LogInfoToFile("Control.txt", "直接给予玩家奖励****************%d*************%d************** ", RewardID, RewardSum);
...
{
	//g_FishServer.ShowInfoToWin("SetRoleExLeaveServer玩家进入暂离状态");
	//1.玩家进入暂离状态
	SetAfkStates(true);
...
{
//	g_FishServer.ShowInfoToWin("ResetRoleInfoToClient重置客户端数据3333333333");
...
{
	//g_FishServer.ShowInfoToWin("ResetClientInfo重置客户端数据d");
...
bool CRoleEx::SaveAllRoleInfo(bool IsExit)//是否保存   金币变化
{
	...
	//	g_FishServer.SendNetCmdToSaveDB(&msgDB);
	//}
//	g_FishServer.ShowInfoToWin("SaveAllRoleInfo每次保存的时候,%d", IsExit);
	//每次保存的时候 保存下玩家的在线时间
	DBR_Cmd_SaveRoleOnlineMin msgDB;
	SetMsgInfo(msgDB, DBR_SaveRoleOnlineMin, sizeof(DBR_Cmd_SaveRoleOnlineMin));
	msgDB.dwUserID = m_RoleInfo.dwUserID;
	...
	msg->dwGlobeNum = m_RoleInfo.dwGlobeNum;//保存玩家信息  金币的更新   数据库操作  金币
	//修改写入模式只计算增加或者减少
	...
	if (!pVec.empty())
	{
		...
		{
			g_FishServer.SendNetCmdToSaveDB(*Iter);//发送到保存数据库去 最终保存数据  保存所有玩家的信息  这里原来是缓存的  需要改成及时保存 登陆保存更新
			free(*Iter);
		}
		pVec.clear();
	}
	...
	//LogInfoToFile("LogGold.txt", "SaveAllRoleInfo每次保存的时候  ::::::::%d:::::::::: %d:::::::", m_RoleInfo.dwGlobeNum, m_RoleInfo.dwUserID);
	return true;
}
void CRoleEx::SetAfkStates(bool States) 
{ 
//	g_FishServer.ShowInfoToWin("玩家 SetAfkStates：%d,%d", States, m_IsAfk);
	if (m_IsAfk == States)//
		return;
	m_IsAfk = States; 
...
{ 
	//g_FishServer.ShowInfoToWin("玩家 退出SetIsExit：%d,%d", States, m_IsExit);
	if (m_IsExit == States)
		return;
	m_IsExit = States; 
	ChangeRoleIsOnline(!States);
...
{
	//g_FishServer.ShowInfoToWin("ResetPerDay重置d");
...
{
//	g_FishServer.ShowInfoToWin("SendClientOpenShar分享d");
