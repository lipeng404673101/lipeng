	...
	case CL_WeiXinLogon:
		{
			...
			LogInfoToFile("LogonError.txt", TEXT("接收微信登陆 接收微信登陆接收微信登陆"));
	...
	case CL_QueryLogon:
		{
			...
			else
			{
				...
				else
				{
					...
					////添加头像
					//TCHARCopy(msgGameServer.szHeadHttp, CountArray(msgGameServer.szHeadHttp), pMsg->szHeadHttp, _tcslen(pMsg->szHeadHttp));//修改头像  xuyao 需要客户端传入头像
					//TCHARCopy(msgGameServer.NickName, CountArray(msgGameServer.NickName), pMsg->NickName, _tcslen(pMsg->NickName));//修改头像  xuyao 需要客户端传入头像
