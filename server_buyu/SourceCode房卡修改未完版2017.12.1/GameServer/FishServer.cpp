...
//房间创建结果   房间号  房卡模式   房间申请结果
bool FishServer::OnHandleCreatePrivateRoomResult(NetCmd* pCmd)
{
	
	DBO_GR_CreatePrivateRoomResult * pMsg = (DBO_GR_CreatePrivateRoomResult*)pCmd;

	LogInfoToFile("DBShujuKu.txt", "房间创建结果2222:::::%d:::::::%d****%d*****%d********", pMsg->bTableTypeID, pMsg->dwUserID, pMsg->cbCreateRoom, pMsg->szPrivateRoomID);
	if (!pMsg)
	{
		ASSERT(false);
		return true;
	...
	LogInfoToFile("DBShujuKu.txt", "房间创建结果:::::%d:::::::%d****%d*****%d********", pMsg->bTableTypeID, pMsg->dwUserID, pMsg->cbCreateRoom, pMsg->szPrivateRoomID);
	CL_JoinTableOne  msg;
	msg.bCreateTable = pMsg->cbCreateRoom;
	msg.bTableTypeID = pMsg->bTableTypeID;
	msg.szPrivateRoomID = pMsg->szPrivateRoomID;
	msg.wChairID = 65535;
	msg.wTableID = 65535;
	...
	//修改或者添加新的加入桌子函数  房卡专用
	m_TableManager.OnPlayerCreateOrJoinTable(pMsg->bTableTypeID, pRole, &msg);
...
bool FishServer::HandleClientMsg(ServerClientData* pClient, NetCmd* pCmd)
{
	...
	{
		//LogInfoToFile("Control.txt", "AAAAAAAA开头开始命令处理类了有没有::::::::::: %d :::::::::::::::%d:::::::%d::::::", pCmd->CmdType, pCmd->SubCmdType, sizeof(pCmd));
	}
...
//////////////////////////////////////////////////////////////////////////
					...
					case CL_QueryFishPool://pClient->OutsideExtraData;
					{
											...
											// pResult->ClientID = ((CL_CMD_QueryFishPool*)pCmd)->ClientID;
											 ...
											 for (HashMap<BYTE, TableInfo>::iterator it = tableinfo.begin(); it != tableinfo.end(); it++)
											 {
												 ...
												 nIndex++;
												 LogInfoToFile("Control.txt", "CL_QueryFishPool开始命令处理类了有没有::::: %d::::||||%d::::::%d::::::", pResult->table[nIndex].npool, pResult->table[nIndex].bopen, it->second.TableTypeID);
											 }
										 ...
										 // ChangeGlobeContrlDown(pMsg->dwGlobeNum, true, true, true, pMsg->dwDaiLiUserID, true, true);
										  ...
										  else
										  {
											  //减少代理金币失败  直接通知充值失败
											   ...
											   //充值失败  
											   LC_Cmd_ChangeRoleGlobeSuccess msg;
											   SetMsgInfo(msg, GetMsgType(Main_Role, LC_ChangeRoleGlobeSuccess), sizeof(LC_Cmd_ChangeRoleGlobe));
											   msg.dwGlobeNum = pMsg->dwGlobeNum;//m_RoleInfo.dwGlobeNum;
											   msg.dwUserID = pMsg->dwUserID;
											   msg.bIsSuccess = false;//
											   msg.bIsUserOnline = true;//充值玩家z不在线
					...
					case CL_SendMsgToAllGame:
					{
								...
								//SendMessageByType(pMsg->Message, pMsg->MessageSize, pMsg->MessageType, /*MessageType::MT_Game,*/ pMsg->MessageColor, pMsg->StepNum, pMsg->StepSec, pMsg->Param, true);
									...
									LogInfoToFile("Control.txt", "CL_SendMsgToAllGame控制消息发送全服公告::::: %d::::||||%d::::::%s:::::::::::", pMsg->MessageColor, pMsg->StepNum, pMsg->MessageInfo);
									//SendNetCmdToCenter(pMsg);
									return  true;
					}
						break;	
					...
					case CL_ChangeTablePool://修改桌子的库存值
					{
				               ...
				               CL_Cmd_HandleTablePoolGailv* pMsg = (CL_Cmd_HandleTablePoolGailv*)pCmd;
								...
								LogInfoToFile("Control.txt", "CL_ChangeTablePool修改桌子的库存值:::::::%d::::::::::%d:::::", pMsg->TableID, pMsg->nValue);
								return true;
					}
						break;
...
//添加减少金币的函数
bool FishServer::ChangeGlobeContrlDown(int ChangeGlobe, bool IsSendToClient, bool IsSaveToDB , bool IsSendToMiniGame , DWORD dwDaiLiUserID , bool bIsCanContrl , bool bIsZongDai )
{
...
bool FishServer::HandleDataBaseMsg(NetCmd* pCmd)
{
	...
	case DBO_QueryPrivateRoomResult:
		return OnHandleCreatePrivateRoomResult(pCmd);//创建房间 返回结果  房间号   房卡模式
...
//加入桌子  客户端消息  点击房间发送索引  在这里添加创建房间消息
bool FishServer::OnHandleRoleJoinTable(ServerClientData* pClient, NetCmd* pCmd)
{
	...
	LogInfoToFile("Control.txt", "OnHandleRoleJoinTable桌子消息22222::::::%d::::::: %d::::::", pCmd->CmdType, pCmd->SubCmdType);
	if (!pMsg)
	{
		ASSERT(false);
		return false;
	}
	LogInfoToFile("Control.txt", "OnHandleRoleJoinTable桌子消息333333::%d::::%d::::::: %d::::::", pCmd->CmdType, pCmd->SubCmdType, pMsg->bCreateTable, pMsg->bTableTypeID);
	...
	LogInfoToFile("Control.txt", "OnHandleRoleJoinTable桌子消息:444444444:%d::::%d::::::: %d::::::", pCmd->CmdType, pCmd->SubCmdType, pMsg->bCreateTable, pMsg->bTableTypeID);

	//////////////////////////////////////////////////////////////////////////
	if (pMsg->bCreateTable != 0 && pMsg->bTableTypeID==0)
	{
		LogInfoToFile("CollideBuYu.txt", "加入桌子  客户端消息  点击房间发送索引  在这里添加创建房间消息桌子插入后 33322 ******%d********** %d-----", pRole->GetRoleInfo().dwGlobeNum, pRole->GetUserID());
		m_TableManager.OnPlayerSitDown(pMsg->bTableTypeID, pRole, pCmd);	
		...
		////修改或者添加新的加入桌子函数  房卡专用
		//m_TableManager.OnPlayerCreateOrJoinTable(pMsg->bTableTypeID, pRole, pCmd);
...
void FishServer::SendMessageByType(TCHAR* pMessage, WORD MessageSize, BYTE MessageType, DWORD MessageColor, BYTE StepNum, BYTE StepSec, DWORD Param, bool IsCenterMessage)
{
	...
	else if (MessageType == MessageType::MT_Center || MessageType == MessageType::MT_CenterMessage)
	{
		...
		LogInfoToFile("ControlSendMessageByType.txt", "SendMessageByType发送到当前去::::::: %d::::::::: %s::::::::::::", MessageType, msg->Message);
	}
