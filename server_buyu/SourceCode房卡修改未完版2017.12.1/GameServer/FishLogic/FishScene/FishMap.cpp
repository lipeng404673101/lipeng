...
USHORT FishMap::GetFlowIndex(bool &bFixedGroup)
{
	...
	for (UINT i = 0; i < m_pMap->FlowOrderData.OrderIndex.size(); ++i)
	{
		...
		if (orderidx == m_nIndex)
		{
			...
			//LogInfoToFile("LogPhonePay.txt", "GetFlowIndex发射固定的流程赋值之前::::::%d ====::%d*******%d----%d--------", m_pMap->FlowOrderData.FlowIndex.size(), orderidx, idx, m_nIndex);
			idx = m_pMap->FlowOrderData.FlowIndex[idx];
			bFixedGroup = true;

			//LogInfoToFile("LogPhonePay.txt", "GetFlowIndex发射固定的流程::::::%d ====::%d*******%d----%d--------", bFixedGroup, orderidx, idx, m_nIndex);
		}
	...
	++m_nIndex;
	///LogInfoToFile("LogPhonePay.txt", "GetFlowIndex发射固定的流程结束返回::::::%d ====::%d*******%d----%d--------", m_pMap->FlowOrderData.FlowIndex.size(), bFixedGroup, idx, m_nIndex);
	return (USHORT)idx;
}
