...
bool FishServer::MainUpdate()
{
	while (!m_IsClose)
	{
		...
		UpdateByMin(dwTimer);	// 时间控制的事件更新
		m_ShopManager.UpdateByMin();//更新函商品信息
		OnSaveInfoToDB(dwTimer);//保存玩家信息心跳设置  倒计时
...
//////////////////////////////////////////////////////////////////////////
//赠送
bool FishServer::OnHandleCheckUserContrlDailiTo(NetCmd* pCmd)
{
	//	DBO_Cmd_CheckUserDaiLiBake

	DBO_Cmd_CheckUserContrlDaiLiBacke * pMsg = (DBO_Cmd_CheckUserContrlDaiLiBacke*)pCmd;
	if (!pMsg)
	{
		ASSERT(false);
		return true;
	...
	//LC_Cmd_CheckClientInfo  
	//LC_Cmd_ChangeRoleGlobe msg;
	//SetMsgInfo(msg, GetMsgType(Main_Role, LC_ChangeRoleGlobeDown), sizeof(LC_Cmd_ChangeRoleGlobe));
	//msg.dwGlobeNum = DownGlobe;//m_RoleInfo.dwGlobeNum;
	//SendDataToClient(&msg);//只发送客户端去


	LC_Cmd_CheckClientInfoContrlDaili msg;
	SetMsgInfo(msg, GetMsgType(Main_Role, LC_CheckRoleUserContrlDailito), sizeof(LC_Cmd_CheckClientInfoContrlDaili));
	...
	msg.BackeNum = pMsg->BackeNum;
	pRole->SendDataToClient(&msg);
	//SendNewCmdToClient(pClient, &msg);

	//LogInfoToFile("Control.txt", "OnHandleCheckUserContrlDailiTo赠送**************** %d-------------%d-------", pMsg->dwUserID, pMsg->BackeNum);

	return true;
}
//////////////////////////////////////////////////////////////////////////
//玩家绑定代理  邀请码   检测结果返回客户端
bool FishServer::OnHandleCheckUserDailiBond(NetCmd* pCmd)
{
	//	DBO_Cmd_CheckUserDaiLiBake

	DBO_Cmd_CheckUserDailiBond * pMsg = (DBO_Cmd_CheckUserDailiBond*)pCmd;
	if (!pMsg)
	{
		ASSERT(false);
		return true;
	...
	LC_Cmd_CheckUserDailiBond msg;
	SetMsgInfo(msg, GetMsgType(Main_Role, LC_CheckRoleUserDailiBond), sizeof(LC_Cmd_CheckUserDailiBond));
	msg.dwUserID = pMsg->dwUserID;
	msg.Result = true;
	msg.BackeNum = pMsg->BackeNum;
	pRole->SendDataToClient(&msg);
	...
	//LogInfoToFile("Control.txt", "OnHandleCheckUserDailiBond代理绑定**************** %d-------------%d-------", pMsg->dwUserID, pMsg->BackeNum);

	return true;
}

//玩家绑定代理  邀请码   绑定结果返回客户端
bool FishServer::OnHandleUserDailiBondResult(NetCmd* pCmd)
{
	//	DBO_Cmd_CheckUserDaiLiBake

	DBO_Cmd_UserDailiBondResult * pMsg = (DBO_Cmd_UserDailiBondResult*)pCmd;
	if (!pMsg)
	{
		ASSERT(false);
		return true;
	...
	LC_Cmd_UserDailiBondResult msg;
	SetMsgInfo(msg, GetMsgType(Main_Role, LC_RoleUserDailiBondResult), sizeof(LC_Cmd_UserDailiBondResult));
	msg.dwUserID = pMsg->dwUserID;
	msg.Result = true;
	msg.BackeNum = pMsg->BackeNum;
	pRole->SendDataToClient(&msg);
	...
	//LogInfoToFile("Control.txt", "OnHandleCheckUserDailiBond代理绑定jie结果**************** %d-------------%d-------", pMsg->dwUserID, pMsg->BackeNum);
...
//////////////////////////////////////////////////////////////////////////
//代理
bool FishServer::OnHandleCheckUserDailiTo(NetCmd* pCmd)
{
//	DBO_Cmd_CheckUserDaiLiBake

		DBO_Cmd_CheckUserDaiLiBake * pMsg = (DBO_Cmd_CheckUserDaiLiBake*)pCmd;
	if (!pMsg)
	{
		ASSERT(false);
		return true;
	...
	//SendDataToClient(&msg);//只发送客户端去
		...
		SetMsgInfo(msg, GetMsgType(Main_Role, LC_CheckRoleUserDailito), sizeof(LC_Cmd_CheckClientInfo));
		msg.dwUserID = pMsg->dwUserID;
		msg.Result = true;
		msg.BackeNum = pMsg->BackeNum;
		pRole->SendDataToClient(&msg);
	...
	//	LogInfoToFile("Control.txt", "OnHandleCheckUserDailiTo代理**************** %d-------------%d-------", pMsg->dwUserID, pMsg->BackeNum);
...
bool FishServer::HandleClientMsg(ServerClientData* pClient, NetCmd* pCmd)
{
	...
	{
		//LogInfoToFile("Control.txt", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA开头开始命令处理类了有没有::::::::::: %d :::::::::::::::%d:::::::%d::::::", pCmd->CmdType, pCmd->SubCmdType, sizeof(pCmd));
	}
	...
	case Main_Control:
	{
			switch (pCmd->SubCmdType)
			{
					case CL_CheckClientInfo://先验证
					{
						...
						//msg.Result = (pMsg->RankValue == 000000 && bvaliddmac);//登陆成功了 可以进入m_ControlRankValue
						//FishServer::SendNetCmdToClient(pClient, &msg);

						//保存到数据库去
						DBR_Cmd_CheckUserDaiLi msgDB;//代理的金币信息
						SetMsgInfo(msgDB, DBR_CheckUserIsDaili, sizeof(DBR_Cmd_CheckUserDaiLi));
						msgDB.dwUserID = pMsg->dwUserID;
						msgDB.dwGlobel = pMsg->dwGlobeNum;	
						SendNetCmdToSaveDB(&msgDB);

						//LogInfoToFile("Control.txt", "CL_CheckClientInfo先验证*********** %d-----------:%d-------:%d---", pMsg->dwUserID, pMsg->dwGlobeNum,pMsg->RankValue);
						//pRole->ChangeRoleGlobe(pMsg->dwGlobeNum, true, true, true, pMsg->dwDaiLiUserID, true, pMsg->bIsZongDai);
						return true;

					}
						break;
	
					case  CL_CheckClientDailiBond://检测玩家是否绑定代理
					{
													  CL_Cmd_CheckClientDailiBond* pMsg = (CL_Cmd_CheckClientDailiBond*)pCmd;
													  if (!pMsg)
													  {
														  ASSERT(false);
														  return true;
													  ...
													  DBR_Cmd_CheckUserDailiBond msgDB;//代理的金币信息
													  SetMsgInfo(msgDB, DBR_CheckUserDailiBond, sizeof(DBR_Cmd_CheckUserDailiBond));
													  msgDB.dwUserID = pMsg->dwUserID;
													  msgDB.dwOtherUserID = pMsg->dwOtherUserID;
													  SendNetCmdToSaveDB(&msgDB);
					...
					case CL_ClientDailiBond:
					{
											   CL_Cmd_ClientDailiBond* pMsg = (CL_Cmd_ClientDailiBond*)pCmd;
											   if (!pMsg)
											   {
												   ASSERT(false);
												   return true;
											   ...
											   DBR_Cmd_UserDailiBond msgDB;//代理的金币信息
											   SetMsgInfo(msgDB, DBR_UserDailiBond, sizeof(DBR_Cmd_UserDailiBond));
											   msgDB.dwUserID = pMsg->dwUserID;
											   msgDB.dwOtherUserID = pMsg->dwOtherUserID;
											   SendNetCmdToSaveDB(&msgDB);
					...
					case CL_CheckClientInfoContrlDaili://检测赠送
					{					
														   
														   CL_Cmd_CheckClientInfoContrlDaili* pMsg = (CL_Cmd_CheckClientInfoContrlDaili*)pCmd;
														   if (!pMsg)
														   {
															   ASSERT(false);
															   return true;
														   }
														   DBR_Cmd_CheckUserContrlDaiLi msgDB;//代理的金币信息
														   SetMsgInfo(msgDB, DBR_CheckUserIsContrlDaili, sizeof(DBR_Cmd_CheckUserContrlDaiLi));
														   msgDB.dwUserID = pMsg->dwUserID;
														   msgDB.dwOtherUserID = pMsg->dwOtherUserID;
														   SendNetCmdToSaveDB(&msgDB);
														  return true;
					...
					case CL_KickUserByID:
					{
											...
											//LogInfoToFile("Control.txt", "CL_KickUserByID开始命令处理类了有没有201 %d", pMsg->dwUserID, pMsg->FreezeMin);
											return true;
					}
						break;
					case CL_QueryOnlineUserInfo://请求在线玩家列表pClient->OutsideExtraData
					{
												...
												//LogInfoToFile("Control.txt", "CL_QueryOnlineUserInfo请求在线玩家列表:::::::%d|||:::::::||||%d::::::::", pMsg->dwUserID, pMsg->ClientID);
												CRoleEx * pRole = GetRoleManager()->QueryUser(pMsg->dwUserID);
												if (pRole)
												{
													LC_Cmd_QueryOnlineUserInfo msg;
													SetMsgInfo(msg, GetMsgType(Main_Role, LC_QueryOnlineUserInfoRol), sizeof(LC_Cmd_QueryOnlineUserInfo));
													//msg.ClientID = pMsg->ClientID;
...
//////////////////////////////////////////////////////////////////////////
												...
												else
												{
													...
													//////////////////////////////////////////////////////////////////////////
													//SendNetCmdToControl(&msg);
													//LogInfoToFile("Control.txt", "CL_QueryOnlineUserInfo开始命令处理类了有没有::::: %d::::::", pMsg->ClientID);
													return true;
												}
					}
						break;
					case CL_ChangeaNickName:
					{
											...
											LogInfoToFile("Control.txt", "CL_ChangeaNickName开始命令处理类了有没有::::: %d::::||||%s:::::::", pMsg->dwUserID, pMsg->NickName);
											if (!pMsg)
											{
												ASSERT(false);
												return false;
					...
					case CL_ChangeParticularStates:
					{
								...
								//LogInfoToFile("Control.txt", "CL_ChangeParticularStates开始命令处理类了有没有::::: %d::::||||%d:::::", pMsg->dwUserID, pMsg->ParticularStates);
								if (!pMsg)
								{
									ASSERT(false);
									return false;
					...
					case CL_ReloadConfig:
					{
											...
											OnReloadConfig();
											m_TableManager.ReadBWIni();//刷新黑白名单
											//LogInfoToFile("Control.txt", "CL_ReloadConfig重新加载配置文件::::::");
											return true;
					}
						break;
					case CL_QueryFishPool://pClient->OutsideExtraData;
					{
											// pClient->OutsideExtraData;
											 ...
											 pResult->SetCmdType((Main_Role << 8) | LC_QueryFishPoolResultRol);
											...
											// pResult->ClientID = ((CL_CMD_QueryFishPool*)pCmd)->ClientID;
											 ...
											 pResult->ClientID = pClient->OutsideExtraData;//赋值

											 pResult->byServerid = m_GameNetworkID;
											 int nIndex = 1;//修改原数值  1   

											 //for (HashMap<BYTE, TableInfo>::iterator it = tableinfo.begin(); it != tableinfo.end(); it++)
											 //{
												// m_TableManager.QueryPool(it->second.TableTypeID, pResult->table[nIndex].bopen, pResult->table[nIndex].npool);//查看桌子的库存信息
												// nIndex++;
											 ...
											 for (int i = 0; i < 5; i++)
											 {
												 ...
												 pResult->lTableTypePool[i] = m_TableManager.QueryTableTypePool(i);
										 LogInfoToFile("Control.txt", "CL_QueryFishPool开始命令处理类了有没有::::: %d::::||||%d:::::::::::", m_TableManager.QueryTableTypePool(i), pResult->lTableTypePool[i]);
											 }

											 


											 CL_CMD_QueryFishPool* pMsg = (CL_CMD_QueryFishPool*)pCmd;
											...
											// SendNetCmdToControl(pResult);   //SendNewCmdToClient
											 pRole->SendDataToClient(pResult);//发送消息

											// SendNewCmdToClient(pClient,pResult);//
											 free(pResult);										
											 return true;
					}
						break;
					case CL_QueryBlackList:
					{
											  ...
											  pResult->SetCmdSize(nSize);
											  pResult->SetCmdType((Main_Role << 8) | LC_QueryBlackListResultRol);
											  pResult->ClientID = pClient->OutsideExtraData; //((LC_CMD_QueryFishBlackList*)pCmd)->ClientID;
											  pResult->byServerid = m_GameNetworkID;
											  int nIndex = 1;
											  for (std::list<DWORD>::iterator it = blacklist.begin(); it != blacklist.end(); it++)
											  {
												  pResult->dwUserID[nIndex] = *it;
												 // LogInfoToFile("Control.txt", "CL_QueryBlackList开始命令处理类了有没有黑名单::::: %d::::||||::%d::::::%d:::::", pResult->dwUserID[nIndex], m_GameNetworkID, pResult->ClientID);
												  nIndex++;
											  }
											  //////////////////////////////////////////////////////////////////////////
											  CRoleEx* pRoleContrl = GetRoleManager()->QuertUserBySocketID(pClient->OutsideExtraData);
											  ...
											  pRoleContrl->SendDataToClient(pResult);
											  //////////////////////////////////////////////////////////////////////////
					...
					case CL_SetBlackList://
					{
											 ...
											 result.SetCmdType((Main_Role << 8) | LC_SetBlackListResultRol);
											 result.ClientID = pClient->OutsideExtraData;//((LC_CMD_SetFishBlackList*)pCmd)->ClientID;
											 ...
											 pRoleContrl->SendDataToClient(&result);
											 //////////////////////////////////////////////////////////////////////////

											// SendNetCmdToControl(&result);

											// LogInfoToFile("Control.txt", "CL_QueryFishPool开始命令处理类了有没有::::: %d::::||||%d::::::%d::::", nCount, m_GameNetworkID, result.ClientID);
											 return  true;
					}
						break;
					case CL_ResetRolePassword:
					{
												...
												// LogInfoToFile("Control.txt", "CL_ResetRolePassword开始命令处理类低调低调顶顶顶顶顶了有没有::::: %d::::||||%d::::::%d:::::::::::", pMsg->dwUserID, pMsg->Password3, pMsg->Password1);
												 return true;
					}
						break;

					case CL_ChangeUserGold://增加金币
					{
														
								  CL_Cmd_ChangeUserGold* pMsg = (CL_Cmd_ChangeUserGold*)pCmd;
								  ...
								  //msg.dwGlobeNum = pMsg->dwGlobeNum;//m_RoleInfo.dwGlobeNum;
								  //pRole->SendDataToClient(&msg);//只发送客户端去
								//  LC_ChangeRoleGlobeSuccess
								  ...
								  //msgTable.dwDestUserID = pMsg->dwUserID;
								  //msgTable.dwGlobelSum = pMsg->dwGlobeNum;//m_RoleInfo.dwGlobeNum;
								  //pRole->SendDataToTable(&msgTable);
								  //////////////////////////////////////////////////////////////////////////
								  //pRole->ChangeRoleGlobe(pMsg->dwGlobeNum, true, true, true, pMsg->dwDaiLiUserID, true, pMsg->bIsZongDai);
								  LogInfoToFile("Control.txt", "CL_ChangeUserGold增加金币pMsg->dwUserID::::: %d::::||||%d::::::::::%d::::%d:::", pMsg->dwUserID, pMsg->dwGlobeNum, pMsg->dwDaiLiUserID, pMsg->bIsZongDai);
									  //给玩家充值								
									  //成功后减少代理金币
									  if (!pMsg->bIsZongDai)
									  {
										 // ChangeGlobeContrlDown(pMsg->dwGlobeNum, true, true, true, pMsg->dwDaiLiUserID, true, true);

										  //先减少代理金币
										  if (ChangeGlobeContrlDown(pMsg->dwGlobeNum, true, true, true, pMsg->dwDaiLiUserID, true, pMsg->bIsZongDai))
										  {											 
											  if (!pRole)
											  {
												  // LC_Cmd_ChangeRoleGlobeSuccess  
												  CRoleEx* pRole33 = m_RoleManager.QueryUser(pMsg->dwDaiLiUserID);
												  if (!pRole33)
												  {
													 // LogInfoToFile("Control.txt", "CL_ChangeUserGold增加金币pMsg->dwUserID玩家不在线代理也不在线充值失败::::: %d::::||||%d::::::::::%d::::%d:::", pMsg->dwUserID, pMsg->dwGlobeNum, pMsg->dwDaiLiUserID, pMsg->bIsZongDai);
													  return false;
												  }
												  LC_Cmd_ChangeRoleGlobeSuccess msg;
												  SetMsgInfo(msg, GetMsgType(Main_Role, LC_ChangeRoleGlobeSuccess), sizeof(LC_Cmd_ChangeRoleGlobeSuccess));
												  msg.dwGlobeNum = pMsg->dwGlobeNum;//m_RoleInfo.dwGlobeNum;
												  msg.dwUserID = pMsg->dwUserID;
												  msg.bIsSuccess = true;//
												  msg.bIsUserOnline = false;//充值玩家z不在线

												  pRole33->SendDataToClient(&msg);//只发送客户端去  通知给代理或者管理

												  return true;
											  }
											  //后增加玩家金币  第三参数就是是否存数据库   Z这里是代理充值 
											  if (pRole->ChangeRoleGlobe(pMsg->dwGlobeNum, true, true, true, pMsg->dwDaiLiUserID, true, pMsg->bIsZongDai))
											  {
												  //玩家金币充值成功

												  CRoleEx* pRole557 = m_RoleManager.QueryUser(pMsg->dwDaiLiUserID);

												  if (!pRole557)
												  {
													 // LogInfoToFile("Control.txt", "CL_ChangeUserGold增加金币pMsg->dwUserID玩家不在线代理也不在线充值失bAAAAAAAAAAA败::::: %d::::||||%d::::::::::%d::::%d:::", pMsg->dwUserID, pMsg->dwGlobeNum, pMsg->dwDaiLiUserID, pMsg->bIsZongDai);
													  return false;
												  }
												  //充值失败  
												  LC_Cmd_ChangeRoleGlobeSuccess msg;
												  ...
												  msg.dwUserID = pMsg->dwUserID;
												  msg.bIsSuccess = true;//
												  msg.bIsUserOnline = true;//充值玩家z不在线
												  msg.bIslowuserGold = false;//减少
												  pRole557->SendDataToClient(&msg);//只发送客户端去  通知给代理或者管理
											  ...
											  {
												  //玩家金币充值失败
												  CRoleEx* pRole665 = m_RoleManager.QueryUser(pMsg->dwDaiLiUserID);
												  if (!pRole665)
												  {
													 // LogInfoToFile("Control.txt", "CL_ChangeUserGold增加金币pMsg->dwUserID玩家不在线代理也不在线充值失bAAAAAAAAAAA败::::: %d::::||||%d::::::::::%d::::%d:::", pMsg->dwUserID, pMsg->dwGlobeNum, pMsg->dwDaiLiUserID, pMsg->bIsZongDai);
													  return false;
												  ...
												  msg.bIsUserOnline = true;//充值玩家z不在线
												  msg.bIslowuserGold = false;//减少
												  pRole665->SendDataToClient(&msg);//只发送客户端去  通知给代理或者管理
												  return true;
										  ...
										  {
											  //减少代理金币失败  直接通知充值失败
											   CRoleEx* pRole44 = m_RoleManager.QueryUser(pMsg->dwDaiLiUserID);
											   if (!pRole44)
											   {
											   // LogInfoToFile("Control.txt", "CL_ChangeUserGold增加金币pMsg->dwUserID玩家不在线代理也不在线充值失bAAAAAAAAAAA败::::: %d::::||||%d::::::::::%d::::%d:::", pMsg->dwUserID, pMsg->dwGlobeNum, pMsg->dwDaiLiUserID, pMsg->bIsZongDai);
											    return false;
											   ...
											   msg.bIsUserOnline = true;//充值玩家z不在线
											   msg.bIslowuserGold = false;//减少
											   pRole44->SendDataToClient(&msg);//只发送客户端去  通知给代理或者管理
									  ...
									  else
									  {
										  ...
										  //{
											 ...
											 // // LC_Cmd_ChangeRoleGlobeSuccess  
											 // CRoleEx* pRole33 = m_RoleManager.QueryUser(pMsg->bIsZongDai);
											 // if (!pRole33)
											 // {
												// // LogInfoToFile("Control.txt", "CL_ChangeUserGold增加金币pMsg->dwUserID玩家不在线代理也不在线充值失败::::: %d::::||||%d::::::::::%d::::%d:::", pMsg->dwUserID, pMsg->dwGlobeNum, pMsg->dwDaiLiUserID, pMsg->bIsZongDai);
												//  return false;
										  ...
										  //是否是减少金币    //如果是减少金币  充错金币 
										  if (pMsg->bIsScoreDown)
										  {

											  if (ChangeGlobeContrlDown(pMsg->dwGlobeNum, true, true, true, pMsg->dwUserID, true, pMsg->bIsZongDai))
											  {
												 
												  //玩家金币充值成功
												  CRoleEx* pRole555 = m_RoleManager.QueryUser(pMsg->dwDaiLiUserID);
												  ...
												  msg.bIslowuserGold = true;//减少
												  pRole555->SendDataToClient(&msg);//只发送客户端去  通知给代理或者管理
											  ...
											  {
												  //玩家金币充值失败
												  CRoleEx* pRole666 = m_RoleManager.QueryUser(pMsg->dwDaiLiUserID);
												  ...
												  msg.bIsUserOnline = true;//充值玩家z不在线
												  msg.bIslowuserGold = true;//减少
												  pRole666->SendDataToClient(&msg);//只发送客户端去  通知给代理或者管理
										  ...
										  {
											  //如果是减少金币  充错金币  
											  //(ChangeGlobeContrlDown(pMsg->dwGlobeNum, true, true, true, pMsg->dwDaiLiUserID, true, pMsg->bIsZongDai));

											  //是管理  第三参数就是数据库是否保存  这里是代理充值
											  if (pRole->ChangeRoleGlobe(pMsg->dwGlobeNum, true, true, true, pMsg->dwDaiLiUserID, true, pMsg->bIsZongDai))
											  {
												  //玩家金币充值成功

												  CRoleEx* pRole55 = m_RoleManager.QueryUser(pMsg->dwDaiLiUserID);
												  ...
												  msg.bIslowuserGold = false;//减少
												  pRole55->SendDataToClient(&msg);//只发送客户端去  通知给代理或者管理
											  ...
											  else
											  {
												  ...
												  CRoleEx* pRole66 = m_RoleManager.QueryUser(pMsg->dwDaiLiUserID);
												  ...
												  msg.bIsUserOnline = true;//充值玩家z不在线
												  msg.bIslowuserGold = false;//减少
												  pRole66->SendDataToClient(&msg);//只发送客户端去  通知给代理或者管理
								  ...
								  //{
									 ...
									 // if (!pRole44)
									 // {
										//  LogInfoToFile("Control.txt", "CL_ChangeUserGold增加金币pMsg->dwUserID玩家不在线代理也不在线充值失AAAAAAAAAAA败::::: %d::::||||%d::::::::::%d::::%d:::", pMsg->dwUserID, pMsg->dwGlobeNum, pMsg->dwDaiLiUserID, pMsg->bIsZongDai);
										//  return false;
									 // }
									 // //充值失败  金币超限
									 // LC_Cmd_ChangeRoleGlobeSuccess msg;
									 ...
									 // msg.dwUserID = pMsg->dwUserID;
									 // msg.bIsSuccess = false;//
									 // msg.bIsUserOnline = true;//充值玩家不在线

									 // pRole44->SendDataToClient(&msg);//只发送客户端去  通知给代理或者管理MT_Game
								  //}
					...
					case CL_QRoleDaili:
					{
										  ...
										  CL_Cmd_QRoleDaili* pMsg = (CL_Cmd_QRoleDaili*)pCmd;
										  if (!pMsg)
										  {
											  ASSERT(false);
											  return  true;
										  ...
										  TCHAR OutBuf[128];
										  _sntprintf(m_szIniFileName, sizeof(m_szIniFileName), TEXT("fishing/fishing_%d.ini"), 2);
										  TCHAR szBuffer[MAX_PATH] = { 0 };


										  LC_Cmd_GetRoleDaili msg;
										  SetMsgInfo(msg, GetMsgType(Main_Role, LC_GetRoleDaili), sizeof(LC_Cmd_GetRoleDaili));

										  memset(OutBuf, 0, 128);
										  GetPrivateProfileString(TEXT("public"), TEXT("GoodUser_1"), TEXT("1"), OutBuf, 128, m_szIniFileName);
										  _sntscanf(OutBuf, lstrlen(OutBuf), _TEXT("%s"), &msg.NumName1);

										  memset(OutBuf, 0, 128);
										  GetPrivateProfileString(TEXT("public"), TEXT("GoodUser_2"), TEXT("1"), OutBuf, 128, m_szIniFileName);
										  _sntscanf(OutBuf, lstrlen(OutBuf), _TEXT("%s"), &msg.NumName2);

										  memset(OutBuf, 0, 128);
										  GetPrivateProfileString(TEXT("public"), TEXT("GoodUser_3"), TEXT("1"), OutBuf, 128, m_szIniFileName);
										  _sntscanf(OutBuf, lstrlen(OutBuf), _TEXT("%s"), &msg.NumName3);

										  memset(OutBuf, 0, 128);
										  GetPrivateProfileString(TEXT("public"), TEXT("GoodUser_4"), TEXT("1"), OutBuf, 128, m_szIniFileName);
										  _sntscanf(OutBuf, lstrlen(OutBuf), _TEXT("%s"), &msg.NumName4);

										  memset(OutBuf, 0, 128);
										  GetPrivateProfileString(TEXT("public"), TEXT("GoodUser_5"), TEXT("1"), OutBuf, 128, m_szIniFileName);
										  _sntscanf(OutBuf, lstrlen(OutBuf), _TEXT("%s"), &msg.NumName5);

										  memset(OutBuf, 0, 128);
										  GetPrivateProfileString(TEXT("public"), TEXT("GoodUser_6"), TEXT("1"), OutBuf, 128, m_szIniFileName);
										  _sntscanf(OutBuf, lstrlen(OutBuf), _TEXT("%s"), &msg.NumName6);

										  memset(OutBuf, 0, 128);
										  GetPrivateProfileString(TEXT("public"), TEXT("GoodUser_7"), TEXT("1"), OutBuf, 128, m_szIniFileName);
										  _sntscanf(OutBuf, lstrlen(OutBuf), _TEXT("%s"), &msg.NumName7);

										  memset(OutBuf, 0, 128);
										  GetPrivateProfileString(TEXT("public"), TEXT("GoodUser_8"), TEXT("1"), OutBuf, 128, m_szIniFileName);
										  _sntscanf(OutBuf, lstrlen(OutBuf), _TEXT("%s"), &msg.NumName8);

										  //memset(OutBuf, 0, 128);
										  //GetPrivateProfileString(TEXT("public"), TEXT("GoodUser_9"), TEXT("1"), OutBuf, 128, m_szIniFileName);
										  //_sntscanf(OutBuf, lstrlen(OutBuf), _TEXT("%s"), &msg.NumName9);

										  //memset(OutBuf, 0, 128);
										  //GetPrivateProfileString(TEXT("public"), TEXT("GoodUser_10"), TEXT("1"), OutBuf, 128, m_szIniFileName);
										  //_sntscanf(OutBuf, lstrlen(OutBuf), _TEXT("%s"), &msg.NumName10);
										//  LogInfoToFile("Control.txt", "CL_ResetRolePassword开始命令处理类低调低调顶顶顶顶顶了有没有::::: %d::::||||%s::::::%s:::::::::::", pMsg->dwUserID, msg.NumName8, msg.NumName7);
										  //CRoleEx * pRole = GetRoleManager()->QueryUser(pMsg->dwUserID);
										  CRoleEx* pRole66 = m_RoleManager.QueryUser(pMsg->dwUserID);
										  if (!pRole66)
										  {
											//  LogInfoToFile("Control.txt", "CL_ResetRolePassword开始命令处理类低调低调顶顶959595959::::: %d::::||||%s::::::%s:::::::::::", pMsg->dwUserID, msg.NumName8, msg.NumName7);
											  ASSERT(false);
											  return false;
										  }
										  else
										  {
											//  LogInfoToFile("Control.txt", "CL_ResetRolePassword开始命令处理类低调低调顶顶顶顶顶了有没有反反复复::::: %d::::||||%s::::::%s:::::::::::", pMsg->dwUserID, msg.NumName8, msg.NumName7);
											  pRole66->SendDataToClient(&msg);
											  return true;
										  }
										  //pRole66->SendDataToClient(&msg);//只发送客户端去  通知给代理或者管理
					...
					case CL_SendMsgToAllGame:
					{
								...
								//SendMessageByType(pMsg->Message, pMsg->MessageSize, pMsg->MessageType, /*MessageType::MT_Game,*/ pMsg->MessageColor, pMsg->StepNum, pMsg->StepSec, pMsg->Param, true);
									...
									SendMessageByType(pMsg->MessageInfo, _tcslen(pMsg->MessageInfo), MT_Game, pMsg->MessageColor, pMsg->StepNum, pMsg->StepSec, pMsg->Param, true);



								//	LogInfoToFile("Control.txt", "CL_SendMsgToAllGame控制消息发送全服公告::::: %ld::::||||%d:::%s::////::::::", pMsg->MessageColor, pMsg->StepNum, pMsg->MessageInfo);
									//SendNetCmdToCenter(pMsg);
									return  true;
					}
						break;	
					case CL_SendSystemEmail:
					{
								...
								//	LogInfoToFile("Control.txt", "CL_SendSystemEmail控制消息发送系统邮件::::: %d::::||||::::%s::::::%d:::::::::::", pMsg->RewardID, pMsg->EmailContext, pMsg->dwUserID);
									return true;
					}
						break;

					case CL_HandleGailv://修改捕鱼概率
					{
									CL_Cmd_HandleGailv* pMsg = (CL_Cmd_HandleGailv*)pCmd;
									...
									LogInfoToFile("Control.txt", "CL_HandleGailv修改捕鱼概率:::::::%f::::::::::%f:::::", pMsg->RNumrateF, pMsg->RNumrateS);
									//概率
									m_TableManager.OnChangeGailv(pMsg->RNumrateF, pMsg->RNumrateS, pMsg->RNumrateT, pMsg->RNumrateR, true);
									return true;
					}
						break;
					...
					case CL_OxAdmin:
					{
									 //  LogInfoToFile("Control.txt", "CL_OxAdmin控制消息牛牛控制消息::::::::::::::%d::::::::::%d::::", sizeof(CL_CMD_OxAdminReq), pCmd->GetCmdSize());
									//   if (pCmd->GetCmdSize() == sizeof(CL_CMD_OxAdminReq))//大小不相等  暂时屏蔽
									   //{
					...
					case CL_AnimalAdmin:
					{
										  ...
										  // {
											   ...
											   SendNetCmdToMiniGame(pCmd);
										   //}
										   LogInfoToFile("Control.txt", "CL_OxAdmin控制消息森林舞会控制消息:::::");
										   return true;
					...
					case CL_CarAdmin:
					{
										...
										LogInfoToFile("Control.txt", "CL_OxAdmin控制消息跑车控制消息:::::");
										return true;
					}
						break;
					case CL_ChangeTablePool://修改桌子的库存值
					{

				               CL_Cmd_HandleTablePoolGailv* pMsg = (CL_Cmd_HandleTablePoolGailv*)pCmd;
								...
								//库存值   影响幸运值
							//	m_TableManager.OnResetTableGlobel(pMsg->TableID, pMsg->nValue);
								m_TableManager.OnAddTableGlobel(pMsg->TableID, pMsg->nValue);
								
								LogInfoToFile("Control.txt", "CL_ChangeTablePool修改桌子的库存值:::::::%d::::::::::%d:::::", pMsg->nValue, pMsg->TableID);
								return true;
					}
						break;
...
//添加减少金币的函数
bool FishServer::ChangeGlobeContrlDown(DWORD ChangeGlobe, bool IsSendToClient, bool IsSaveToDB , bool IsSendToMiniGame , DWORD dwDaiLiUserID , bool bIsCanContrl , bool bIsZongDai )
{

//////////////////////////////////////////////////////////////////////////

	CRoleEx* pRole222 = m_RoleManager.QueryUser(dwDaiLiUserID);
	if (!pRole222)
	{
		return false;
	}
	//pRole222->ChangeRoleGlobeDown(ChangeGlobe, true, true, true, true);
	//LogInfoToFile("Control.txt", "ChangeGlobeContrlDown减少金币添加减少金币的函数::::: %d::::||||%d::::::::::%d::::::", dwDaiLiUserID, ChangeGlobe, bIsZongDai);
	if (pRole222->ChangeRoleGlobeDown(ChangeGlobe, true, true, true, true))
	{
		return true;
	}
	else
...
bool FishServer::HandleDataBaseMsg(NetCmd* pCmd)
{
	...
	case DBO_CheckDailiBond:
		return OnHandleCheckUserDailiBond(pCmd);//检测玩家绑定代理
	case DBO_DailiBondResult:
		return OnHandleUserDailiBondResult(pCmd);//玩家绑定代理结果

	case DBO_CheckUserIsDailiO:
		return OnHandleCheckUserDailiTo(pCmd);//检查玩家代理返回

	case DBR_CheckUserIsContrlDailiBack:
		return OnHandleCheckUserContrlDailiTo(pCmd);//检测赠送

	case DBO_GetAccountInfoByUserID://获取玩家信息
...
bool FishServer::HandleCenterMsg(NetCmd* pCmd)
{
	if (pCmd->CmdType==Main_Control)
	{
		LogInfoToFile("Control.txt", "HandleCenterMsg中央服务器发送来的数据201************ %d************* %d********", pCmd->CmdType, pCmd->SubCmdType);
	}
			...
			case CG_AddNormalOrderID:
				{
					...
					LogInfoToFile("Chongzhi.txt", TEXT("CG_AddNormalOrderID订单写入数据库成功发送消息通知客户端*************"));
...
{
	LogInfoToFile("Control.txt", "OnHandlePhonePay话费充值 %d ", pMsg->dwUserID);
	CRoleEx* pRole = GetRoleManager()->QueryUser(pMsg->dwUserID);
	if (!pRole)
	{
		if (pMsg->Result)
...
{
	LogInfoToFile("Control.txt", "OnHandleUseRMB人民币充值1 %d ", pMsg->rechargeInfo.UserID);
	...
	//LogInfoToFile("Control.txt", "充值******%d************%d********%d************%d**************** ", pMsg->rechargeInfo.UserID, Iter->second.IsFirstAdd(), pRole->GetRoleInfo().bIsFirstPayCurrcey, pRole->GetRoleInfo().bIsFirstPayGlobel);
	if (!pRole)
	{
	...
	else
	{
		...
		//直接操作玩家
		DWORD AddMoney = static_cast<DWORD>(pRole->GetRoleVip().AddReChargeRate() * Iter->second.AddMoney);//vip充值奖励

		//LogInfoToFile("Control.txt", "充值直接操作玩家****************%d*************%d************** ", AddMoney,Iter->second.AddMoney);
		if (Iter->second.IsAddCurrcey())
		{
		...
		else if (Iter->second.IsAddGlobel())
		{
			DWORD  OldCurrcey = pRole->GetRoleInfo().dwCurrencyNum;
			DWORD  OldGlobel = pRole->GetRoleInfo().dwGlobeNum;
			pRole->ChangeRoleGlobe(AddMoney, true, true, true);//第三参数就是保存数据库  是否  这里是第三方充值 
			if (Iter->second.IsFirstAdd())
...
//游戏启动后进入倒计时  定时保存玩家信息   p配置文件里面的时间  xin心跳设置
...
{
	//LogInfoToFile("Control.txt", "OnHandLogonLogonMsg获得玩家渠道的数据1:::::::::::: %d ::::::::", pCmd->SubCmdType);
	if (!pCmd)
		return false;
	switch (pCmd->SubCmdType)
	{
	...
	case LG_AccountOnlyID:
		{
			...
			//SendNetCmdToLogon(&msg);
			LogInfoToFile("LogonError.txt", TEXT("LG_AccountOnlyID 快速登陆  密码:%u,%u,%u"), pMsg->dwUserID, pMsg->dwOnlyID, pMsg->ClientID);
			return true;
		}
		break;
	}
...
{
	//LogInfoToFile("Control.txt", "OnHandClientLogonMsg处理客户端发送来的登陆数据:::::::: %d:::::: ", pCmd->SubCmdType);
	//处理客户端发送来的登陆数据
	if (!pClient || !pCmd)
		return false;
	switch (pCmd->SubCmdType)
	...
	case CL_ResetLogonGameServer:
		{
			...
			if (pRole)
			{
				...
				if (!pRole->IsAFK())
				{
					...
					//告诉客户端玩家登陆成功了
					pRole->OnUserLoadFinish(true);//加载玩家信息  金币值  1
				}
				else
				{
					//pRole->SetRoleExIsOnline(pClient->OutsideExtraData);
					m_RoleCache.OnDleRoleCache(pRole->GetUserID());
					pRole->ChangeRoleSocketID(pClient->OutsideExtraData);//修改缓存玩家的Socket
					pRole->OnUserLoadFinish(true);//加载玩家信息  金币值2
				}
				if (pRole->IsExit())
				{
					//ShowInfoToWin("玩家进行离线保存 在回发确认命令之前 重新登陆了  回发确认命令 失效 ");
...
{	
	...
	//LogInfoToFile("CollideBuYu.txt", "OnHandDBLogonMsg玩家登陆成功1************ %d***********玩家***%d********** ", pMsg->RoleInfo.dwGlobeNum, pMsg->RoleInfo.dwUserID);
	...
	if (pMsg->Result)
	{
		...
		if (pRole)
		{
			...
			if (!pRole->IsAFK())
			{
				...
				//pRole->
				//告诉客户端玩家登陆成功了  暂时
				pRole->OnUserLoadFinish(pMsg->LogonByGameServer);//玩家登陆 不在桌子里面  这里是玩家信息更新的  金币的获取 属于链表内存储的
				...
				// 新加的玩家登陆信息更新  金币更新   获取的还是缓存
				//pRole->OnUserLoadFinish(&pMsg->RoleInfo, &pMsg->RoleServerInfo, pMsg->LogonByGameServer);
			...
			else
			{
				...
				pRole->OnUserLoadFinish(false);//离线 进入 可能在桌子里面  获取玩家信息 金币等信息跟新位置   wanjia
				//新添
				//pRole->OnUserLoadFinish(&pMsg->RoleInfo, &pMsg->RoleServerInfo, false);
		...
		else if (!m_RoleManager.CreateRole(&pMsg->RoleInfo, &pMsg->RoleServerInfo, SocketID, pMsg->LastOnlineTime, pMsg->LogonByGameServer, pMsg->IsRobot))//第一次登陆 获取玩家信息 金币更新
		{
			//创建玩家失败后 让玩家离开
			ASSERT(false);
...
bool FishServer::OnHandleResetAccountName(NetCmd* pCmd)
{
	...
	//LogInfoToFile("Control.txt", "OnHandleResetAccountName挤掉玩家1 %d%d ", pCmd->SubCmdType, pMsg->dwUserID);
	if (!pMsg)
	{
		ASSERT(false);
		return false;
...
bool FishServer::OnHandleResetPasswordResult(NetCmd* pCmd)
{
	...
	LogInfoToFile("Control.txt", "OnHandleResetPasswordResult修改密码结果1 %d ", pCmd->SubCmdType);
	//修改密码的结果
	if (!pCmd)
	{
		ASSERT(false);
...
{
//	LogInfoToFile("Control.txt", "OnHandleTCPNetworkTable桌子消息:::::::::: %d:::::::: %d:::::::::::", pCmd->CmdType, pCmd->SubCmdType);
	if (!pCmd || !pClient)
		return false;
...
{
	//LogInfoToFile("Control.txt", "OnHandleRoleJoinTable桌子消息::::::%d::::::: %d::::::", pCmd->CmdType, pCmd->SubCmdType);
	if (!pCmd || !pClient)
		return false;
	//玩家进入桌子
...
{
//	LogInfoToFile("Control.txt", "OnHandleRoleLeaveTable离开桌子消息1 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pCmd || !pClient)
		return false;
	CRoleEx* pRole = m_RoleManager.QuertUserBySocketID(pClient->OutsideExtraData);
	if (!pRole)
...
{
	//LogInfoToFile("Control.txt", "OnHandleTCPNetworkGame游戏消息1 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pCmd || !pClient)
		return false;
...
{
//	LogInfoToFile("Control.txt", "OnGateLeaveGameRoom当一个网关离开1");
...
{
	LogInfoToFile("Control.txt", "OnHandleTCPNetworkItem玩家物品*********** %d******** %d*****", pCmd->CmdType, pCmd->SubCmdType);
	if (!pCmd || !pClient)
		return false;
	CRoleEx* pRole = m_RoleManager.QuertUserBySocketID(pClient->OutsideExtraData);
	if (!pRole)
...
{
	LogInfoToFile("Control.txt", "OnHandleTCPNetworkRelation关系1 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pCmd || !pClient)
		return false;
	CRoleEx* pRole = m_RoleManager.QuertUserBySocketID(pClient->OutsideExtraData);
	if (!pRole)
...
{
	LogInfoToFile("Control.txt", "OnHandleSocketRelation处理关系数据1 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pCmd)
		return false;
...
{
	LogInfoToFile("ControlMail.txt", "OnHandleTCPNetworkMail邮件1::::::: %d::::::::: %d::::::", pCmd->CmdType, pCmd->SubCmdType);
	if (!pClient || !pCmd)
		return false;
	//接收客户端发送来的邮件方面的数据
	CRoleEx* pRole = m_RoleManager.QuertUserBySocketID(pClient->OutsideExtraData);
...
{
	LogInfoToFile("Control.txt", "OnHandleTCPNetworkRole接收客户端发送来的:::: %d::::: %d:::::", pCmd->CmdType, pCmd->SubCmdType);
	if (!pClient || !pCmd)
		return false;
	//接收客户端发送来的 玩家数据 修改
	CRoleEx* pRole = m_RoleManager.QuertUserBySocketID(pClient->OutsideExtraData);
...
bool FishServer::OnHandleGameIDConvertUserID(NetCmd* pCmd)
{
	...
	LogInfoToFile("Control.txt", "OnHandleGameIDConvertUserID试图赠送物品给其他玩家1 %d %d", pCmd->CmdType, pCmd->SubCmdType);
...
{
	LogInfoToFile("Control.txt", "OnHandleTCPNetworkQuery接收到客户端的查询信息1 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pClient || !pCmd)
		return false;
	CRoleEx* pRole = m_RoleManager.QuertUserBySocketID(pClient->OutsideExtraData);
	if (!pRole)
	...
	switch (pCmd->SubCmdType)
	{
		...
		case CL_QueryUserByGameID:
			{
				...
				else
				{
					...
					//添加参数
					TCHARCopy(msg.RoleInfo.IPAddress, CountArray(msg.RoleInfo.IPAddress), pDestRole->GetRoleInfo().IPAddress, _tcslen(pDestRole->GetRoleInfo().IPAddress));


					TCHARCopy(msg.RoleInfo.szHeadHttp, CountArray(msg.RoleInfo.szHeadHttp), pDestRole->GetRoleInfo().szHeadHttp, _tcslen(pDestRole->GetRoleInfo().szHeadHttp));//头像

					LogInfoToFile("CL_QueryUserByGameID.txt", "查询玩家数据:::: %s::::: %d******", msg.RoleInfo.IPAddress, msg.RoleInfo.dwUserID);
					pRole->SendDataToClient(&msg);
				}
				return true;
			}
		...
		case CL_QueryUserByUserID:
		{
			...
			else
			{
				...
				//
				TCHARCopy(msg.RoleInfo.szHeadHttp, CountArray(msg.RoleInfo.szHeadHttp), pDestRole->GetRoleInfo().szHeadHttp, _tcslen(pDestRole->GetRoleInfo().szHeadHttp));
...
{
	LogInfoToFile("Control.txt", "OnHandleDataBaseLoadQueryUserInfo接收到DBO的命令了1 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	//接收到DBO的命令了 我们可以开始处理
...
{
	LogInfoToFile("Control.txt", "OnHandleDataBaseLoadQueryUserInfoByGameID加载1 %d %d", pCmd->CmdType, pCmd->SubCmdType);
...
{
	LogInfoToFile("Control.txt", "OnHandleDataBaseLoadQueryUserInfoByUserID加载1111111111 %d %d", pCmd->CmdType, pCmd->SubCmdType);
...
{
	LogInfoToFile("Control.txt", "OnHandleTCPNetworkCheck签到1 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pClient || !pCmd)
		return false;
	CRoleEx* pRole = m_RoleManager.QuertUserBySocketID(pClient->OutsideExtraData);
	if (!pRole)
...
{
	//LogInfoToFile("Control.txt", "OnHandleTCPNetworkTask任务******%d*******%d******", pCmd->CmdType, pCmd->SubCmdType);
	if (!pClient || !pCmd)
		return false;
	CRoleEx* pRole = m_RoleManager.QuertUserBySocketID(pClient->OutsideExtraData);
	if (!pRole)
...
{
	//LogInfoToFile("Control.txt", "OnHandleSocketTask任务66666 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pCmd)
		return false;
	switch (pCmd->SubCmdType)
	{
...
{
	//LogInfoToFile("Control.txt", "OnHandleDataBaseLoadUserTaskInfo从数据库读取任务信息66666 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	//从数据库读取任务信息
...
{
	//LogInfoToFile("Control.txt", "OnHandleTCPNetworkAchievement成就////////%d//////// %d//////", pCmd->CmdType, pCmd->SubCmdType);
	if (!pClient || !pCmd)
		return false;
	CRoleEx* pRole = m_RoleManager.QuertUserBySocketID(pClient->OutsideExtraData);
	if (!pRole)
...
{
	//LogInfoToFile("Control.txt", "OnHandleTCPNetworkMonth判断是符合比赛的要求倍率++++++ %d +++++++%d++++", pCmd->CmdType, pCmd->SubCmdType);
	if (!pClient || !pCmd)
		return false;
	CRoleEx* pRole = m_RoleManager.QuertUserBySocketID(pClient->OutsideExtraData);
	if (!pRole)
...
{
	//LogInfoToFile("Control.txt", "OnHandleSocketMonth接收到中央服务器发送来的比赛的信息66666 ********%d*************** %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pCmd)
		return false;
	//接收到中央服务器发送来的比赛的信息
	switch (pCmd->SubCmdType)
...
{
	LogInfoToFile("Control.txt", "OnHandleTCPNetworkRank排行66666 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pClient || !pCmd)
		return false;
	CRoleEx* pRole = m_RoleManager.QuertUserBySocketID(pClient->OutsideExtraData);
	if (!pRole)
...
{
	//LogInfoToFile("Control.txt", "OnHandleTCPNetworkCharm魅力66666 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pClient || !pCmd)
	{
		return false;
	}
...
{
	LogInfoToFile("Control.txt", "OnHandleSocketCharm处理中央服务器发送来的消息66666 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pCmd)
		return false;
	//处理中央服务器发送来的消息
	switch (pCmd->SubCmdType)
...
bool FishServer::OnHandleTCPNetworkShop(ServerClientData* pClient, NetCmd* pCmd)
{
	...
	LogInfoToFile("Control.txt", "CL_ShopGoldItem金币兑换======::*%d****** %d *****%d*****", pRoleEx->GetScore(), pRoleEx->GetUserID(), pCmd->SubCmdType);
	if (!pRoleEx)
	{
		ASSERT(false);
		return false;
	...
	case CL_ShopItem:
		{
			...
			//msgDB.ItemID = 2256;
			//msgDB.ItemSum = 2256;
			//msgDB.UseMedal = pMsg->ChangeGlobel;//花费的奖牌数量  金币
			//msgDB.NowMedalSum = pRoleEx->GetScore();//剩余
			////设置现实的地址数据
			////tagRoleAddressInfo& pInfo = pRole->GetRoleEntity().GetEntityInfo();
			//TCHARCopy(msgDB.Name, CountArray(msgDB.Name), pMsg., _tcslen
			//	(pInfo.EntityItemUseName));
			//TCHARCopy(msgDB.IDEntity, CountArray(msgDB.IDEntity), pInfo.IdentityID, _tcslen
			//	(pInfo.IdentityID));
			//msgDB.Phone = pInfo.EntityItemUsePhone;
			//TCHARCopy(msgDB.Addres, CountArray(msgDB.Addres), ,
			//	_tcslen(pInfo.EntityItemUseAddres));
			//g_FishServer.SendNetCmdToSaveDB(&msgDB);
			//g_FishServer.GetAnnouncementManager().OnAddNewAnnouncementOnce(pRole->GetRoleInfo
			//	().NickName, 22, 22);//添加一条公告

			//pRoleEx->ChangeRoleCashSum(1);//添加次数

			//pRoleEx->ChangeRoleShareStates(false);

		//	LogInfoToFile("RoleManager.txt", "OnHandleTCPNetworkShop商城物品***::::::%d ====::%d*******%d----%d------", pRoleEx->GetUserID(), pMsg->ShopID, pMsg->ItemSum, pMsg->ShopItemIndex);
			//////////////////////////////////////////////////////////////////////////
			m_ShopManager.OnShellShopItem(pRoleEx, pMsg->ShopID, pMsg->ShopItemIndex, pMsg->ItemSum);
			return true;
		}
	case CL_ShopGoldItem:
	{
							CL_Cmd_ShopGoldItem* pMsg = (CL_Cmd_ShopGoldItem*)pCmd;
							if (!pMsg)
							{
								ASSERT(false);
								return false;
							...
							TCHARCopy(msgDB.Name, CountArray(msgDB.Name), pMsg->UseName, _tcslen(pMsg->UseName));

							TCHARCopy(msgDB.IDEntity, CountArray(msgDB.IDEntity), pMsg->IDEntity, _tcslen(pMsg->IDEntity));
							//msgDB.IDEntity[33] = {1111};//身份证,
							//LogInfoToFile("Control.txt", "CL_ShopGoldItem金币兑换======::*%d****** %d *****%d*****", pMsg->dwUserID, pRoleEx->GetUserID(), pMsg->ChangeGlobel);
							msgDB.Phone = 156865452;//手机号 
							TCHARCopy(msgDB.Addres, CountArray(msgDB.Addres), pMsg->Addres, _tcslen(pMsg->Addres));


							g_FishServer.SendNetCmdToSaveDB(&msgDB);


							g_FishServer.GetAnnouncementManager().OnAddNewAnnouncementOnce(pRoleEx->GetRoleInfo().NickName, 1, 1);//添加一条公告
							...
							//并非正常充值  只是兑换金币减少玩家金币
							//ChangeGlobeContrlDown(pMsg->ChangeGlobel, true, true, true, pMsg->dwUserID, false, false);//

							//pRoleEx->ChangeRoleGlobeDown(pMsg->ChangeGlobel, true, true, true, true);

							LC_Cmd_ShopItemResult msg;
							SetMsgInfo(msg, GetMsgType(Main_Shop, LC_ShopItemResult), sizeof(LC_Cmd_ShopItemResult));

							if (pRoleEx->ChangeRoleGlobeDown(pMsg->ChangeGlobel, true, true, true, true))
							{
								msg.Result = true;
							}
							else{
...
{
	LogInfoToFile("Control.txt", "OnHandleTCPNetworlEntity实体66666====== %d *****%d*****", pCmd->CmdType, pCmd->SubCmdType);
	if (!pClient || !pCmd)
		return false;
	CRoleEx* pRole = m_RoleManager.QuertUserBySocketID(pClient->OutsideExtraData);
	if (!pRole)
	...
	case CC_UserLeaveGame:
		{
			...
			//m_RoleManager.OnDelUser(pRole->GetUserID(), true, false);//离开玩家的集合 直接顶掉 无须离开中央服务器   需要修改玩家登出时需要保存玩家信息金币变化
			LogInfoToFile("LogGold.txt", "CC_UserLeaveGame离开房间  ::::::::%d:::::::::: %d:::::::", pRole->GetUserID(), pRole->GetScore());
			DelClient pDel;
			pDel.LogTime = timeGetTime();
			pDel.SocketID = pRole->GetGameSocketID();
...
bool FishServer::OnHandleSocketGiff(NetCmd* pCmd)
{
	...
	LogInfoToFile("Control.txt", "OnHandleSocketGiff赠送66666 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	//中央服务器发送来的命令
	if (!pCmd)
		return false;
	switch (pCmd->SubCmdType)
	...
	case CG_SendMessage:
		{
			...
			//LogInfoToFile("ControlSendMessageByType.txt", "SendMessageByType公告5555555:::::::%d::::%d::::::::: %s::::::::::::", pMsg->Param, pMsg->MessageType, pMsg->Message);
			SendMessageByType(pMsg->Message, pMsg->MessageSize, pMsg->MessageType, /*MessageType::MT_Game,*/ pMsg->MessageColor, pMsg->StepNum, pMsg->StepSec, pMsg->Param, true);
			return true;
		}
		break;
...
void FishServer::SendMessageByType(TCHAR* pMessage, WORD MessageSize, BYTE MessageType, DWORD MessageColor, BYTE StepNum, BYTE StepSec, DWORD Param, bool IsCenterMessage)
{
	...
	if (MessageType == MessageType::MT_Table || MessageType == MessageType::MT_TableMessage)
	{
		...
		LogInfoToFile("ControlSendMessageByType.txt", "SendMessageByType公告::::::: %d::::::::: %s::::::::::::", MessageType, msg->Message);
		free(msg);
	}
	else if (MessageType == MessageType::MT_Game || MessageType == MessageType::MT_GameMessage)
	{
		if (IsCenterMessage || m_GameNetworkID == ConvertDWORDToBYTE(Param))
		{
			...
			//LogInfoToFile("ControlSendMessageByType.txt", "SendMessageByType发送公告到客户端去:::::%d:::::::::%s::::", MessageType, (msg->Message), msg->StepNum, MessageSize);
			free(msg);
		}
		else
		{
			...
			LogInfoToFile("ControlSendMessageByType.txt", "SendMessageByType发送到中央去::::::: %d::::::::: %s::::::::::::", MessageType, msg->Message);
		}
	}
	else if (MessageType == MessageType::MT_Center || MessageType == MessageType::MT_CenterMessage)
	{
		...
		LogInfoToFile("ControlSendMessageByType.txt", "SendMessageByType发送到当前去::::%ld:::::%d:::%d:::::%ld:::%d;;;;::%s::::", Param, StepSec, StepNum, MessageColor, MessageType, msg->Message);
	}
	else if (MessageType == MessageType::MT_User || MessageType == MessageType::MT_UserMessage)
	{
		...
		if (pDestUser)
		{
			...
			//LogInfoToFile("ControlSendMessageByType.txt", "SendMessageByType发送到单个人去::::::: %d::::::::: %s::::::::::::", MessageType, msg->Message);
		}
		else
		{
			DWORD PageSize = sizeof(GC_Cmd_SendMessage)+sizeof(TCHAR)*(MessageSize + 1);
			...
			SendNetCmdToCenter(msg);
			free(msg);
			LogInfoToFile("ControlSendMessageByType.txt", "SendMessageByType发送到礼包去::::::: %d::::::::: %s::::::::::::", MessageType, msg->Message);
		}
	}
	else
	{
...
bool FishServer::OnHandleTCPNetworkRecharge(ServerClientData* pClient, NetCmd* pCmd)
{
	...
	//LogInfoToFile("Chongzhi.txt", TEXT("OnHandleTCPNetworkRecharge充值****************%d----------"), pCmd->SubCmdType);
	...
	case CL_Recharge:
		{
			LogInfoToFile("Chongzhi.txt", TEXT("OnHandleTCPNetworkRecharge充值普通类型"));
			//接收客户端发送来的命令 
			//1.桌子上的免费发送
			...
			if (!g_FishServerConfig.GetIsOperateTest())
			{
				...
				msg.IsPC = pMsg->IsPC;
				SendNetCmdToDB(&msg);
				//LogInfoToFile("Chongzhi.txt", TEXT("OnHandleTCPNetworkRecharge正常充值暂时记录数据"));
				return true;
			}
			else
			{
				...
				if (Iter == m_FishConfig.GetFishRechargesConfig().m_FishRechargeMap.end())
				{
					...
					msg.Result = false;
					pRole->SendDataToClient(&msg);
					//LogInfoToFile("Chongzhi.txt", TEXT("OnHandleTCPNetworkRecharge测试充值_______________________"));
					return true;
				}
				if (Iter->second.IsAddCurrcey()&& Iter->second.IsFirstAdd() && !pRole->GetRoleInfo().bIsFirstPayCurrcey)
					return false;
				LogInfoToFile("Chongzhi.txt", TEXT("测试需要进行区分"));
				if (Iter->second.IsAddGlobel() && Iter->second.IsFirstAdd() && !pRole->GetRoleInfo().bIsFirstPayGlobel)
					return false;
				LogInfoToFile("Chongzhi.txt", TEXT("OnHandleTCPNetworkRecharge测试充值38888888888888883"));

				//LogInfoToFile("ControlSendMessageByType.txt", "SendMessageByType公告66666 %d %s", MessageType, pMessage);
				...
				if (Iter->second.IsAddCurrcey())
				{
					...
					if (Iter->second.IsFirstAdd())
						pRole->ChangeRoleIsFirstPayCurrcey();
					LogInfoToFile("Chongzhi.txt", TEXT("OnHandleTCPNetworkRecharge测试充值获得钻石444"));
				...
				else if (Iter->second.IsAddGlobel())
				{
					...
					pRole->ChangeRoleGlobe(AddMoney, true, true,true);//充值的金币 立刻保存起来    这里是充值的第三参数就是是否保存数据库   金币变化
					if (Iter->second.IsFirstAdd())
						pRole->ChangeRoleIsFirstPayGlobel();
					LogInfoToFile("Chongzhi.txt", TEXT("OnHandleTCPNetworkRecharge测试充值获得充值的金币222222"));

				}
				else if (Iter->second.IsAddReward())
				{
					LogInfoToFile("Chongzhi.txt", TEXT("OnHandleTCPNetworkRecharge测试充值获得奖励"));
				...
				//LogInfoToFile("Chongzhi.txt", TEXT("OnHandleTCPNetworkRecharge添加玩家总充值数ttttttttttttttttt"));
				pRole->ChangeRoleTotalRechargeSum(Iter->second.dDisCountPrice);//添加玩家总充值数
				pRole->OnHandleEvent(true, true, true, ET_Recharge, 0, pRole->GetRoleInfo().TotalRechargeSum/*Iter->second.dDisCountPrice*/);//充值记录
				//LogInfoToFile("Chongzhi.txt", TEXT("OnHandleTCPNetworkRecharge添加玩家总充值数gggggggggggggggggg"));
				LC_Cmd_Recharge msg;
				SetMsgInfo(msg, GetMsgType(Main_Recharge, LC_Recharge), sizeof(LC_Cmd_Recharge));
				msg.ID = pMsg->ID;
				msg.Result = true;
				pRole->SendDataToClient(&msg);
				//LogInfoToFile("Chongzhi.txt", TEXT("OnHandleTCPNetworkRecharge添加玩家总充值数dddddddddddddddddddddddd"));
				return true;
			}
		}
		break;
...
bool FishServer::OnHandleTCpNetworkOperate(ServerClientData* pClient, NetCmd* pCmd)
{
	...
	LogInfoToFile("Control.txt", "OnHandleTCpNetworkOperate运营服务器处理命令66666 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pClient || !pCmd)
	{
		ASSERT(false);
		return false; 
...
{
	LogInfoToFile("Control.txt", "HandleOperateMsg将命令发送到客户端去///// %d******** %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pCmd)
	{
		ASSERT(false);
		return false;
...
bool FishServer::OnHandleGetRechargeOrderID(NetCmd* pCmd)
{
	...
	if (pMsg->OrderID != 0 && pMsg->IsPC)
	{
		...
		msg.OrderID = pMsg->OrderID;
		SendNetCmdToOperate(&msg);
		LogInfoToFile("Chongzhi.txt", TEXT("GO_AddNormalOrderID客户端玩家获取充值的结fa发送的开始*******%d*********%d***"), pRole->GetUserID(), pMsg->OrderID);
		return true;
	}
...
{
	LogInfoToFile("Control.txt", "KickUserByID踢出玩家44 %d %d", dwUserID, FreezeMin);
	if (FreezeMin == 0)
		return 0;
	//踢掉一个玩家 并且不管是否拥有缓存
	CRoleEx* pRole = m_RoleManager.QueryUser(dwUserID);
...
{
	LogInfoToFile("Control.txt", "HandleControlMsg处理运营服务器发送来的数据{{{{{{{{ %d|||||| %d", pCmd->CmdType, pCmd->SubCmdType);
	//处理运营服务器发送来的数据
	if (!pCmd)
		return false;
	switch (pCmd->CmdType)
...
{
	LogInfoToFile("Control.txt", "OnHandleTCPNetwordLottery抽奖44 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pClient || !pCmd)
	{
		ASSERT(false);
		return false;
...
{
	LogInfoToFile("ControlCar.txt", "OnHandleTCPNetworkCar奔驰宝马44 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pClient || !pCmd)
	{
		ASSERT(false);
		return false;
	...
	case CL_CarRoleJoinTable:
		{
			...
			////添加头像获取
			TCHARCopy(msgJoin.RoleInfo.szHeadHttp, CountArray(msgJoin.RoleInfo.szHeadHttp), pRole->GetRoleInfo().szHeadHttp, _tcslen(pRole->GetRoleInfo().szHeadHttp));
	...
	case CL_CarGetNormalSeatInfo:
		...
		break;
	case CL_CarRoleBetGlobelByLog://豪车 的金币变化
		{
...
{
	//LogInfoToFile("ControlDial.txt", "OnHandleTCPNetworkDial森林舞会44 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pClient || !pCmd)
	{
		ASSERT(false);
		return false;
	...
	case CL_DialGetNormalSeatInfo:
		...
		break;
	case CL_DialRoleBetGlobelByLog:   //森林的金币变化
		{
			...
			msg.Result = false;
			if (!pRole->ChangeRoleGlobe(AllGlobel*-1, true, false, false))//金币变化 第三参数就是是否保存数据库
			{
				pRole->SendDataToClient(&msg);
				return true;
			}
			...
			//
			CopyMemory(msg.betGlobel, pMsg->betGlobel, sizeof(pMsg->betGlobel));
			LogInfoToFile("ControlDial.txt", "CL_DialRoleBetGlobelByLog森林的金币变化44:::%d:::::: %d:::::%d:::::: %d::::%d:::::%d::::", pMsg->betGlobel[0], msg.betGlobel[0], pMsg->betGlobel[1], pMsg->betGlobel[2], pMsg->betGlobel[3], pMsg->betGlobel[4]);
			//msg.Result = true;
...
{
//	LogInfoToFile("ControlNiuNiu.txt", "OnHandleTCPNetworkNiuNiu牛牛44::::::::: %d::::::::::: %d:::::::::::::", pCmd->CmdType, pCmd->SubCmdType);
	if (!pClient || !pCmd)
	{
		ASSERT(false);
		return false;
...
{
	//LogInfoToFile("ControlCar.txt", "OnHandleTCPNetworkChar跑车44 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pCmd)
	{
		ASSERT(false);
		return false;
...
bool FishServer::HandleMiniGameMsg(NetCmd* pCmd)
{
	...
	//LogInfoToFile("Control.txt", "HandleMiniGameMsg处理MiniGame的数据::::::::::::::%d::::::::::::::", pCmd->CmdType);
	if (pCmd->CmdType == Main_MiniGame)
	{
		return true;
	}
...
{
	//LogInfoToFile("ControlCar.txt", "HandleCarMsg跑车消息555544 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pCmd)
	{
		ASSERT(false);
		return false;
		...
		case MG_CarRoleBetGlobelByLog:
			{
				...
				if (!pMsg->Result)
				{
					if (pRole)
					{
						pRole->ChangeRoleGlobe(pMsg->AllGlobel, true, false, false);//金币变化  第三参数就是数据库是否保存   
					}
					else
					{
						//归还玩家金币
...
{
//	LogInfoToFile("ControlDial.txt", "HandleDialMsg舞会消息88555544 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pCmd)
	{
		ASSERT(false);
		return false;
	...
	switch (pCmd->SubCmdType)
		...
		case MG_DialRoleBetGlobelByLog:  
			{
				...
				if (!pMsg->Result)
				{
					if (pRole)
					{
						pRole->ChangeRoleGlobe(pMsg->AllGlobel, true, false, false);//金币变化   第三参数就是保存数据库是否
					}
					else
					{
						//归还玩家金币
			...
			break;
			//////////////////////////////////////////////////////////////////////////
		case LC_AnimalAdminContrlBack://接受并转发mini游戏的消息
		{
									  ...
									  CRoleEx* pRole = GetRoleManager()->QueryUser(pMsg->dwUserIDContrl);
									  if (!pRole)
									  {
										  ASSERT(false);
										  return false;
									  }
									  LogInfoToFile("Control.txt", "LC_AnimalAdminContrlBack控制消息舞会0000000*::::::::::::::%d::::::::::%d::::", pMsg->dwUserIDContrl, pMsg->cbResult);
									  pRole->SendDataToClient(pMsg);

									  return true;
		}
			break;

		case LC_AnimalAdminQueryPlayerJettonContrl:
		{
												  ...
												  LogInfoToFile("Control.txt", "LC_AnimalAdminQueryPlayerJettonContrl控制消息舞会6DDDDDD*::::::::::::::%d::::::::::%d::::", pMsg->dwUserIDContrl, pMsg->dwUserID);
												  pRole->SendDataToClient(pMsg);
		...
		case LC_AnimalAdminQueryAllJettonContrl:
		{
											   ...
											   LogInfoToFile("Control.txt", "LC_AnimalAdminQueryAllJettonContrl控制消息舞会||||||||||*::::::::::::::%d::::::::::%d::::", pMsg->dwUserIDContrl, pMsg->nJetton);
											   pRole->SendDataToClient(pMsg);
											   return true;
		}
		...
		case LC_AnimalAdminQueryNameContrl:
		{
										  ...
										  LogInfoToFile("Control.txt", "LC_AnimalAdminQueryNameContrl控制消息舞会433242342424*::::::::::::::%d::::::::::::::", pMsg->dwUserIDContrl);
										  pRole->SendDataToClient(pMsg);
...
{
	//LogInfoToFile("ControlNiuNiu.txt", "HandleNiuNiuMsg牛牛消息555544 %d %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pCmd)
	{
		ASSERT(false);
		return false;
	...
	//LogInfoToFile("ControlNiuNiu.txt", "HandleNiuNiuMsg牛牛消息:::::::::%d:::::::::::: %d************", pCmd->CmdType, pCmd->SubCmdType);
	switch (pCmd->SubCmdType)
		{
		...
		case MG_GetNormalSeatInfo:
			...
			break;
		case LC_OxAdminContrlBack://接受并转发mini游戏的消息
		{
									  ...
									  LogInfoToFile("Control.txt", "LC_OxAdminContrlBack控制消息牛牛0000000*::::::::::::::%d::::::::::%d::::", pMsg->dwUserIDContrl, pMsg->cbResult);
									  pRole->SendDataToClient(pMsg);
		...
		case LC_OxAdminQueryPlayerJettonContrl:
		{
												  ...
												  LogInfoToFile("Control.txt", "LC_OxAdminQueryPlayerJettonContrl控制消息牛牛76DDDDDD*::::::::::::::%d::::::::::%d::::", pMsg->dwUserIDContrl, pMsg->dwUserID);
												  pRole->SendDataToClient(pMsg);
		...
		case LC_OxAdminQueryAllJettonContrl:
		{
											   ...
											   LogInfoToFile("Control.txt", "LC_OxAdminQueryAllJettonContrl控制消息牛牛||||||||||*::::::::::::::%d::::::::::%d::::", pMsg->dwUserIDContrl, pMsg->nJetton);
											   pRole->SendDataToClient(pMsg);
											   return true;
		}
		...
		case LC_OxAdminQueryNameContrl:
		{
										  LC_CMD_OxAdmin_PlayerNames * pMsg = (LC_CMD_OxAdmin_PlayerNames*)pCmd;
											   ...
											   LogInfoToFile("Control.txt", "LC_OxAdminQueryNameContrl控制消息牛牛433242342424*::::::::::::::%d::::::::::::::", pMsg->dwUserIDContrl);
											   pRole->SendDataToClient(pMsg);
...
{
	//LogInfoToFile("Control.txt", "GetUserMacAddress获取mac地址555544 %d, %d", dwUserID, dwUserID);
	return "";
	CRoleEx* pRole = GetRoleManager()->QueryUser(dwUserID);
	if (!pRole)
	{
...
{
//	LogInfoToFile("Control.txt", "GetUserIpAddress获取IP地址555544 %d, %d", dwUserID, dwUserID);
	CRoleEx* pRole = GetRoleManager()->QueryUser(dwUserID);
	if (!pRole)
	{
		return "";
...
{
	//LogInfoToFile("Control.txt", "OnHandleTCPNerwordRelationRequest获取qqqqqqqqqqqqqqqqqqqqq555544 %d, %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pCmd)
	{
		ASSERT(false);
		return false;
...
bool FishServer::OnHandleSocketRelationRequest(NetCmd* pCmd)
{
	...
	//LogInfoToFile("Control.txt", "OnHandleSocketRelationRequest兑换555544 %d, %d", pCmd->CmdType, pCmd->SubCmdType);
	if (!pCmd)
	{
		ASSERT(false);
		return false;
