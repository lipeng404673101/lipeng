...
bool DialManager::OnInit()
{
	...
	for (BYTE i = 0; i < MAX_DIAL_ClientSum; ++i)
	{
		...
		m_TableBetAreaGlobelAllUser[i] = 0;
	}
...
void DialManager::OnRoleJoinDialTable(DWORD dwUserID)
{
	...
	if (msg.TableBankerInfo.dwUserID != 0)
	{
		...
		if (!pBankerRole)
		{
			...
			msg.TableBankerInfo.szHeadHttp[0] = 0;
		}
		else
		{
			...
			//头像
			TCHARCopy(msg.TableBankerInfo.szHeadHttp, CountArray(msg.TableBankerInfo.szHeadHttp), pBankerRole->GetRoleInfo().szHeadHttp, _tcslen(pBankerRole->GetRoleInfo().szHeadHttp));
		}
	}
	for (BYTE i = 0; i < MAX_DIALSHOWBRAND_Sum; ++i)
	{
		...
		msg.TableWriteBankerList[i].szHeadHttp[0] = 0;
	}
	if (!m_TableWriteBankerVec.empty())
	{
		...
		for (BYTE i = 0; Iter != m_TableWriteBankerVec.end() && i < MAX_DIALSHOWBRAND_Sum;)
		{
			...
			//头像
			TCHARCopy(msg.TableWriteBankerList[i].szHeadHttp, CountArray(msg.TableWriteBankerList[i].szHeadHttp), pDestRole->GetRoleInfo().szHeadHttp, _tcslen(pDestRole->GetRoleInfo().szHeadHttp));
	...
	for (BYTE i = 0; i < MAX_DIALVIPSEAT_Sum; ++i)
	{
		if (m_VipSeatVec[i].dwUserID == 0)
		{
			...
			msg.VipSeatList[i].szHeadHttp[0] = 0;
		}
		else
		{
			...
			if (pDestRole)
			{
				...
				//头像
				TCHARCopy(msg.VipSeatList[i].szHeadHttp, CountArray(msg.VipSeatList[i].szHeadHttp), pDestRole->GetRoleInfo().szHeadHttp, _tcslen(pDestRole->GetRoleInfo().szHeadHttp));
			}
			else
			{
				msg.VipSeatList[i].dwUserID = 0;
...
void DialManager::OnBankerUserChange()
{
	...
	else
	{
		...
		if (!pBankerRole)
		{
			...
			msg.BankerUserInfo.szHeadHttp[0] = 0;
		}
		else
		{
			...
			//修改头像显示
			TCHARCopy(msg.BankerUserInfo.szHeadHttp, CountArray(msg.BankerUserInfo.szHeadHttp), pBankerRole->GetRoleInfo().szHeadHttp, _tcslen(pBankerRole->GetRoleInfo().szHeadHttp));
...
void DialManager::OnTableJoinEnd()
{
	...
	for (BYTE i = 0; i < MAX_DIAL_ClientSum; ++i)
	{
		TotalAllAddGlobel += m_TableBetAreaGlobel[i];//非机器人下注总和
		m_TableBetAreaGlobelAllUser[i] = 0;//初始化一下区域总量
	}
	//////////////////////////////////////////////////////////////////////////
	HashMap<DWORD, tagDialRoleInfo>::iterator IterUser = m_TableUserMap.begin();
	for (; IterUser != m_TableUserMap.end(); ++IterUser)
	{
		for (BYTE i = 0; i < MAX_DIAL_ClientSum; ++i)
		{
			if (IterUser->second.BetGlobel[i] == 0)
				continue;
			m_TableBetAreaGlobelAllUser[i] += (IterUser->second.BetGlobel[i] );//每个区域的输赢
		}
	}
//////////////////////////////////////////////////////////////////////////
	...
	if (g_FishServer.GetAnimalControl().NeedControl())	
	{
		...
		//////////////////////////////////////////////////////////////////////////
		UINT64 TotalOtherAddGlobel = 0;
		//添加判断条件 是否是庄家输赢
		DWORD  Style=g_FishServer.GetAnimalControl().GetContrlStyle();

		if (Style==1)//庄家输 
		{
				//////庄家输赢
				//BYTE	Num = RandUInt()%12;
				//LogInfoToFile("Control.txt", "OnTableJoinEnd赋值是怎么回事::::::%d::::::::::::::%d::::::::::::", Value, Num);
				//for (BYTE m = 0; m < 12; ++m)
				//{	
				//		for (BYTE n = 0; n < MAX_DIAL_ClientSum; ++n)
				//		{
				//			if (Num == n)
				//			{
				//				continue;
				//			}
				//			TotalOtherAddGlobel += m_TableBetAreaGlobelAllUser[n] ;
				//			LogInfoToFile("Control.txt", "OnTableJoinEnd看看这几个倍率是多少1::::::%d::::::::%d:::::%d:::::%d::::", m_TableBetAreaGlobelAllUser[n], m_TableBetAreaGlobelAllUser[Num], Value, Num);
				//		}
				//		LogInfoToFile("Control.txt", "OnTableJoinEnd什么情况::::::%d::::::::::::::%d::::::::::::", Value, Num);
				//		if (TotalOtherAddGlobel <= m_TableBetAreaGlobelAllUser[Num] * m_TableAreaRate[Num])//庄家输
				//		{
				//			Value = Num;
				//			LogInfoToFile("Control.txt", "OnTableJoinEnd庄家输赢::::::%d::::::::%d:::::%d:::::%d::::::::::", Value, m_TableBetAreaGlobelAllUser[Num], Value, Num);			
				//			TotalOtherAddGlobel = 0;
				//			break;				
				//		}
				//		else
				//		{
				...
				LogInfoToFile("Control.txt", "OnTableJoinEnd赋值是怎么回事::::::::::::::%d:::::::::::::::::", Value);
					...
					for (BYTE m = 0; m < 12; ++m)
					{
						...
						for (BYTE i = 0; i < MAX_DIAL_ClientSum; ++i)
						{
							...
							LogInfoToFile("Control.txt", "OnTableJoinEnd区域下注情况::::::%d::::::::::::::%d::::::%d::::::", Value, GlobelChange, m_TableBetAreaGlobel[i]);

						}
						//////////////////////////////////////////////////////////////////////////
						if (GlobelChange<0)//庄家输
						{
							Value = m;//赋值
							LogInfoToFile("Control.txt", "OnTableJoinEnd赋值是怎么回事::::::%d::::::::::::::%d::::::::::::", Value, m);
							break;
						}
					}
			//////////////////////////////////////////////////////////////////////////
		...
		for (BYTE j = 0; j < MAX_DIAL_GameSum; ++j)
		{
			if (m_TableAreaData[j] / 4 == Value / 4)//7/4=1
			{
				Index1 = j;
				LogInfoToFile("Control.txt", "OnTableJoinEnd内圈::::::%d::::::::%d::::::::::%d::::::%d:::", m_TableAreaData[j], Value, Index1, (m_TableAreaData[j] / 4));
				break;
			}
		}
		...
		for (BYTE j = 0; j < MAX_DIAL_GameSum; ++j)
		{
			if (g_FishServer.GetFishConfig().GetFishMiNiGameConfig().dialConfig.DialAreaData[j] % 4 == Value % 4)//7%4  =3
			{
				Index2 = j;
				LogInfoToFile("Control.txt", "OnTableJoinEnd外圈:::::::%d:::::::%d::::::::%d:::::::%d::",j, Value, Index2, (Value % 4));
				break;
			}
		}
		LogInfoToFile("Control.txt", "获取控制后的内外全数据:::::%d:::::::::%d::::::::::%d::::", Index1, Index2, m_TableResult);		
		m_TableResult = (Index1 << 8) | Index2;
		SystemGlobel = GetSystemGlobelChange();
		BrandUserGlobel = GetBrandUserGlobelChange();//庄家的输赢
		LogInfoToFile("Control.txt", "OnTableJoinEnd控制消息舞会:::::%d:::::::::%d::::::::::%d::::", Index1, Index2,m_TableResult);
...

	{
		...
		//LogInfoToFile("Control.txt", "普通的内外圈数据:::::%d:::::::::%d:::::::%d:::::::%d:", Index1, Index2, m_TableAreaData[Index1] / 4);
	...
	for (; Iter != m_TableUserMap.end(); ++Iter)
	{
		...
		if (m_TableBankerUserID != 0 && msg.dwUserID == m_TableBankerUserID)
		{
			...
			//添加抽水
			if (BrandUserGlobel >= 1000)
			{
				BrandUserGlobel = BrandUserGlobel - BrandUserGlobel * 1 / 1000;
			}
		...
		else
		{
			...
			for (BYTE i = 0; i < MAX_DIAL_ClientSum; ++i)
			{
				...
				ChoushuiGlobel += Iter->second.BetGlobel[i] * (Rate - 1);
...
void DialManager::OnRoleGetNormalInfoByPage(DWORD dwUserID, DWORD Page)
{
	...
	for (DWORD Index = 0; Iter != m_TableUserMap.end(); ++Iter)
	{
		...
		if (Index >= BeginIndex)
		{
			...
			//添加头像显示
			TCHARCopy(msg.Array[Index - BeginIndex].szHeadHttp, CountArray(msg.Array[Index - BeginIndex].szHeadHttp), pDestRole->GetRoleInfo().szHeadHttp, _tcslen(pDestRole->GetRoleInfo().szHeadHttp));
			if (Index == EndIndex)
			{
				pRole->SendDataToGameServer(&msg);
				return;
...
void DialManager::QueryNames(DWORD ClientID, DWORD	ContrlUserID)
{
	...
	pResult->SetCmdType((Main_Dial << 8) | LC_AnimalAdminQueryNameContrl);
	...
	Role* pRoleContrl = g_FishServer.GetRoleManager().QueryRole(ContrlUserID);
	if (!pRoleContrl)
	{
		return;
	}
	pRoleContrl->SendDataToGameServer(pResult);//修改
	//////////////////////////////////////////////////////////////////////////
	free(pResult);
}

void DialManager::QueryAllJetton(DWORD ClientID, DWORD	ContrlUserID)
{
	LC_CMD_AnimalAdmin_AllJetton result;
	ZeroMemory(&result, sizeof(result));
	result.SetCmdSize(sizeof(result));
	result.SetCmdType((Main_Dial << 8) | LC_AnimalAdminQueryAllJettonContrl);
	...
	//////////////////////////////////////////////////////////////////////////
	LogInfoToFile("Control.txt", "QueryAllJetton控制消息舞会444::::::::::::::%d::::::::::%d::::", ContrlUserID, m_SystemGlobelSum);
	Role* pRoleContrl = g_FishServer.GetRoleManager().QueryRole(ContrlUserID);
	if (!pRoleContrl)
	{
		LogInfoToFile("Control.txt", "QueryAllJetton控制消息舞会消息发哪里；了直接返回了？？为毛::::::::::::::%d::::::::::%d::::", pRoleContrl->GetRoleID(), m_SystemGlobelSum);
		return;
	}
	LogInfoToFile("Control.txt", "QueryAllJetton控制消息舞会消息发哪里；了::::::::::::::%d::::::::::%d::::", pRoleContrl->GetRoleID(), m_SystemGlobelSum);
	pRoleContrl->SendDataToGameServer(&result);//修改
	//////////////////////////////////////////////////////////////////////////

}

void DialManager::QueryPlayerJetton(DWORD dwUserID, DWORD ClientID, DWORD	ContrlUserID)
{
	for (HashMap<DWORD, tagDialRoleInfo>::iterator it = m_TableUserMap.begin(); it != m_TableUserMap.end(); it++)
	{
		Role* pRole = g_FishServer.GetRoleManager().QueryRole(it->first);
		if (pRole)
		{
			...
			//{
				...
				//if (itJetton != m_TableUserMap.end())
				{
					...
					result.SetCmdSize(sizeof(result));
					result.SetCmdType((Main_Dial << 8) | LC_AnimalAdminQueryPlayerJettonContrl);
					result.ClientID = ClientID;
