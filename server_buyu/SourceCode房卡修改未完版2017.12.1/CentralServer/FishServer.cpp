...
bool FishServer::HandleGameServerMsg(ServerClientData* pClient, NetCmd* pCmd)
{
	...
	else if (pCmd->CmdType == Main_Role)
	{
		...
		case CC_ChangeRoleVipLevel://vip
			{
	...
	else if (pCmd->CmdType == Main_Operate)
	{
		...
		case OC_UseRMB:
			{
				...
				if (pRole)
				{
					...
					memcpy_s(&msg->rechargeInfo, (pMsg->rechargeInfo.Sum - 1)*sizeof(BYTE)+sizeof(RechageInfo), &pMsg->rechargeInfo, (pMsg->rechargeInfo.Sum - 1)*sizeof(BYTE)+sizeof(RechageInfo));
						...
						LogInfoToFile("Chongzhi.txt", TEXT("购买成功中心服务器玩家在线********%d**********%d************%d******************"), pMsg->rechargeInfo.Price, pMsg->rechargeInfo.UserID, pMsg->rechargeInfo.ShopItemID);
		...
		case OC_AddNormalOrderID:
			{
				 LogInfoToFile("Chongzhi.txt", TEXT("OC_AddNormalOrderID普通充值订单***111***"));
				...
				LogInfoToFile("Chongzhi.txt", TEXT("OC_AddNormalOrderID普通充值订单***222***"));
				CenterRole* pRole = GetRoleManager().QueryCenterUser(pMsg->dwUserID);
				if (!pRole)
				{
					ASSERT(false);
				...
				LogInfoToFile("Chongzhi.txt", TEXT("OC_AddNormalOrderID普通充值订单验证成功准备写入数据库***333***"));
				...
				pRole->SendDataToGameServer(&msg);
				LogInfoToFile("Chongzhi.txt", TEXT("OC_AddNormalOrderID普通充值订单发送到数据库***4444***"));
				return true;
			}
			break;
		}
...
void FishServer::SendMessageByType(TCHAR* pMessage, WORD MessageSize,BYTE MessageType, DWORD MessageColor, BYTE StepNum, BYTE StepSec, DWORD Param)
{
	if (MessageType == MT_Center || MessageType == MT_CenterMessage)
	{
		...
		//LogInfoToFile("ControlSendMessageByType.txt", "SendMessageByType公告0000000000:::::::%d::::%d::::::::: %s::::::::::::", MessageSize, MessageType, msg->Message);
		free(msg);
	}
	...
	//	LogInfoToFile("ControlSendMessageByType.txt", "SendMessageByType公告11111111:::::::%d:::::::%d::::::::: %s::::::::::::", MessageSize, MessageType, msg->Message);
		free(msg);
	}
	else if (MessageType == MT_User || MessageType == MT_UserMessage)
	{
		...
		pRole->SendDataToGameServer(msg);
		//LogInfoToFile("ControlSendMessageByType.txt", "SendMessageByType公告222222222:::::::%d::::::%d::::::::: %s::::::::::::", MessageSize, MessageType, msg->Message);
		free(msg);
	}
	else
	{
	...
	case CL_SendMsgToAllGame:
		{
			...
			LogInfoToFile("ControlSendMessageByType.txt", "SendMessageByType公告//////////:::::::%d::::%d::::::::: %s::::::::::::", pMsg->Param, pMsg->MessageSize, pMsg->MessageInfo);
			if (!pMsg)
			{
				ASSERT(false);
				return false;
			...
			LogInfoToFile("ControlSendMessageByType.txt", "SendMessageByType公告9999999999999999:::::::%d::::%d::::::::: %s::::::::::::", pMsg->Param, pMsg->MessageSize, pMsg->MessageInfo);
			SendMessageByType(pMsg->MessageInfo, _tcslen(pMsg->MessageInfo), MT_Center, pMsg->MessageColor, pMsg->StepNum, pMsg->StepSec, pMsg->Param);
