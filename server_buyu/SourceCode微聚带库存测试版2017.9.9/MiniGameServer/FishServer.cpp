...
void FishServer::HandleClientMsg(ServerClientData* pClient, NetCmd* pCmd)
{
	...
	//LogInfoToFile("Control.txt", "HandleNiuNiuMsg控制消息牛牛控制消息消息到哪里了8888888888888::::::::::::::%d::::::::::%d::::", pCmd->CmdType, pCmd->SubCmdType);
...
// 添加控制返回回调
void FishServer::HandleNiuNiuContrlBack(NetCmd* pCmd)
{
	...
	LogInfoToFile("Control.txt", "HandleNiuNiuMsg控制消息牛牛控制消息消息到哪里了7777777777::::::::::::::%d::::::::::%d::::", pCmd->CmdType, pCmd->SubCmdType);
	switch (pCmd->SubCmdType)
	{


			case LC_OxAdminContrlBack:
				{
					   ...
					   Role* pRole = m_RoleManager.QueryRole(pMsg->dwUserIDContrl);
					   if (!pRole)
					   {
						   //ASSERT(false);
						   return;
					   ...
					   pRole->SendDataToGameServer(pCmd);
...
//
// 添加控制返回回调
void FishServer::HandleDailContrlBack(NetCmd* pCmd)
{
	...
	LogInfoToFile("Control.txt", "HandleDailContrlBack控制消息舞会的消息7777777777::::::::::::::%d::::::::::%d::::", pCmd->CmdType, pCmd->SubCmdType);
	switch (pCmd->SubCmdType)
	{
		
				case LC_AnimalAdminContrlBack:
				{
...
void FishServer::HandleCarMsg(NetCmd* pCmd)
{
	...
	//LogInfoToFile("Control.txt", "HandleNiuNiuMsg控制消息牛牛控制消息消息到哪里了6666666666666::::::::::::::%d::::::::::%d::::", pCmd->CmdType, pCmd->SubCmdType);
	switch (pCmd->SubCmdType)
		{
...
void FishServer::HandleDialMsg(NetCmd* pCmd)
{
	...
	//LogInfoToFile("Control.txt", "HandleNiuNiuMsg控制消息牛牛控制消息消息到哪里了555555555555555::::::::::::::%d::::::::::%d::::", pCmd->CmdType, pCmd->SubCmdType);
	switch (pCmd->SubCmdType)
		{
...
void FishServer::HandleNiuNiuMsg(NetCmd* pCmd)
{
	...
	//LogInfoToFile("Control.txt", "HandleNiuNiuMsg控制消息牛牛控制消息消息到哪里了::::::::::::::%d::::::::::%d::::", pCmd->CmdType, pCmd->SubCmdType);
	if (pCmd->CmdType != Main_NiuNiu)
	{
		ASSERT(false);
		return;
		...
		case DBO_LoadRobotInfo:
			{
				...
				if (pMsg->States & MsgEnd)
				{
					...
					for (; Iter != pVec.end(); ++Iter)
					{
						...
						////添加头像
						TCHARCopy(pInfo.szHeadHttp, CountArray(pInfo.szHeadHttp), (*Iter)->szHeadHttp, _tcslen((*Iter)->szHeadHttp));
...
void FishServer::HandleAllMsg()
{
	...
	//LogInfoToFile("Control.txt", "HandleNiuNiuMsg控制消息牛牛控制消息消息到哪里了44444444444444444444444444::::::::::::::%d::::::::::%d::::", pCmd->CmdType, pCmd->SubCmdType);
}
...
{
	//LogInfoToFile("Control.txt", "HandleNiuNiuMsg控制消息牛牛控制消息消息到哪里了111111111111111111111111::::::::::::::%d::::::::::%d::::", pCmd->CmdType, pCmd->SubCmdType);
	if (!pCmd)
	{
		ASSERT(false);
		return;
	}
	LogInfoToFile("Control.txt", "HandleControlMsg控制消息牛牛控制接受消息的位置::::%d::::::::::%d:::::::::::::::::::::::::::::::::::", pCmd->CmdType, pCmd->SubCmdType);
	//m_Oxcontrol.Control(pCmd);
	switch (pCmd->CmdType)
	{
	case Main_Control:
...
void FishServer::HandleCenterMsg(NetCmd* pCmd)
{
	...
	//LogInfoToFile("Control.txt", "HandleNiuNiuMsg控制消息牛牛控制消息消息到哪里了222222222222222::::::::::::::%d::::::::::%d::::", pCmd->CmdType, pCmd->SubCmdType);
...
void FishServer::UpdateInfoToControl(DWORD dwTimer)
{
	...
	{
	//	LogInfoToFile("Control.txt", "HandleNiuNiuMsg控制消息牛牛控制消息消息到哪里了333333333333333::::::::::::::%d::::::::::%d::::");
