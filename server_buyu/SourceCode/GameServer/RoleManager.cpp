...
void  RoleManager::OnDelUser(DWORD dwUserID, bool IsWriteSaveInfo, bool IsLeaveCenter)
{
	...
	if (Iter != m_RoleMap.end())
	{
		...
		if (IsWriteSaveInfo)
		{
			if (Iter->second->SaveAllRoleInfo(true))//金币变化
			{
				//需要保持 并且 需要等待
				return;
			}
...
void RoleManager::OnDelUserResult(DBO_Cmd_SaveRoleAllInfo* pResult)
{
	...
	if (Iter != m_RoleMap.end())
	{
		...
		{
		//	LogInfoToFile("RoleManager.txt", "OnDelUserResult玩家在退出::::::: %d,", pResult->dwUserID);
		...
		else
		{
			...
			//LogInfoToFile("RoleManager.txt", "OnDelUserResult玩家查询上线了::::::::::: %d,", pResult->dwUserID);
			return;
		}
	}
	else
...
{
	//LogInfoToFile("RoleManager.txt", "OnKickAllUser踢下线所有玩家555544,"/*, pResult->dwUserID*/);
	//直接踢掉所有的玩家 
	HashMap<DWORD, CRoleEx*>::iterator Iter = m_RoleMap.begin();
	for (; Iter != m_RoleMap.end(); ++Iter)
	{
	...
	//	Iter->second->SaveAllRoleInfo(true);//保存下玩家的数据
...
//	LogInfoToFile("CollideBuYu.txt", "CreateRole玩家对象初始化完毕后{{{{,::::::%d,:::::::金币:%d", pUserInfo->dwUserID, pUserInfo->dwGlobeNum);
	...
	else
	{
		...
		//玩家对象初始化完毕后 我们存储起来  第一次登陆加入到链表里面  玩家信息就更新了  金币是从数据库更新的
		...
		{
			OnDelUser(pUserInfo->dwUserID,false,false);//不需要删除中央服务器  玩家新加失败就删除玩家的缓存
			delete pUser;
			return false;
		}
		return true;
