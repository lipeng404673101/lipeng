...
bool FishServer::HandleDataBaseMsg(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	case DBR_AccountHeadHttp://微信头像
		return OnHandleAccountHeadHttp(Index, ClientID, pCmd);
	...
	case DBR_SaveRoleGlobelDown://添加减少金币
		return	OnHandleSaveRoleGlobelDown(Index, ClientID, pCmd);
	case DBR_CheckUserIsDaili://代理
		return  OnHandleCheckUserDaili(Index, ClientID, pCmd);
	
	case	DBR_CheckUserIsContrlDaili:// 检测赠送
		return OnHandleCheckUserContrlDaili(Index, ClientID, pCmd);

		//////////////////////////////////////////////////////////////////////////

	case	DBR_CheckUserDailiBond:// 检测玩家绑定代理
		return OnHandleCheckDailiBond(Index, ClientID, pCmd);

	case	DBR_UserDailiBond:// 玩家绑定代理
		return OnHandleDailiBond(Index, ClientID, pCmd);

	case	DBR_QueryPrivateRoom:// 查询房间号   房卡模式
		return OnHandleCreatePrivateRoom(Index, ClientID, pCmd);

	case	DBR_DeletePrivateRoom:// 删除房间   房卡模式
		return OnHandleDeletePrivateRoom(Index, ClientID, pCmd);
		//////////////////////////////////////////////////////////////////////////
...
bool FishServer::OnHandleAccountLogon(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//LogInfoToFile("DBShujuKu.txt", "OnHandleAccountLogon根据参数的类型执行存储过程:::::%s:::::::%s*********%s*****", DestAccountName, DestMacAddress, AccountName);
...
bool FishServer::OnHandleQueryLogon(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleQueryLogon接收到快速登陆wwwwwwwwwwwww:::::%s:::::::%s*********%s*****", AccountName, DestMacAddress, MacAddress);
	//用户名称
	...
	//添加绑定邀请码  代理  添加绑定参数  id

	sprintf_s(SqlStr, sizeof(SqlStr), "call FishQueryLogon('%s','%s','%u','%u','%u','%s',%d,'%u','%s','%d.%d.%d.%d','%d-%d-%d %d:%d:%d','%u');", DestAccountName, DestMacAddress, pMsg->PasswordCrc1, pMsg->PasswordCrc2, pMsg->PasswordCrc3, strNickName.c_str(), 1, m_Config.GetSystemConfig().RsgInitGlobelSum, pRateStates, pMsg->ClientIP & 0xff, (pMsg->ClientIP >> 8) & 0xff, (pMsg->ClientIP >> 16) & 0xff, (pMsg->ClientIP >> 24) & 0xff, NowTime.tm_year + 1900, NowTime.tm_mon + 1, NowTime.tm_mday, NowTime.tm_hour, NowTime.tm_min, NowTime.tm_sec,0);//最后参数默认给0  此处修改在绑定代理逻辑
	free(DestAccountName);
	free(AccountName);
	free(DestMacAddress);
	free(MacAddress);
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleQueryLogon接收到快速登陆:::::%s:::::::%s*********%s*****", strNickName.c_str(), DestMacAddress, AccountName);
	...
	if (m_Sql[Index].Select(SqlStr, 0, pTable1, true) && pTable1.Rows() == 1)
	{
		...
		TCHARCopy(msg->NickName, CountArray(msg->NickName), pMsg->NickName, _tcslen(pMsg->NickName));//修改头像  xuyao 需要客户端传入头像
	...
	if (msg->dwUserID!=0)
	{
		...
		char SqlStr[MAXSQL_LENGTH] = { 0 };
		UINT Size = 0;
		char* NickName = WCharToChar(pMsg->szHeadHttp, Size);
		...
		sprintf_s(SqlStr, sizeof(SqlStr), "call FishUpdateHardhttp('%u','%s');", msg->dwUserID, DestNickName);
		free(DestNickName);
		free(NickName);
		bool Result = (m_Sql[Index].Select(SqlStr, 0, pTable122, true) && pTable122.Rows() == 1 /*&& pTable1.GetUint(0, 0) == 1*/);
		if (!Result)
		{
			ASSERT(false);
			LogInfoToFile(DBErrorSqlFileName, SqlStr);
		...
		LogInfoToFile("DBShujuKu.txt", "OnHandleQueryLogon接收到快速登陆修改头像是否成功:::::%s:::::::%s*********%s*****", strNickName.c_str(), DestMacAddress, AccountName);
		...
		//	char SqlStr[MAXSQL_LENGTH] = { 0 };
		//	UINT Size = 0;
		//	char* NickName33 = WCharToChar(pMsg->NickName, Size);
		//	char* DestNickName33 = (char*)malloc((Size * 2 + 1) * sizeof(char));
		//	if (!DestNickName33)
		//	{
		//		free(msg);
		//		ASSERT(false);
		//		return false;
		//	}
		//	ZeroMemory(DestNickName33, (Size * 2 + 1) * sizeof(char));
		//	m_Sql[Index].GetMySqlEscapeString(NickName33, Size, DestNickName33);

		//	sprintf_s(SqlStr, sizeof(SqlStr), "call FishSaveRoleNickName('%u','%s');", msg->dwUserID, DestNickName33);
		//	free(DestNickName33);
		//	free(NickName33);

		//	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleNickName保存玩家昵称:::::%s:::::::%d*********%d*****", pMsg->NickName, msg->dwUserID, ClientID);

		//	bool Result = (m_Sql[Index].Select(SqlStr, 0, pTable133, true) && pTable133.Rows() == 1 && pTable133.GetUint(0, 0) == 1);
		//	if (!Result)
		//	{
		//		ASSERT(false);
		//		LogInfoToFile(DBErrorSqlFileName, SqlStr);
...
bool FishServer::OnHandleAccountRsg(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//////////////////////////////////////////////////////////////////////////绑定代理   修改添加微信此处不校验代理
	SqlTable pTable133;
	char SqlStr133[MAXSQL_LENGTH] = { 0 };
	//sprintf_s(SqlStr133, sizeof(SqlStr133), "call FishAccountRsgCode('%u');", pMsg->dwUserID);
	//bool Result = (m_Sql[Index].Select(SqlStr133, 0, pTable133, true) && pTable133.Rows() == 1 && pTable133.GetUint(0, 0) == 1);
	//LogInfoToFile("DBShujuKu.txt", "OnHandleQueryLogon注册存储4444444444444++++:::::%d:::::::%d*******%u******%u*****", Result, pTable133.Rows(), pMsg->dwUserID, pTable133.GetUint(0, 0));
	...
	int256 RateValue;
	int256Handle::Clear(RateValue);
	DWORD InitRateValue = m_Config.GetSystemConfig().InitRateValue;//炮台大小

		//	g_FishServer.GetTableManager()->GetGameConfig()->RateCount()
	...
	//InitRateValue = 500;
	//for (BYTE i = 0; i < 500; ++i)//玩家的默认倍率 只可以是前32个  500
	//{
	//	//if ((InitRateValue& (1 << i)) != 0)
	//	//{
	//	//	int256Handle::SetBitStates(RateValue, i, true);
	...
	m_Sql[Index].GetMySqlEscapeString((char*)&RateValue, sizeof(RateValue), pRateStates);
	//注册存储  王者添加新参数  绑定代理
	sprintf_s(SqlStr, sizeof(SqlStr), "call FishAccountRsg('%s','%s','%u','%u','%u','%s',%d,'%u','%s','%d.%d.%d.%d','%d-%d-%d %d:%d:%d','%u');", DestAccountName, DestMacAddress, pMsg->PasswordCrc1, pMsg->PasswordCrc2, pMsg->PasswordCrc3, strNickName.c_str(), 1, m_Config.GetSystemConfig().RsgInitGlobelSum, pRateStates, pMsg->ClientIP & 0xff, (pMsg->ClientIP >> 8) & 0xff, (pMsg->ClientIP >> 16) & 0xff, (pMsg->ClientIP >> 24) & 0xff, NowTime.tm_year + 1900, NowTime.tm_mon + 1, NowTime.tm_mday, NowTime.tm_hour, NowTime.tm_min, NowTime.tm_sec, pMsg->dwUserID);
...
///
	...
	////0 注册失败  1绑定失败   其它值为成功   绑定代理
	//if (m_Sql[Index].Select(SqlStr, 0, pTable1, true) && pTable1.Rows() == 0)
	//{
	...
	//	LogInfoToFile(DBErrorSqlFileName, SqlStr);
	//	ASSERT(false);
	//	LogInfoToFile("DBShujuKu.txt", "OnHandleQueryLogon注册存储失败:::::%s:::::::%s*********%s*****", strNickName.c_str(), DestMacAddress, AccountName);
	//	//////////////////////////////////////////////////////////////////////////
	//	
	//
	//}
	//else if (m_Sql[Index].Select(SqlStr, 0, pTable1, true) && pTable1.Rows() == 1)
	//{
	//	msg->dwUserID = 0;
	//	//////////////////////////////////////////////////////////////////////////
	//	msg->dwBangDing = 0;
	...
	//	LogInfoToFile(DBErrorSqlFileName, SqlStr);
	//	ASSERT(false);
	//	LogInfoToFile("DBShujuKu.txt", "OnHandleQueryLogon注册存储绑定失败:::::%s:::::::%s*********%s*****", strNickName.c_str(), DestMacAddress, AccountName);
	//}
	//else  if (m_Sql[Index].Select(SqlStr, 0, pTable1, true) && pTable1.Rows() != 1 && pTable1.Rows() != 0)
	//{
	//	msg->dwBangDing = 1;

	//	msg->dwUserID = pTable1.GetUint(0, 0);
	//	LogInfoToFile("DBShujuKu.txt", "OnHandleQueryLogon注册存储成功:::::%s:::::::%s*********%s****%d*****", strNickName.c_str(), DestMacAddress, AccountName, msg->dwUserID);
	//}
	...
	if (!Result)
	{
		...
		msg->dwBangDing = 0;
		LogInfoToFile("DBShujuKu.txt", "OnHandleQueryLogon注册存储绑定失败:::::%s:::::::%s*********%s*****", strNickName.c_str(), DestMacAddress, AccountName);
		LogInfoToFile(DBErrorSqlFileName, SqlStr133);
		ASSERT(false);
	}
	else
	{
		...
		else
		{
			...
			LogInfoToFile(DBErrorSqlFileName, SqlStr);
			ASSERT(false);
			LogInfoToFile("DBShujuKu.txt", "OnHandleQueryLogon注册shi失败:::::%s:::::::%s*********%s*****", strNickName.c_str(), DestMacAddress, AccountName);
		}
	}


	//if (m_Sql[Index].Select(SqlStr, 0, pTable1, true) && pTable1.Rows() == 1 && Result)
	//{
...
//微信头像   存储
bool FishServer::OnHandleAccountHeadHttp(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{

	DBR_Cmd_AccountHeadHttp* pMsg = (DBR_Cmd_AccountHeadHttp*)pCmd;
	if (!pMsg)
	{
		return false;
	}
	...
	sprintf_s(SqlStr, sizeof(SqlStr), "call FishUpdateHardhttp('%u','%s');", pMsg->dwUserID, DestNickName);
	free(DestNickName);
	free(NickName);
	...
	LogInfoToFile("LogonError.txt", TEXT("OnHandleAccountHeadHttp 微信头像存储  :%u,:::%s,:::::::%d"), pMsg->dwUserID, pMsg->szHeadHttp, Result);
	return true;
}
...
bool FishServer::OnHandleAccountReset(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleAccountReset快速账号转化为普通账号:::::%d:::::::%s*********%s*****", pMsg->dwUserID, DestAccountName, AccountName);
...
bool FishServer::OnHandleChangeAccountPassword(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleChangeAccountPassword修改当前账号的密码:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->PasswordCrc1, pMsg->PasswordCrc2);
...
bool FishServer::OnHandleChangeAccountOnline(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//LogInfoToFile("DBShujuKu.txt", "OnHandleChangeAccountOnline设置账号上线或者下线状态:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->IsOnline);
	return true;
}
...
bool FishServer::OnHandleRoleClientInfo(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//LogInfoToFile("DBShujuKu.txt", "OnHandleRoleClientInfo保存玩家数据统计:::::%d:::::::%d*********%d*****", pMsg->dwUserID, ClientID,0);
	return true;
}
...
bool FishServer::OnHandleSetAccountFreeze(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSetAccountFreeze保存玩家数据统计:::::%d:::::::%d*********%d*****", pMsg->dwUserID, ClientID, 0);
	return true;
}
bool FishServer::OnHandlePhoneSecPwdLogon(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandlePhoneSecPwdLogon二级密码登陆:::::%d:::::::%d*********%d*****", pMsg->PhoneNumber, ClientID, msg->dwUserID);
	//发送命令
	OnAddDBResult(Index, ClientID, msg);
	return true;
}
bool FishServer::OnHandlePhoneLogon(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandlePhoneLogon玩家试图进行手机登陆:::::%d:::::::%d*********%d*****", pMsg->PhoneNumber, ClientID, Mac);
...
bool FishServer::OnHandleGetNewAccount(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleGetNewAccount生成一个账号名称:::::%s:::::::%d*********%s*****", strAccountName.c_str(), ClientID, strNickName.c_str());
...
bool FishServer::OnHandleGetRoleInfoByUserID(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	if (m_Sql[Index].Select(SqlStr, 0, pTable1, true) && pTable1.Rows() == 1)
	{
		...
		//////////////////////////////////////////////////////////////////////////
		const TCHAR* szHeadHttp = pTable1.GetStr(0, 54);
		TCHARCopy(msg->RoleInfo.szHeadHttp, CountArray(msg->RoleInfo.szHeadHttp), szHeadHttp, _tcslen(szHeadHttp));
		...
		LogInfoToFile("CollideBuYu.txt", "OnHandleGetRoleInfoByUserID获取玩家信息数据库读取:::::%d:::::::%d*********%s*****", msg->RoleInfo.dwGlobeNum, pMsg->dwUserID, msg->RoleInfo.szHeadHttp);
...
bool FishServer::OnHandleSaveRoleInfo(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("LogGold.txt", "将玩家的数据保存起来金币::::::::::::::::: %d:::::::::::: %d---------", pMsg->RoleInfo.dwGlobeNum, pMsg->RoleInfo.dwUserID);
	//设置存储过程的参赛
	SqlTable pTable1;
...
bool FishServer::OnHandleSaveRoleNickName(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleNickName保存玩家昵称1111:::::%s:::::::%d*********%d*****", pMsg->NickName, pMsg->dwUserID, ClientID);
	SqlTable pTable1;
	char SqlStr[MAXSQL_LENGTH] = { 0 };
	UINT Size = 0;
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleNickName保存玩家昵称:::::%s:::::::%d*********%d*****", pMsg->NickName, pMsg->dwUserID, ClientID);
...
bool FishServer::OnHandleSaveRoleLevel(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleLevel保存玩家等级:::::%d:::::::%d*********%d*****", pMsg->wLevel, pMsg->dwUserID, ClientID);
	return true;
}
bool FishServer::OnHandleSaveRoleGender(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleGender保存玩家性别:::::%d:::::::%d*********%d*****", pMsg->bGender, pMsg->dwUserID, ClientID);
...
bool FishServer::OnHandleSaveRoleCurTitle(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleCurTitle保存头:::::%d:::::::%d*********%d*****", pMsg->TitleID, pMsg->dwUserID, ClientID);
...
bool FishServer::OnHandleSaveRoleCharmArray(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleCharmArray保存333333333333:::::%d:::::::%d*********%d*****", CharmValue, pMsg->dwUserID, ClientID);
	return true;
}
bool FishServer::OnHandleSaveRoleCurency(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleCurency保存玩家钻石:::::%d:::::::%d*********%d*****", pMsg->dwCurrencyNum, pMsg->dwUserID, ClientID);
	return true;
}
bool FishServer::OnHandleSaveRoleFaceID(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleFaceID保存玩家头像:::::%d:::::::%d*********%d*****", pMsg->dwFaceID, pMsg->dwUserID, ClientID);
	return true;
}

//////////////////////////////////////////////////////////////////////////

// 检测                   绑定代理  玩家绑定   添加检测玩家绑定代理存储
bool FishServer::OnHandleCheckDailiBond(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{

	DBR_Cmd_CheckUserDailiBond  * pMsg = (DBR_Cmd_CheckUserDailiBond*)pCmd;

	DWORD	OtherUserID = 0;
	SqlTable pTable1;
	char SqlStr[MAXSQL_LENGTH] = { 0 };
	sprintf_s(SqlStr, sizeof(SqlStr), "call FishAccountRsgCodeExit('%u');", pMsg->dwUserID);
	//if (m_Sql[Index].Select(SqlStr, 0, pTable1, true) && pTable1.Rows() == 1)
	//{
	//	OtherUserID = pTable1.GetUint(0, 0);
	//}
	//else
	//{
	//	OtherUserID = 0;
...
//////////////////////////////////////////////////////////////////////////
	bool Result = (m_Sql[Index].Select(SqlStr, 0, pTable1, true) && pTable1.Rows() == 1 && pTable1.GetUint(0, 0) == 1);//返回值 为 1
	LogInfoToFile("DBShujuKu.txt", "OnHandleCheckDailiBond绑定代理  玩家绑定   添加检测玩家绑定代理存储:::::%d:::::::%d*********%d*****", Result, pMsg->dwUserID, pTable1.Rows());

	DBO_Cmd_CheckUserDailiBond* msg = (DBO_Cmd_CheckUserDailiBond*)CreateCmd(DBO_CheckDailiBond, sizeof(DBO_Cmd_CheckUserDailiBond));
	msg->dwUserID = pMsg->dwUserID;
	msg->BackeNum = Result;
	OnAddDBResult(Index, ClientID, msg);
	//////////////////////////////////////////////////////////////////////////OnHandleCheckUserDailiBond

	return  true;
}

//绑定代理  玩家绑定   添加检测玩家绑定代理存储  
bool FishServer::OnHandleDailiBond(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{

	DBR_Cmd_UserDailiBond  * pMsg = (DBR_Cmd_UserDailiBond*)pCmd;

	DWORD	OtherUserID = 0;
	SqlTable pTable1;
	char SqlStr[MAXSQL_LENGTH] = { 0 };
	sprintf_s(SqlStr, sizeof(SqlStr), "call FishAccountRsgCode('%u','%u');", pMsg->dwUserID, pMsg->dwOtherUserID);
	//if (m_Sql[Index].Select(SqlStr, 0, pTable1, true) && pTable1.Rows() == 1)
	//{
	//	OtherUserID = pTable1.GetUint(0, 0);
	//}
	...
	//////////////////////////////////////////////////////////////////////////
	bool Result = (m_Sql[Index].Select(SqlStr, 0, pTable1, true) && pTable1.Rows() == 1 && pTable1.GetUint(0, 0) == 1);//返回值 为 1
	DBO_Cmd_UserDailiBondResult* msg = (DBO_Cmd_UserDailiBondResult*)CreateCmd(DBO_DailiBondResult, sizeof(DBO_Cmd_UserDailiBondResult));
	msg->dwUserID = pMsg->dwUserID;
	msg->BackeNum = Result;
	OnAddDBResult(Index, ClientID, msg);
	//////////////////////////////////////////////////////////////////////////
...
//删除房间  房间号   房卡模式
bool FishServer::OnHandleDeletePrivateRoom(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	
	DBR_GR_DeletePrivateRoom  * pMsg = (DBR_GR_DeletePrivateRoom*)pCmd;
	LogInfoToFile("DBShujuKu.txt", "OnHandleDeletePrivateRoom删除房间:::::%d:::::::%d****%d*********", pMsg->szPrivateTableID, pMsg->dwUserID, ClientID);

	SqlTable pTable1;
	char SqlStr[MAXSQL_LENGTH] = { 0 };
	sprintf_s(SqlStr, sizeof(SqlStr), "call FishDeletePrivateRoom('%u','%u');", pMsg->dwUserID, pMsg->szPrivateTableID);
...
//添加申请创建房间  房间号   房卡模式
bool FishServer::OnHandleCreatePrivateRoom(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	DBR_Cmd_QueryPrivateRoom  * pMsg = (DBR_Cmd_QueryPrivateRoom*)pCmd; //FishCreatePrivateRoom


	//--ServerID       服务器ID  2017   
	//	--  EndScore       底分
	//	--  GameNumber     局数  //现在用作时间  10分20  分
	//	--  KindID         类型标识  2013
	//	--  UserID         创建者ID
	//	--  FeeCount       消耗费用
	//	--  FeeType        货币类型
	//	--  CreateRoom    是否创建房间
	//	--  strErrorDescribe  输出信息

	LogInfoToFile("DBShujuKu.txt", "OnHandleCreatePrivateRoom添加申请创建房间:::::%d:::::::%d****%d*****%d*****", pMsg->bTableTypeID, pMsg->dwUserID, ClientID, pMsg->cbCreateRoom);

	//返回结果
	DBO_GR_CreatePrivateRoomResult* msg = (DBO_GR_CreatePrivateRoomResult*)CreateCmd(DBO_QueryPrivateRoomResult, sizeof(DBO_GR_CreatePrivateRoomResult));

	SqlTable pTable1;

	TCHAR szDescribeString[128] = TEXT("@strErrorDescribe");
	char SqlStr[MAXSQL_LENGTH] = { 0 };
	sprintf_s(SqlStr, sizeof(SqlStr), "call FishCreatePrivateRoom('%d','%u','%u','%d','%u','%u','%u','%d',%s);", 2017, pMsg->wDifen, pMsg->wPlayCountRule, 2013, pMsg->dwUserID, pMsg->dwCostFeeCount, pMsg->wFeeType, pMsg->cbCreateRoom, szDescribeString);

	//获取存储过程的数据
	if (m_Sql[Index].Select(SqlStr, 0, pTable1, true) && pTable1.Rows() == 1)
	{

		//获取数据成功了 我们将数据设置到集合里面去  暂时只返回房间号 
		msg->szPrivateRoomID = pTable1.GetUint(0, 0);
		msg->bTableTypeID = pMsg->bTableTypeID;
		msg->dwUserID = pMsg->dwUserID;
		msg->wDifen = pMsg->wDifen;
		msg->wPlayCountRule = pMsg->wPlayCountRule;
		
		msg->wFeeType = pMsg->wFeeType;
		msg->dwCostFeeCount = pMsg->dwCostFeeCount;
		msg->cbCreateRoom = pMsg->cbCreateRoom;
		LogInfoToFile("DBShujuKu.txt", "OnHandleCreatePrivateRoom返回结果444444444444444444:::::%d:::::::%d****%d*****%d*******%d*", pMsg->bTableTypeID, pMsg->dwUserID, ClientID, pMsg->cbCreateRoom, msg->szPrivateRoomID);
	
	}
	else
	{
		msg->szPrivateRoomID = 0;

		msg->bTableTypeID = pMsg->bTableTypeID;
		msg->dwUserID = 0;
		msg->wDifen = 0;
		msg->wPlayCountRule = 0;

		msg->wFeeType = 0;
		msg->dwCostFeeCount = 0;
		msg->cbCreateRoom = 0;
		LogInfoToFile("DBShujuKu.txt", "OnHandleCreatePrivateRoom返回结果5555555555555555:::::%d:::::::%d****%d*****%d*******%d*", pMsg->bTableTypeID, pMsg->dwUserID, ClientID, pMsg->cbCreateRoom, msg->szPrivateRoomID);
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleCreatePrivateRoom返回结果:::::%d:::::::%d****%d*****%d*******%d*", pMsg->bTableTypeID, pMsg->dwUserID, ClientID, pMsg->cbCreateRoom, msg->szPrivateRoomID);
...
//////////////////////////////////////////////////////////////////////////
//添加赠送
bool FishServer::OnHandleCheckUserContrlDaili(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	DBR_Cmd_CheckUserContrlDaiLi  * pMsg = (DBR_Cmd_CheckUserContrlDaiLi*)pCmd;

	DWORD	OtherUserID = 0;
	SqlTable pTable1;
	char SqlStr[MAXSQL_LENGTH] = { 0 };
	sprintf_s(SqlStr, sizeof(SqlStr), "call FishGameIDConvertUserID('%u');", pMsg->dwOtherUserID);
	if (m_Sql[Index].Select(SqlStr, 0, pTable1, true) && pTable1.Rows() == 1)
	{
		OtherUserID = pTable1.GetUint(0, 0);
	}
	...
	SqlTable pTable122;
	char SqlStr22[MAXSQL_LENGTH] = { 0 };
	sprintf_s(SqlStr22, sizeof(SqlStr22), "call FishGiveGoldUser('%u','%u');", pMsg->dwUserID, OtherUserID);

	bool Result22 = (m_Sql[Index].Select(SqlStr22, 0, pTable122, true) && pTable122.Rows() == 1);
	DBO_Cmd_CheckUserContrlDaiLiBacke* msg = (DBO_Cmd_CheckUserContrlDaiLiBacke*)CreateCmd(DBR_CheckUserIsContrlDailiBack, sizeof(DBO_Cmd_CheckUserContrlDaiLiBacke));
	...
	if (!Result22)
	{
		...
		msg->BackeNum = 0;
		LogInfoToFile(DBErrorSqlFileName, SqlStr22);
		ASSERT(false);
	}
	else
	{
		msg->dwUserID = pMsg->dwUserID;
		//msg->BackeNum
		msg->BackeNum = pTable122.GetUint(0, 0);

	}
//	LogInfoToFile("DBShujuKu.txt", "OnHandleCheckUserDaili添加赠送:::::%d:::::::%d************", pMsg->dwUserID, msg->BackeNum);
	OnAddDBResult(Index, ClientID, msg);
...
//添加代理消息
bool FishServer::OnHandleCheckUserDaili(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	DBR_Cmd_CheckUserDaiLi* pMsg = (DBR_Cmd_CheckUserDaiLi*)pCmd;

	//if (pMsg->bIsCanContrl)//如果是控制消息   代理金币的刷新
	//{
//	LogInfoToFile("DBShujuKu.txt", "OnHandleCheckUserDaili添加代理消息1111111111111111111:::::%d:::::::%d************", pMsg->dwUserID, pMsg->dwGlobel);
		SqlTable pTable1;
		char SqlStr[MAXSQL_LENGTH] = { 0 };
		sprintf_s(SqlStr, sizeof(SqlStr), "call FishAgencyRecord('%u','%u');", pMsg->dwUserID, pMsg->dwGlobel);
		bool Result = (m_Sql[Index].Select(SqlStr, 0, pTable1, true) && pTable1.Rows() == 1 /*&& pTable1.GetUint(0, 0) == 1*/);
		if (!Result)
		{
			LogInfoToFile(DBErrorSqlFileName, SqlStr);
...
//////////////////////////////////////////////////////////////////////////
		...
		SqlTable pTable122;
		char SqlStr22[MAXSQL_LENGTH] = { 0 };
		sprintf_s(SqlStr22, sizeof(SqlStr22), "call FishAgencyType('%u');", pMsg->dwUserID);

		bool Result22 = (m_Sql[Index].Select(SqlStr22, 0, pTable122, true) && pTable122.Rows() == 1);
		DBO_Cmd_CheckUserDaiLiBake* msg = (DBO_Cmd_CheckUserDaiLiBake*)CreateCmd(DBO_CheckUserIsDailiO, sizeof(DBO_Cmd_CheckUserDaiLiBake));
		...
		LogInfoToFile("DBShujuKu.txt", "OnHandleCheckUserDaili添加代理消息:::::%d:::::::%d************", pMsg->dwUserID, msg->BackeNum);
		OnAddDBResult(Index, ClientID, msg);
...
//OnHandleSaveRoleGlobelDown  添加金币减少
bool FishServer::OnHandleSaveRoleGlobelDown(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	DBR_Cmd_SaveRoleGlobelDown* pMsg = (DBR_Cmd_SaveRoleGlobelDown*)pCmd;
	LogInfoToFile("LogGold.txt", "添加金币减少::::::::::::: %d:::::::::: %d------------------", pMsg->dwGlobel, pMsg->dwUserID);
	//////////////////////////////////////////////////////////////////////////
	...
	//////////////////////////////////////////////////////////////////////////
	SqlTable pTable133;
	char SqlStr33[MAXSQL_LENGTH] = { 0 };
	sprintf_s(SqlStr33, sizeof(SqlStr33), "call FishSaveRoleGlobel('%u','%u');", pMsg->dwUserID, pMsg->dwGlobel);
	bool Result33 = (m_Sql[Index].Select(SqlStr33, 0, pTable133, true) && pTable133.Rows() == 1 /*&& pTable1.GetUint(0, 0) == 1*/);
	if (!Result33)
	{
		LogInfoToFile(DBErrorSqlFileName, SqlStr33);
		ASSERT(false);
	}
	return true;
...
bool FishServer::OnHandleSaveRoleGlobel(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("LogGold.txt", "金币的保存::::::::::::: %d:::::::::: %d-------%d-----------", pMsg->dwGlobel, pMsg->dwUserID, pMsg->bIsCanContrl);
//////////////////////////////////////////////////////////////////////////

	if (pMsg->bIsCanContrl)//如果是控制消息
	{
		SqlTable pTable144;
		char SqlStr44[MAXSQL_LENGTH] = { 0 };
		sprintf_s(SqlStr44, sizeof(SqlStr44), "call FishGoldRechargeRecord('%u','%u','%u');", pMsg->dwDaiLiUserID, pMsg->dwUserID, pMsg->dwAddGlobel);
		bool Result44 = (m_Sql[Index].Select(SqlStr44, 0, pTable144, true) && pTable144.Rows() == 1 /*&& pTable1.GetUint(0, 0) == 1*/);
		if (!Result44)
		{
			LogInfoToFile(DBErrorSqlFileName, SqlStr44);
			ASSERT(false);
		}
	}
...
bool FishServer::OnHandleSaveRoleOnLineMin(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleOnLineMin保存玩家在线时间:::::%d:::::::%d*********%d*****", pMsg->OnLineMin, pMsg->dwUserID, ClientID);
	return true;
}
...
bool FishServer::OnHandleSaveRoleTotalRecharge(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleTotalRecharge保存玩家总充值:::::%d:::::::%d*********%d*****", pMsg->Sum, pMsg->dwUserID, ClientID);
	return true;
}
bool FishServer::OnHandleAddRoleTotalRecharge(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleAddRoleTotalRecharge增加玩家的充值数:::::%d:::::::%d*********%d*****", pMsg->Sum, pMsg->dwUserID, ClientID);
	return true;
}
...
bool FishServer::OnHandleSaveRoleIsFirstPayGlobel(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleIsFirstPayGlobel修改玩家是否首冲金币了:::::%d:::::::%d*********%d*****", Index, pMsg->dwUserID, ClientID);
	return true;
...
bool FishServer::OnHandleSaveRoleIsFirstPayCurrcey(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleIsFirstPayCurrcey修改玩家是否首冲钻石了:::::%d:::::::%d*********%d*****", Index, pMsg->dwUserID, ClientID);
	return true;
}
bool FishServer::OnHandleChangeRoleMoney(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleChangeRoleMoney保存玩家的金币修改:::::%d:::::::%d*********%d*****", pMsg->GlobelSum, pMsg->dwUserID, pMsg->CurrceySum);
	return true;
}
bool FishServer::OnHandleTableChange(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//LogInfoToFile("DBShujuKu.txt", "OnHandleTableChange保存玩家进入桌子的数据:::::%d:::::::%d*********%d*****", pMsg->GlobelSum, pMsg->dwUserID, pMsg->CurrceySum);
...
bool FishServer::OnHandleRoleEntityItem(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleRoleEntityItem处理玩家实体物品的操作:::::%d:::::::%d*********%d*****", msg->ItemSum, msg->UserID, msg->MedalNum);
	OnAddDBResult(Index, ClientID, msg);
	return true;
}
bool FishServer::OnHandleDelRoleEntityItem(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleDelRoleEntityItem删除玩家实体物品的操作:::::%d:::::::%d*********%d*****", pMsg->ID, ClientID, Index);
...
bool FishServer::OnHandleSaveRoleMonthCardInfo(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleMonthCardInfo月卡:::::%d:::::::%d*********%d*****", pMsg->UserID, pMsg->MonthCardID, ClientID);
	return true;
}
bool FishServer::OnHandleSaveRoleMaxRate(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleMaxRate最大倍率:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->MaxRate, ClientID);
	return true;
}
...
//	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleRateValue保存倍率:::::%d:::::::%d*********%d*****", pMsg->UserID, pMsg->MaxRateValue, ClientID);
	return true;
}
bool FishServer::OnHandleSaveRoleGetMonthCardRewardTime(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleGetMonthCardRewardTime月卡时间:::::%d:::::::%d*********%d*****", pMsg->UserID, Index, ClientID);
	return true;
}
bool FishServer::OnHandleSaveRoleVipLevel(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleVipLevel保存玩家的VIP等级:::::%d:::::::%d*********%d*****", pMsg->UserID, pMsg->VipLevel, ClientID);
	return true;
}
bool FishServer::OnHandleSaveRoleCashSum(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleCashSum保存玩家今天兑换的次数:::::%d:::::::%d*********%d*****", pMsg->UserID, pMsg->CashSum, pMsg->TotalCashSum);
	return true;
}
bool FishServer::OnHandleSaveRoleParticularStates(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleParticularStates保存价格:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->ParticularStates, ClientID);
	return true;
}
bool FishServer::OnHandleSaveRoleSecPassword(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleSecPassword保存密码:::::%d:::::::%d*********%d*****", pMsg->dwUserID, Index, ClientID);
...
bool FishServer::OnHandleChangeRoleShareStates(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleChangeRoleShareStates分享:::::%d:::::::%d*********%d*****", pMsg->dwUserID, Index, ClientID);
	return true;
}
//根据游戏id获取玩家id
bool FishServer::OnHandleGameIDConvertUserID(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleGameIDConvertUserID根据游戏id获取玩家id:::::%d:::::::%d*********%d*****", msgDB->dwUserID, pMsg->dwGameID, ClientID);
...
bool FishServer::OnHandleChangeRoleSecPassword(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleChangeRoleSecPassword修改玩家的二级密码id:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->dwOldSecPasswordCrc1, ClientID);
...
bool FishServer::OnHandleDelRoleCashSum(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleDelRoleCashSum删除玩家兑换数量:::::%d:::::::%d*********%d*****", pMsg->UserID, pMsg->DelCashSum, ClientID);
	return true;
}
bool FishServer::OnHandleSaveRoleMedal(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleMedal保存玩家奖牌id:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->dwMedalSum, ClientID);
	return true;
}
bool FishServer::OnHandleSaveRoleSendGiffSum(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleSendGiffSum保存玩家发送的礼物:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->SendSum, ClientID);
	return true;
}
bool FishServer::OnHandleSaveRoleAcceptGiffSum(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleAcceptGiffSum保存玩家接受礼物:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->AcceptSum, ClientID);
	return true;
}
bool FishServer::OnHandleSaveRoleTaskStates(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleTaskStates保存玩家任务:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pTaskStates, ClientID);
	return true;
}
bool FishServer::OnHandleSaveRoleAchievementStates(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleAchievementStates保存成就222222:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pAchievementStates, ClientID);
	return true;
}
...
//	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleActionStates保存动作:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pActionStates, ClientID);
...
//	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleOnlineStates保存玩家在线:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->OnlineStates, ClientID);
	return true;
}
bool FishServer::OnHandleSaveRoleCheckData(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleCheckData保存玩家检查:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->CheckData, ClientID);
	return true;
}
bool FishServer::OnHandleSaveRoleIsShopIpAddress(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleIsShopIpAddress保存玩家地址:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->IsShowIP, ClientID);
	return true;
}
bool FishServer::OnHandleSaveRoleAllInfo(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//LogInfoToFile("LogGold.txt", "一次性保存玩家全部的数据金币的数据::::::::wan玩家%d::::::金币:: %d::::: %d::::::", pMsg->dwUserID, pMsg->dwGlobeNum, pMsg->RoleServerInfo.TotalFishGlobelSum);
	//一次性保存玩家全部的数据
	...
	if ((pMsg->States & MsgBegin) != 0)
	{
		...
		//2.GameData
		{
	//	g_FishServer.ShowInfoToWin("玩家11111 数据进行保存");
...
//	LogInfoToFile("DBShujuKu.txt", "OnHandleGetUserItem加载玩家物品:::::%d:::::::%d*********%d*****", pMsg->dwUserID, Index, ClientID);
	return true;
}
...
bool FishServer::OnHandleAddUserItem(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	if (pMsg->ItemInfo.LastMin != 0)
	{
		sprintf_s(SqlStr, sizeof(SqlStr), "call FishAddItem('%u','%u','%u','%d-%d-%d %d:%d:%d');", pMsg->dwUserID, pMsg->ItemInfo.ItemID, pMsg->ItemInfo.ItemSum,
			NowTime.tm_year + 1900, NowTime.tm_mon + 1, NowTime.tm_mday, NowTime.tm_hour, NowTime.tm_min, NowTime.tm_sec);
	//	LogInfoToFile("DBShujuKu.txt", "OnHandleAddUserItem往玩家背包里添加一件物品背包有的:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->ItemInfo.ItemSum, NowTime.tm_year + 1900);
	}
	else
	{
		//call FishAddItem('20449','904','1','0-0-0 0:0:0')
	...
	//	LogInfoToFile("DBShujuKu.txt", "OnHandleAddUserItem往玩家背包里添加一件物品背包没有的:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->ItemInfo.ItemSum, NowTime.tm_year + 1900);
	}
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleAddUserItem往玩家背包里添加一件物品:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->ItemInfo.ItemSum, ClientID);
	return true;
}
bool FishServer::OnHandleDelUserItem(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleDelUserItem玩家删除指定的物品:::::%d:::::::%d*********%d*****", pMsg->ItemOnlyID, Index, ClientID);
	return true;
}
bool FishServer::OnHandleChangeUserItem(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//LogInfoToFile("DBShujuKu.txt", "OnHandleChangeUserItem修改一组物品的数量:::::%d:::::::%d*********%d*****", pMsg->ItemOnlyID, pMsg->ItemSum, ClientID);
	return true;
}
...
//	LogInfoToFile("DBShujuKu.txt", "OnHandleGetUserRelation加载玩家的关系:::::%d:::::::%d*********%d*****", pMsg->dwUserID, Index, ClientID);
	return true;
}
bool FishServer::OnHandleAddUserRelation(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleAddUserRelation添加玩家的关系数据:::::%d:::::::%d*********%d*****", pMsg->dwSrcUserID, msg->RelationInfo.GameID, ClientID);
	return true;
}
...
bool FishServer::OnHandleGetUserMail(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//LogInfoToFile("DBShujuKu.txt", "OnHandleGetUserMail获取玩家全部的邮件:::::%d:::::::%d*********%d*****", pMsg->dwUserID, Index, ClientID);
	return true;
}
bool FishServer::OnHandleAddUserMail(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//LogInfoToFile("DBShujuKu.txt", "OnHandleAddUserMail添加一封邮件:::::%d:::::::%d*********%d*****", pMsg->dwDestUserID, Index, ClientID);
	return true;
}
bool FishServer::OnHandleDelUserMail(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleDelUserMail移除掉一封邮件:::::%d:::::::%d*********%d*****", pMsg->dwMailID, Index, ClientID);
	return true;
}
bool FishServer::OnHandleGetUserMailItem(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//LogInfoToFile("DBShujuKu.txt", "OnHandleGetUserMailItem从邮件里拿去物品:::::%d:::::::%d*********%d*****", pMsg->dwMailID, Index, ClientID);
	return true;
}
bool FishServer::OnHandleChangeUserMailIsRead(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleChangeUserMailIsRead将邮件设置为已读:::::%d:::::::%d*********%d*****", pMsg->dwMailID, Index, ClientID);
	return true;
}
...
bool FishServer::OnHandleQueryRoleInfoByNickName(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleQueryRoleInfoByNickName玩家进行查询:::::%d:::::::%s*********%d*****", pMsg->dwUserID, DestNickName, pMsg->Page);
	return true;
}
bool FishServer::OnHandleQueryRoleInfoByGameID(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleQueryRoleInfoByGameID玩家进行查询游戏id:::::%d:::::::%s*********%d*****", pMsg->dwUserID, msg->RoleInfo.NickName, msg->RoleInfo.GameID);
	return true;
}
bool FishServer::OnHandleQueryRoleInfoByUserID(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleQueryRoleInfoByUserID玩家按ID进行查询:::::%d:::::::%s*********%d*****", pMsg->dwUserID, msg->RoleInfo.NickName, msg->RoleInfo.GameID);
	return true;
}
...
bool FishServer::OnHandleLoadRoleTask(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//LogInfoToFile("DBShujuKu.txt", "OnHandleLoadRoleTask加载任务:::::%d:::::::%d*********%d*****", pMsg->dwUserID, ClientID, Index);
	return true;
}
bool FishServer::OnHandleSaveRoleTask(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	std::cout << IpStr;*/
	//LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleTask保存一个任务:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->TaskInfo.TaskID, Index);
	return true;
}
...
//	LogInfoToFile("DBShujuKu.txt", "OnHandleClearRoleTask清理当前数据库的全部的任务:::::%d:::::::%d*********%d*****", pMsg->dwUserID, ClientID, Index);
	return true;
}
...
bool FishServer::OnHandleSaveAllAction(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveAllAction保存记录:::::%d:::::::%d*********%d*****", pMsg->dwUserID, ClientID, Index);
	return true;
}
...
bool FishServer::OnHandleSaveRoleEntityItem(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	sprintf_s(SqlStr, sizeof(SqlStr), "call FishAddRoleEntityItem('%u','%u','%u','%u','%u','%s','%llu','%s', '%d-%d-%d %d:%d:%d','%s');", pMsg->dwUserID, pMsg->ItemID, pMsg->ItemSum, pMsg->UseMedal, pMsg->NowMedalSum, DestAddress, pMsg->Phone, DestName, pTime.wYear, pTime.wMonth, pTime.wDay, pTime.wHour, pTime.wMinute, pTime.wSecond, DestIDEntity);
	free(DestAddress);
	free(pAddress);
	free(DestName);
	free(pName);
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleEntityItem添加实物兑换数据保存:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->ItemSum, Index);
	return true;
}
...
//	LogInfoToFile("DBShujuKu.txt", "OnHandleLoadRoleEntity加载实物数据:::::%d:::::::%s*********%d*****", pMsg->dwUserID, msg->RoleInfo.Email, Index);
	return true;
}
...
bool FishServer::OnHandleAddRoleGiff(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleAddRoleGiff发送一个赠送给玩家:::::%d:::::::%d*********%d*****", pMsg->SrcUserID, pMsg->DestUserID, Index);
	return true;
}
...
bool FishServer::OnHandleLoadRoleGameData(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//LogInfoToFile("DBShujuKu.txt", "OnHandleLoadRoleGameData加载游戏数据:::::%d:::::::%d*********%d*****", pMsg->dwUserID, msg->GameData.RoleGetGlobelSum, Index);
	return true;

}
bool FishServer::OnHandleSaveRoleGameData(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
//	g_FishServer.ShowInfoToWin("玩家 OnHandleSaveRoleGameData数据进行保存");
	...
	//LogInfoToFile("DBShujuKu.txt", "OnHandleSaveRoleGameData保存游戏数据:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->GameData.RoleGetGlobelSum, Index);
	return true;
}
...
//	LogInfoToFile("DBShujuKu.txt", "OnHandleChangeRoleGameDataByMonth保存游戏数据月卡:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->MonthIndex, Index);
	return true;
}
...
//	LogInfoToFile("DBShujuKu.txt", "OnHandleLoadAllAccountInfo加载玩家的关系:::::%d:::::::%d*********%d*****", Index, ClientID, Index);
	return true;
}
bool FishServer::OnHandleLogRechargeInfo(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleLogRechargeInfo充值记录:::::%d:::::::%d*********%d*****", pMsg->UserID, pMsg->OldGlobelNum, pMsg->AddGlobel);
	return true;
}
//玩家上下线的信息处理   wanjia玩家信息    
bool FishServer::OnHandleLogRoleOnlineInfo(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	//OnHandleGetRoleInfoByUserID();
	...
	//LogInfoToFile("DBShujuKu.txt", "OnHandleLogRoleOnlineInfo玩家上下线的信息处理:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->IsOnline, pMsg->GlobelSum);
	////////////////////////////////////////////////////////////////////////////添加存储修改
	////让玩家进行数据库查询 登陆
	...
	//OnHandleGetRoleInfoByUserID(Index, ClientID, &msg);//添加玩家信息刷新
...
bool FishServer::OnHandleLogPhonePayInfo(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleLogPhonePayInfo话费充值的记录:::::%d:::::::%d*********%d*****", pMsg->UserID, pMsg->PhoneNumber, pMsg->FacePrice);
	return true;
}
bool FishServer::OnHandleLogRoleEntityItemInfo(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleLogRoleEntityItemInfo记录玩家实体物品的数据:::::%d:::::::%d*********%d*****", pMsg->UserID, pMsg->ItemID, pMsg->ItemSum);
	return true;
}
...
bool FishServer::OnHandleLogExChangeInfo(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleLogExChangeInfo记录玩家使用兑换码的状态:::::%d:::::::%d*********%d*****", pMsg->dwUserID, ClientID, Index);
	return true;
}
...
//	LogInfoToFile("DBShujuKu.txt", "OnHandleLogDialInfo记录森林舞会的数据:::::%d:::::::%d*********%d*****", pMsg->BrandGlobel, pMsg->SystemGlobel, pMsg->RoleSum);
	return true;
}
bool FishServer::OnHandleLogCarInfo(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	m_Sql[Index].Select(SqlStr, 0, pTable1, true);
	//LogInfoToFile("DBShujuKu.txt", "OnHandleLogCarInfo记录车的数据:::::%d:::::::%d*********%d*****", pMsg->BrandGlobel, pMsg->SystemGlobel, pMsg->RoleSum);
	return true;
}
...
bool FishServer::OnAddRecharge(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnAddRecharge玩家不在线:::::%d:::::::%d*********%d*****", pMsg->AddGlobelSum, pMsg->rechargeInfo.UserID, pMsg->rechargeInfo.Price);
	return true;
}
bool FishServer::OnLoadPhonePayOrderList(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//LogInfoToFile("DBShujuKu.txt", "OnLoadPhonePayOrderList话费充值订单:::::%d:::::::%d*********%d*****", ClientID, Index, Index);
	return true;
}
bool FishServer::OnLoadRechargeOrderID(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	//LogInfoToFile("DBShujuKu.txt", "OnLoadRechargeOrderID加载全部的订单号:::::%d:::::::%d*********%d*****", ClientID, Index, Index);
	return true;
}
...
bool FishServer::OnGetRechargeOrderID(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnGetRechargeOrderID创建订单写入数据库:::::%d:::::::%d*********%d*****", pMsg->dwUserID, pMsg->ShopID, pMsg->IsPC);
	OnAddDBResult(Index, ClientID, msg);
	return true;
}
bool FishServer::OnQueryRechargeOrderInfo(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnQueryRechargeOrderInfo充值消息查询:::::%d:::::::%d*********%d*****", msg->dwUserID, msg->CheckShopID, pMsg->Price);
	OnAddDBResult(Index, ClientID, msg);
	return true;
}
...
//	LogInfoToFile("DBShujuKu.txt", "OnHandleLoadAllAnnouncementInfo加载全部的公告:::::%d:::::::%d*********%d*****", Index, ClientID, Index);
	return true;
}
...
//	LogInfoToFile("DBShujuKu.txt", "OnHandleClearFishDB清理下数据库:::::%d:::::::%d*********%d*****", Index, ClientID, Index);
	return true;
}
...
bool FishServer::OnHandleLoadCharInfo(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleLoadCharInfo加载椅子:::::%d:::::::%d*********%d*****", pMsg->dwUserID, ClientID, Index);
	return true;
}
...
bool FishServer::OnHandleLoadRelationRequest(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleLoadRelationRequest加载请求:::::%d:::::::%d*********%d*****", pMsg->dwUserID, ClientID, Index);
	return true;
}
