...
float CConfig::CatchChance(PlayerID player, USHORT bulletId, byte bulletType, byte fishType, int byOrder, int byMaxCatch, BYTE byPackageType,bool bLocked)
{
	...
	if (pPlayer)
	{
		...
		if (m_pTableManager->RNumrateF != 0 && m_pTableManager->RNumrateF <= 10 && m_pTableManager->RNumrateS<=10)
		{
			...
			Con_Cannon::s_finalratio = m_pTableManager->RNumrateS;//最大值
...
//////////////////////////////////////////////////////////////////////////
	...
	/*		{
		...
		else if (pTable->GetTableTypeID() == 1)
		{
			...
			Tablerate = 10;
			fbase = fbase*0.99;
			//LogInfoToFile("Control.txt", "看看桌子2:::::::%f:::::::::::%u::::::::::%d::::::%u::::", fbase, m_pTableManager->KucunNum[pTable->GetTableTypeID()], pTable->GetTableTypeID(), Tablerate);
		}
		...
		if (fishType == 22)
		{
			fbase = (fbase*Index) / 90;
		}
		else if (fishType == 0)
		{
			fbase = (fbase*Index) / 70;
		}
		else if (fishType == 4)
		{
			fbase = (fbase*Index) / 40;
		}
		else if (fishType == 13)
		{
			fbase = (fbase*Index) / 60;
		}
		else if (fishType == 23)
		{
			fbase = (fbase*Index) / 120;
		}
		else if (fishType == 16)
		{
			fbase = (fbase*Index) / 110;
		}
		else if (fishType == 10)
		{
			fbase = (fbase*Index) / 160;
		}
		else if (fishType == 9)
		{
		...
		{
			fbase = (fbase*Index) / 150;
			
		}
		else if (fishType == 20)
		{
			fbase = (fbase*Index) / 180;
			
		}
		else if (fishType == 1)
		{
			fbase = (fbase*Index) / 200;
			
		}
		else if (fishType == 3)
		{
			fbase = (fbase*Index) / 100;
