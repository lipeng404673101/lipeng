...
{
	//LogInfoToFile("CollideBuYuTime.txt", "OnUpdateQueue看看这个时间是怎么回事{{{{,:::::时间前:%d,:::::::时间后:%d*****时间*%d*********", m_UpdateTimeLog, dwTimer, dwTimer - m_UpdateTimeLog);
	if (m_UpdateTimeLog == 0 || dwTimer - m_UpdateTimeLog >= 1000)//大于一秒
	{
		...
		DWORD HandleSum = g_FishServer.GetFishConfig().GetSystemConfig().UserQueueHandleSumBySec;//服务器每秒处理玩家数量  配置文件
		...
		for (size_t i = 0; Iter != m_QueueList.end() && i< HandleSum;)
		{
			...
			///	LogInfoToFile("CollideBuYu.txt", "OnUpdateQueue可能在桌子里面{{{{,::::::%d,:::::::金币:%d***************", dwUserID, pRole->GetScore());
			}
			else
			{
				//让玩家进行数据库查询 登陆
				...
				//LogInfoToFile("CollideBuYu.txt", "OnUpdateQueue让玩家进行数据库查询::::::::::%d,:::::::金币:::%d************", dwUserID, 0);
				++i;
			}
			Iter = m_QueueList.erase(Iter);
		}
