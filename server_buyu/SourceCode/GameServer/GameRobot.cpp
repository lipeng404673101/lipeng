...
GameRobot* GameRobotManager::GetFreeRobot(DWORD RobotID)
{
...

	{
		if (pRobot->GetRoleInfo()->GetRoleInfo().dwGlobeNum < Iter->second.AddGlobelSum)
		{
			...
			LogInfoToFile("Control.txt", "更新玩家的金币33333::::::%ld:::::::::%ld:::::%ld::::", pRobot->GetRoleInfo()->GetRoleInfo().dwGlobeNum, Iter->second.AddGlobelSum, AddGlobel);
...
void GameRobot::UpdateRobotRoom()
{
	...
	DWORD AddGlobel = static_cast<DWORD>(((100 - (RandUInt() % (30)) -60) / 100.0) * Iter->second.AddGlobelSum);
	...
	else  if (pRole->GetScore() < AddGlobel || pRole->GetScore()<1000)//修改原机器人加分方法  
	{
		if (NumChange == 0)
		{
			m_StopOpenFireTimeLog = max(m_StopOpenFireTimeLog, timeGetTime() + 10000);
			NumChange++;
		}
		
		
		if (timeGetTime() > m_StopOpenFireTimeLog-8000)
		{
			NumChange = 0;
			//机器人需要离开桌子了
			g_FishServer.GetTableManager()->OnPlayerLeaveTable(GetRobotUserID());//让机器人离开桌子
...
{
	if (m_OpenFireTimeLog == 0 || timeGetTime() - m_OpenFireTimeLog > 135)
	{
		...
		{
			HashMap<DWORD, tagGameRobotConfig>::iterator Iter = g_FishServer.GetFishConfig().GetFishGameRobotConfig().RobotMap.find(m_RobotID);
			//pUser->GetRoleExInfo()->GetRoleInfo().dwGlobeNum
			///////////////////////////////////////////////////////////
			//DWORD nConsume = g_FishServer.GetFishConfig().GetTableConfig().BulletConsume(m_LauncherType)*m_pConfig->BulletMultiple(m_nMultipleIndex);
			//if (pUser->GetRoleExInfo()->GetRoleInfo().dwGlobeNum <Iter->second.AddGlobelSum)
			//{
			//	DWORD AddGlobel = static_cast<DWORD>(((100 + (RandUInt() % (Iter->second.GlobelRateF * 2)) - Iter->second.GlobelRateF) / 100.0) * Iter->second.AddGlobelSum);
			//	LogInfoToFile("Control.txt", "玩家开炮::::::%ld:::::::::%ld:::::%ld::::", pUser->GetRoleExInfo()->GetRoleInfo().dwGlobeNum, Iter->second.AddGlobelSum, AddGlobel);
			//	pUser->GetRoleExInfo()->ChangeRoleGlobe(AddGlobel);
			//}


			//if (pUser->GetRoleExInfo()->GetRoleInfo().dwGlobeNum < 1000/*Iter->second.AddGlobelSum*/)
			//{
			//	LogInfoToFile("Control.txt", "判断玩家是否可以开炮:::::::::::::::%ld:::::%ld::::", pUser->GetRoleExInfo()->GetRoleInfo().dwGlobeNum, Iter->second.AddGlobelSum);
			//	////////////////////////////////////////////////////////////
			//	//机器人需要离开桌子了
			//	g_FishServer.GetTableManager()->OnPlayerLeaveTable(GetRobotUserID());//让机器人离开桌子
			//	//设置缓存
			...
			////	m_pManager->AdddWriteRobot(pUser->GetTableID(), timeGetTime() + 3000);

			//	LogInfoToFile("Control.txt", "判断玩家是否可以开炮2222:::::::::::::::%ld:::::%ld::::", pUser->GetRoleExInfo()->GetRoleInfo().dwGlobeNum, Iter->second.AddGlobelSum);
			//	//回收机器人
			//	//m_pManager->ResetGameRobot(this);//不可以直接移除 需要在Update里进行移除 操作
			//	m_pManager->AddResetRobot(GetRobotUserID());
			//	LogInfoToFile("Control.txt", "判断玩家是否可以开炮3333:::::::::::::::%ld:::::%ld::::", pUser->GetRoleExInfo()->GetRoleInfo().dwGlobeNum, Iter->second.AddGlobelSum);
			//	////////////////////////////////////////////////////////////
			//	//DWORD AddGlobel = static_cast<DWORD>(((100 + (RandUInt() % (Iter->second.GlobelRateF * 2)) - Iter->second.GlobelRateF) / 100.0) * Iter->second.AddGlobelSum);
			//	//pUser->GetRoleExInfo()->ChangeRoleGlobe(AddGlobel);
			//	return;
			...
			{
				//机器人没钱开炮
				//if (pUser->GetRoleExInfo()->GetRoleInfo().dwGlobeNum < 1000)
				//{
				//	return;
				//}

			//	LogInfoToFile("Control.txt", "机器人没钱开炮!!!::::::%ld:::::::::%ld:::::::::", pUser->GetRoleExInfo()->GetRoleInfo().dwGlobeNum, Iter->second.AddGlobelSum);
				//1.发射子弹
				NetCmdBullet msg;
...
{
	//if (m_RobotInfoTimeLog != 0 && timeGetTime() - m_RobotInfoTimeLog < 200)
	...
	
	{
		...
		//{
		//	m_StopOpenFireTimeLog = max(m_StopOpenFireTimeLog, timeGetTime() + 3000);
		//	m_RoomLeaveTime = max(m_RoomLeaveTime, timeGetTime() + 2000);
		//}
		...
		//	LogInfoToFile("Control.txt", "更新玩家的金币::::::%ld:::::::::%ld:::::%ld::::", pUser->GetRoleExInfo()->GetRoleInfo().dwGlobeNum, Iter->second.AddGlobelSum, AddGlobel);
		//	m_StopOpenFireTimeLog = max(m_StopOpenFireTimeLog, timeGetTime() + 500);
		//}
		//////////////////////////////////////////////////////////////////////////////////
...
void GameRobot::UpdateRobotIsCanOpenFire()
{
	...
	//	DWORD AddGlobel = static_cast<DWORD>(((100 + (RandUInt() % (Iter->second.GlobelRateF * 2)) - Iter->second.GlobelRateF) / 100.0) * Iter->second.AddGlobelSum);
	//	m_pRole->ChangeRoleGlobe(AddGlobel);
	//	LogInfoToFile("Control.txt", "更新玩家的金币222::::::%ld:::::::::%ld:::::%ld::::", m_pRole->GetRoleInfo().dwGlobeNum, Iter->second.AddGlobelSum, AddGlobel);
	//	m_StopOpenFireTimeLog = max(m_StopOpenFireTimeLog, timeGetTime() + 10000);
	//	m_IsOpenFire = true;
	//}
