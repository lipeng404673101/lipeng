...
CheckResult CheckResponse(HttpClientResult *hcr)
{
	...
	if (hcr->Orignal)
	{
		if (hcr->RecvSize >= *((UINT*)hcr->RecvBuff))
		{
			...
			//LogInfoToFile("Chongzhi.txt", TEXT("CheckResponse监听6*****%d*********%s**"),  hcr->RecvSize , hcr->RecvBuff);
			return CHECK_OK;
		}
	}
...
bool RecvResponse(HttpClientResult *hcr)
{
	...
	//	LogInfoToFile("Chongzhi.txt", TEXT("Recv:%d, total:%d, string:%s"), nSize, hcr->RecvSize + nSize, hcr->RecvBuff);
		/*
		FILE *pp;
		char xx[100];
		static int kkk = 0;
...
bool SendRequest(HttpClientResult *hcr, HttpHostData &hhd)
{
	...
	//LogInfoToFile("Chongzhi.txt", TEXT("SendRequest监听3**************%s**"), url);
...
bool ConnectRequest(HttpClientResult *hcr, HttpHostData &hhd)
{
	...
	//LogInfoToFile("Chongzhi.txt", TEXT("ConnectRequest监听4*****%d*********%d********%s******%d***********"), hhd.Port, hhd.IP, addr.sin_addr, ret);
...
bool HttpClient::AddPostRequest(UINT_PTR ID, USHORT hostIdx, const char *pcURL, const char *pcPostData, bool bOrignal)
{
	...
	LogInfoToFile("Chongzhi.txt", TEXT("AddPostRequest请求进来了吗***%d***%d*****%s******%d**********%s*****"), hostIdx, m_HostList.size(), pcURL, ID,  pcPostData);
	...
	//////////////////////////////////////////////////////////////////////////
	LogInfoToFile("Chongzhi.txt", TEXT("AddPostRequest发送字符串是不是成功了***%d***%d*****%d******%d*****%d*****%s*****"), hcr->ID, hcr->SendSize, hcr->PostSize, hcr->State, hcr->HostIdx, pcPostData);
//	cout << pcPostData;

	//////////////////////////////////////////////////////////////////////////
	if (m_RequestList.Push(hcr) == false)
	{
		LogInfoToFile("Chongzhi.txt", TEXT("添加URLRequest失败***%d***%s*****%d******%d***********"), hostIdx, pcURL, bOrignal, ID);
		Log("添加URLRequest失败");
		return false;//修改了这个  订单发出
	}
