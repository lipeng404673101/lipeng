...
bool CheckCatchFish(Fish *pFish, CatchData &catchData, FishManager *pMgr, Bullet *pBullet)	//HasActionReduction :必须有动作才减速
{
	...
	else
	{
		...
		else
		{
			...
			//计算概率
			float chance = RandFloat();//随机数小于概率就会捕获
			...
			if (chance < catchChance)
			{
				...
				fc.eventType = CATCH_EVENT_CATCHED;//捕获消息
...
void FishCollider::Collide(CTableRoleManager *pm, BulletManager *pBulletMgr, FishManager *pFishMgr, FishSendInterface *pSend)
{
	...
	for (UINT i = 0; i < pBulletMapList->size();)
	{
		...
		if (bt == BULLET_TYPE_FREEZE)
		{
			...
			//LogInfoToFile("CollideBuYu.txt", "冰冻子弹::::: %d::::::: %d:::::::-------", cd.goldNum, cd.playerID, pCmd->TotalNum);
		}
		else
		{
			//普通子弹
			...
			pCmd->GoldNum  = cd.goldNum;
			pCmd->TotalNum = pm->GetRoleByUserID(pBullet->Player)->GetRoleExInfo()->GetRoleInfo().dwGlobeNum;//获取玩家的金币  主动获取  这个可以用来做登陆获取
			pSend->SendAll(pCmd);
			::free(pCmd);
			//LogInfoToFile("CollideBuYu.txt", "普通子弹::::::: %d::::::------- %d-----------------%d*************", cd.goldNum, cd.playerID, pCmd->TotalNum);
