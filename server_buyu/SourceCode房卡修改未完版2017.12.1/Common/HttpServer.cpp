...
//	LogInfoToFile("Chongzhi.txt", TEXT("监听********%d******%d********%d****"), addr.sin_port, hsi.Port, addr.sin_addr.S_un.S_addr);
...
UINT CheckContentIsEnd(HttpClientData *pc)
{
	if (pc->IsPost || (pc->RecvSize > POST_COUNT && _strnicmp(pc->Buff, POST_STR, POST_COUNT) == 0))
	{
		if (pc->IsPost == false)
		{
			...
			pc->IsPost = true;
			//LogInfoToFile("Chongzhi.txt", TEXT("CheckContentIsEnd监听1**************%s**"), pc->RequestUrl);
		}
...
void SendResponse(HttpClientData *pc, const char *pcTime, const char *pcText)
{
	...
	//LogInfoToFile("Chongzhi.txt", TEXT("响应:%s, 字节:%d/%d, 发送:%d"), pc->Buff, ret, ret, count);
}
//替换&,=符号
...
bool HttpServer::SplitStr(HttpClientData *pc, UINT endSize, HttpRecvData &recvData)
{
	...
	{ 
		...
		LogInfoToFile("Chongzhi.txt", TEXT("recv ->key:%s, value:%s, crc:%d"), appkey.c_str(), value.c_str(), key);
...
bool HttpServer::CheckPostData(HttpClientData *pc, HttpRecvData &recvData)
{
	...
	//LogInfoToFile("Chongzhi.txt", TEXT("OnHandleAddOrderResult检查请求的地址**************%s**"), pAddrData->WebAddr);
	if (bFind == false)
		return false;
				...
				if (endSize != END_ERR)
				{
					...
					if (pc->IsPost)
					{
						if (CheckPostData(pc, rd))
						{
							...
							LogInfoToFile("Chongzhi.txt", TEXT("CheckPostData检查收到内容**************%s**"), m_InitData.AddrList[rd.RequestIdx].Response);
					...
					{
						trd.ThreadRecvList.AddItem(rd);
						LogInfoToFile("Chongzhi.txt", TEXT("消息来自哪里内容已经收到111**************%s**"), m_InitData.AddrList[rd.RequestIdx].Response);
						SendResponse(pc, strTime, m_InitData.AddrList[rd.RequestIdx].Response);
					}
					else
						LogInfoToFile("Chongzhi.txt", TEXT("消息来自哪里内容已经收到22222**************%s**"), m_InitData.AddrList[rd.RequestIdx].Response);
						SendResponse(pc, strTime, FAIL_TXT);
					pc->Removed = true;
				}
