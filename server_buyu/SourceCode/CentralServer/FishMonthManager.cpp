...
void FishMonth::OnLoadRobotInfo()
{
	...
	for (; IterRobot != IterConfig->second.m_RobotMap.end(); ++IterRobot)
	{
		...
		for (DWORD i = 0; i < RobotSum; ++i)
		{
			...
			TCHARCopy(pRole->NickName, CountArray(pRole->NickName), pInfo->NickName, CountArray(pRole->NickName)-1);//玩家名称用于记录xml文件使用的
...
void FishMonth::UpdateMonthIndex()
{
	...
	for (BYTE i = 0; i < MAX_MonthList_Length && i<m_RoleVec.size(); ++i)
	{
		...
		TCHARCopy(msg.MonthInfo[i].NickName, CountArray(msg.MonthInfo[i].NickName), m_RoleVec[i]->NickName, CountArray(msg.MonthInfo[i].NickName)-1);
	}
