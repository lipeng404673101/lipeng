...
bool FishServer::OnHandleAccountRsg(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	LogInfoToFile("DBShujuKu.txt", "OnHandleQueryLogon注册存储:::::%s:::::::%s*********%s*****", strNickName.c_str(), DestMacAddress, AccountName);
...
bool FishServer::OnHandleGetRoleInfoByUserID(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	if (m_Sql[Index].Select(SqlStr, 0, pTable1, true) && pTable1.Rows() == 1)
	{
		...
		LogInfoToFile("CollideBuYu.txt", "OnHandleGetRoleInfoByUserID获取玩家信息数据库读取:::::%d:::::::%d*********%d*****", msg->RoleInfo.dwGlobeNum, pMsg->dwUserID, msg->RoleServerInfo.TotalFishGlobelSum);
...
bool FishServer::OnHandelLoadRobotInfo(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	if (m_Sql[Index].Select(SqlStr, 0, pTable1, true))
	{
		...
		for (DWORD i = 0; i < Rows; ++i)
		{
			...
			//////////////////////////////////////////////////////////////////////////7
			//添加参数  头像
			const TCHAR* szHeadHttp = pTable1.GetStr(i, 15);
			TCHARCopy(msg->Array[i].szHeadHttp, CountArray(msg->Array[i].szHeadHttp), szHeadHttp, _tcslen(szHeadHttp));
		}
