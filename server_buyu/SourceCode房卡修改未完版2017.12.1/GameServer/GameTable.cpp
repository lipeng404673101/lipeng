...
void GameTable::SendRoleJoinInfo(DWORD dwUserID)
{
	...
	LogInfoToFile("CollideBuYu.txt", "将新加入的玩家数据进行同步处理:::::::::::::::  %d :::::::::::::%d************",  pUser->GetRoleExInfo()->GetRoleInfo().dwGlobeNum, pUser->GetID());
	...
	//头像
	TCHARCopy(msgUser.UserInfo.szHeadHttp, CountArray(msgUser.UserInfo.szHeadHttp), pUser->GetRoleExInfo()->GetRoleInfo().szHeadHttp, _tcslen(pUser->GetRoleExInfo()->GetRoleInfo().szHeadHttp));
...
//////////////////////////////////////////////////////////////////////////
	...
	for (BYTE i = 0; i < m_RoleManager.GetMaxPlayerSum(); ++i)
	{
		...
		//	msg.UserInfo.dwExp = pOtherUser->GetRoleExInfo()->GetRoleInfo().dwExp;
			...
			TCHARCopy(msg.UserInfo.szHeadHttp, CountArray(msg.UserInfo.szHeadHttp), pOtherUser->GetRoleExInfo()->GetRoleInfo().szHeadHttp, _tcslen(pOtherUser->GetRoleExInfo()->GetRoleInfo().szHeadHttp));
...
bool GameTable::OnRoleLeaveTable(DWORD dwUserID)
{
	...
	if (m_RoleManager.GetRoleSum() == 0)
	{
		...
		//			LogInfoToFile("CollideBuYu.txt", "一个桌子对象 已经被销毁33322******* %d******* %d----------%d--------------", pRole->GetRoleInfo().dwGlobeNum, pRole->GetUserID(), Iter);
		//			delete *Iter;
		//		}
		//		m_TableVec.clear();
		//	}
