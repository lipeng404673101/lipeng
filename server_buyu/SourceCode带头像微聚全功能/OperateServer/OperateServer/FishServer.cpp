...
bool FishServer::MainUpdate()
{
	while (!m_IsClose)
	{
		...
		m_WeiXinLogon.Update();//微信登录
		m_QQLogon.Update();//qq登录
...
bool FishServer::ConnectHttpClient()
{
	...
	//	LogInfoToFile("Chongzhi.txt", TEXT("ConnectHttpClient是否进入********%d*****"), m_NormalRechargeID);
		...
		case GO_AddNormalOrderID:
			{
				...
				LogInfoToFile("Chongzhi.txt", TEXT("GO_AddNormalOrderID充值普通的订单id*************"));  
				if (!m_NormalRechargeManager.OnHandleAddOrder(pMsg->dwUserID, pMsg->OrderID, pMsg->ShopID))
				{
					//订单处理   以后的订单都按照  msg.Result=true
				...
				//	LogInfoToFile("Chongzhi.txt", TEXT("GO_AddNormalOrderID普通订单请求失败********************"));
				}
				return true;
			}
			break;
...
//	LogInfoToFile("Chongzhi.txt", TEXT("HandleHttpClientMsg是不是外部连接jjjjjjjjjj************%d***********%d**********"), pResult->ID,pResult->HostIdx);
	...
	else if (pResult->HostIdx == m_NormalRechargeID)
	{
		...
		LogInfoToFile("Chongzhi.txt", TEXT("HandleHttpClientMsg到充值订单去************%u***"), *(UINT64*)pResult->ID);
