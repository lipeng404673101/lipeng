...
void ShopManager::OnShellShopItem(CRoleEx* pRole, BYTE ShopID, BYTE ItemIndex, DWORD ItemSum)
{
	...
	//////////////////////////////////////////////////////////////////////////cichu此处购买逻辑本版本不用   此处免费  ruguo如果需要可以解开
	////购买物品 判断配置文件
	...
	//判断玩家的金钱 和 物品的价格    此处扣除玩家货币  扣除逻辑已改此处一直是true
	...
	if (IterItem->second.ShopType == SIT_Entity)//实物兑换
	{
		//购买实体物品 发送命令
	...
	else
	{
		...
		////////////////////////////////////////////////////////////////////////////
		//修改购买逻辑不扣分无限购买    直接返还玩家所花金币和钻石

		//LogInfoToFile("RoleManager.txt", "OnHandleTCPNetworkShop购买数量***::::::%d ====::%d*******%d----%d------", pRole->GetUserID(), IterItem->second.ShopType, ItemSum, ItemIndex);
		//if (IterItem->second.ShopType == SIT_GlobelBag)
		//{
		//	g_DBLogManager.LogToDB(pRole->GetUserID(), LT_GlobelBag, static_cast<int>(ItemSum),0, TEXT("购买聚宝盆"), SendLogDB);
		//}
		...
		//tagItemOnce pOnce = IterItem->second.ItemInfo;
		//pOnce.ItemSum *= ItemSum;
		//pRole->GetItemManager().OnAddUserItem(pOnce);//增加物品数量

		//////////////////////////////////////////////////////////////////////////


		//pRole->ChangeRoleGlobe(IterItem->second.PriceCurrey*ItemSum,true);//返还金币
		//pRole->ChangeRoleMedal(IterItem->second.PriceMabel*ItemSum,TEXT("物品免费 归还玩家奖牌"));  //奖牌暂时没有定
		//pRole->ChangeRoleCurrency(IterItem->second.PriceCurrey*ItemSum, TEXT("物品免费  归还玩家钻石"));//玩家钻石返还


		//if (!pRole->GetItemManager().OnAddUserItem(pOnce))
		//{
		//	pRole->ChangeRoleGlobe(IterItem->second.PriceCurrey*ItemSum,true);//返还金币
		//	pRole->ChangeRoleMedal(IterItem->second.PriceMabel*ItemSum,TEXT("商店购买物品 添加物品失败 归还玩家奖牌"));  //奖牌暂时没有定
		//	pRole->ChangeRoleCurrency(IterItem->second.PriceCurrey*ItemSum, TEXT("商店购买物品 添加物品失败 归还玩家钻石"));//玩家钻石返还
		//	pRole->SendDataToClient(&msg);
		//	ASSERT(false);
		//	pRole->SendDataToClient(&msg);
		//	return;
