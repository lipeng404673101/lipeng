...
TableManager::TableManager() //:m_GameConfig(this)
{
	...
	//KucunNum[]
	//memset(KucunNum, 5000, sizeof(KucunNum));
	KucunNum[0] = 5000;
	KucunNum[1] = 5000;
	KucunNum[2] = 5000;
	KucunNum[3] = 5000;
	...
	ZeroMemory(BlackUser, sizeof(BlackUser));
	ZeroMemory(WightUser, sizeof(WightUser));
	ZeroMemory(m_szIniFileName, sizeof(m_szIniFileName));
	//LogInfoToFile("Control.txt", "TableManager全局桌子管理器::%d::: %d:::%d::::%d::::::::::||||::::::%d::::::", KucunNum[0], KucunNum[1], KucunNum[2], KucunNum[3], BlackUser[0]);
...
void TableManager::OnInit()
{
	...
	TCHAR OutBuf[128];
	_sntprintf(m_szIniFileName, sizeof(m_szIniFileName), TEXT("fishing/BlackOrWight_%d.ini"), 2);
	TCHAR szBuffer[MAX_PATH] = { 0 };
	for (int i = 0; i < 10; i++)
	{
		//读取垃圾玩家
		_sntprintf(szBuffer, CountArray(szBuffer), TEXT("BadUser_%d"), i);
		BlackUser[i] = GetPrivateProfileInt(TEXT("public"), szBuffer, 0, m_szIniFileName);
	//	LogInfoToFile("Control.txt", "OnInit读取黑名单::::: %d::::||||::::::%d::::::", BlackUser[i], i);
	}

	for (int i = 0; i < 10; i++)
	{
		//读取垃圾玩家
		_sntprintf(szBuffer, CountArray(szBuffer), TEXT("GoodUser_%d"), i);
		WightUser[i] = GetPrivateProfileInt(TEXT("public2"), szBuffer, 0, m_szIniFileName);
		//LogInfoToFile("Control.txt", "OnInit读取白名单::::: %d::::||||::::::%d::::::", WightUser[i], i);
	}
...
//修改库存
void TableManager::OnAddTableGlobel(BYTE TableID, double nValue)
{

	KucunNum[TableID] += nValue;
	LogInfoToFile("Control.txt", "OnAddTableGlobel修改桌子的库存值:::::::%ld:::::::::::%d::::::::::%d::::::::::", nValue, TableID, KucunNum[TableID]);
}
...
void TableManager::OnResetTableGlobel(WORD TableID, int64 nValue)
{
	...
	//KucunNum[pTable->GetTableTypeID()] += nValue;
...
//查询库存
double TableManager::QueryTableTypePool(BYTE TableTypeID)
{

	return KucunNum[TableTypeID];
}
...
//黑名单
bool TableManager::IsBlackUser(DWORD deUserID)
{

	for (int i = 0; i < 10; i++)
	{
		if (BlackUser[i] == deUserID)//是白名单
		{
...
//白名单
bool TableManager::IsWhightUser(DWORD dwWUserId)
{
	for (int i = 0; i < 10; i++)
	{
		if (WightUser[i] == dwWUserId)//是白名单
		{
...
bool TableManager::ReadBWIni()
{
	...
	for (int i = 0; i < 10; i++)
	{
		...
		_sntprintf(szBuffer, CountArray(szBuffer), TEXT("BadUser_%d"), i);
		BlackUser[i] = GetPrivateProfileInt(TEXT("public"), szBuffer, 0, m_szIniFileName);
		//LogInfoToFile("Control.txt", "ReadBWIni刷新黑名单::::: %d::::||||::::::%d::::::", BlackUser[i],i);
	}
	...
	for (int i = 0; i < 10; i++)
	{
		...
		_sntprintf(szBuffer, CountArray(szBuffer), TEXT("GoodUser_%d"), i);
		WightUser[i] = GetPrivateProfileInt(TEXT("public2"), szBuffer, 0, m_szIniFileName);
		//LogInfoToFile("Control.txt", "ReadBWIni刷新白名单::::: %d::::||||::::::%d::::::", WightUser[i], i);
	}
